...
export class TextureAtlas {
  ...
  private sourceImage: HTMLImageElement | HTMLCanvasElement | ImageBitmap;
  ...
  constructor(gl: WebGL2RenderingContext | null, desc: AtlasDescriptor) {
    ...
    // way.
    this.width = (desc.image as { width: number }).width;
    this.height = (desc.image as { height: number }).height;
  ...
  upload(gl: WebGL2RenderingContext): void {
    ...
    gl.texImage2D(
      ...
      gl.RGBA,
      gl.UNSIGNED_BYTE,
      this.sourceImage as TexImageSource,
    );
    this.texture = tex;
  }
  ...
  lookupUVRect(frame: number, out: Float32Array, offset: number): boolean {
    if (frame < 0 || frame >= this.frameCount) return false;
    var base = frame * 4;
  ...
  lookupFrameSize(frame: number, out: { w: number; h: number }): boolean {
    if (frame < 0 || frame >= this.frameCount) return false;
    var base = frame * 2;
...
export function makeParticleDiscAtlas(
