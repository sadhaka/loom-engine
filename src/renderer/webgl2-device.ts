...
import {
  ...
  type BlendMode,
  FLOATS_PER_INSTANCE,
} from './sprite-batcher.js';
import {
  SPRITE_VERT_SRC,
  SPRITE_FRAG_SRC,
  UNIT_QUAD_VERTICES,
} from './shaders/sprite-shader-source.js';
...
const ATTRIB_QUAD_VERTEX: number = 0;
const ATTRIB_ORIGIN: number = 1;
const ATTRIB_SIZE: number = 2;
const ATTRIB_UV_RECT: number = 3;
const ATTRIB_TINT: number = 4;

// Bytes per instance. 12 floats * 4 bytes.
const INSTANCE_STRIDE_BYTES: number = FLOATS_PER_INSTANCE * 4;

// Initial dynamic-buffer capacity in instances. Grows by doubling
// whenever the batcher's CPU buffer outgrows it.
const INITIAL_INSTANCE_CAPACITY: number = 1024;
...
// for a typical scene.
const TEXT_CACHE_LIMIT: number = 256;
...
export class WebGL2Device implements IGraphicsDevice {
  ...
  private instanceVBOBytes: number = 0;

  private atlases: Array<TextureAtlas | null> = [];
  ...
  private particleAtlas: TextureAtlas | null = null;
  ...
  private textCache: Map<string, TextureAtlas> = new Map();

  // Context-loss state. While true, every draw is a no-op until we
  // recover. Listeners track the live state.
  private contextLost: boolean = false;
  private boundOnLost: ((e: Event) => void) | null = null;
  private boundOnRestored: (() => void) | null = null;

  // Test/diagnostic hooks.
  private lastUpload: { count: number; floats: number } = { count: 0, floats: 0 };
  ...
  constructor(canvas: HTMLCanvasElement, gl?: WebGL2RenderingContext) {
    ...
    if (!ctx) {
      throw new Error(
        'WebGL2Device: failed to acquire webgl2 context. ' +
    ...
    if (typeof (canvas as { addEventListener?: unknown }).addEventListener === 'function') {
      ...
      canvas.addEventListener('webglcontextlost', this.boundOnLost as EventListener, false);
      canvas.addEventListener('webglcontextrestored', this.boundOnRestored as EventListener, false);
    }
    ...
    this.batcher = new SpriteBatcher(
      (atlas, blend, data, count) => this.executeFlush(atlas, blend, data, count),
  ...
  private compileProgram(): ProgramInfo | null {
    var gl = this.gl;
  ...
  private handleContextLoss(): void {
    ...
    for (var i = 0; i < this.atlases.length; i++) {
      ...
      if (a) a.handleContextLoss();
    }
    this.textCache.forEach((t) => t.handleContextLoss());
    if (this.particleAtlas) this.particleAtlas.handleContextLoss();
  ...
  private handleContextRestored(): void {
    ...
    for (var i = 0; i < this.atlases.length; i++) {
      var a = this.atlases[i];
      if (a && !a.released) a.upload(gl);
    }
    this.textCache.forEach((t) => t.upload(gl));
    if (this.particleAtlas) this.particleAtlas.upload(gl);
  }
  ...
  drawSprite(
    ...
    var cam = this.camera;
    if (!cam) return;
    if (!a.lookupFrameSize(frame, SCRATCH_FRAME_SIZE)) return;
    if (!a.lookupUVRect(frame, SCRATCH_UV, 0)) return;
    ...
    this.batcher.submit(
      a, 'alpha',
      dx, dy, dw, dh,
      SCRATCH_UV[0] ?? 0, SCRATCH_UV[1] ?? 0,
      SCRATCH_UV[2] ?? 1, SCRATCH_UV[3] ?? 1,
      tr, tg, tb, ta,
    );
    this.drawCallCount++;
  }
  ...
  drawText(
    ...
    if (!atlas.lookupFrameSize(0, SCRATCH_FRAME_SIZE)) return;
  ...
  drawParticle(
    ...
    if (!atlas || atlas.released) return;
    ...
    var blend: BlendMode = additive ? 'add' : 'alpha';
    this.batcher.submit(
      atlas, blend,
      dx, dy, dw, dh,
      0, 0, 1, 1,
  ...
  dispose(): void {
    var gl = this.gl;
    if (this.boundOnLost && typeof (this.canvas as { removeEventListener?: unknown }).removeEventListener === 'function') {
      this.canvas.removeEventListener('webglcontextlost', this.boundOnLost as EventListener, false);
    }
    if (this.boundOnRestored && typeof (this.canvas as { removeEventListener?: unknown }).removeEventListener === 'function') {
      this.canvas.removeEventListener('webglcontextrestored', this.boundOnRestored as EventListener, false);
    }
  ...
  getBatcherStats(): { flushCount: number; instanceTotal: number; capacity: number } {
  ...
  // the flush handler.
  _peekLastUpload(): { count: number; floats: number } {
  ...
  private executeFlush(
    ...
    if (!program || !this.vao || !this.instanceVBO) return;
    if (!atlas.texture) return; // atlas never uploaded (e.g. context-lost mid-frame)
    ...
    if (bytesNeeded > this.instanceVBOBytes) {
      ...
      while (newBytes < bytesNeeded) newBytes *= 2;
    ...
    gl.bufferSubData(
      gl.ARRAY_BUFFER, 0,
      data.subarray(0, count * FLOATS_PER_INSTANCE),
    );
  ...
  private getOrBakeText(text: string, style: TextStyle): TextureAtlas | null {
    ...
    if (hit && !hit.released && hit.texture) return hit;
    ...
    if (this.textCache.size >= TEXT_CACHE_LIMIT) {
      var firstKey = this.textCache.keys().next().value as string | undefined;
      if (firstKey !== undefined) {
        var oldest = this.textCache.get(firstKey);
        if (oldest) oldest.dispose(this.gl);
