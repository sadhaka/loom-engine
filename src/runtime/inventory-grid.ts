...
export interface ItemInfo {
  // Maximum stack size for this item. Default 1 (non-stackable).
  // Items with maxStack > 1 will merge into existing slots until
  // they fill, then occupy further slots if available.
  maxStack?: number;
}

export interface InventoryGridOptions {
  // Number of slots. Required; must be > 0.
  capacity: number;
  // Optional callback returning per-item config. The inventory
  // calls this every time it adds / merges to discover the maxStack.
  // If omitted, items are non-stackable (maxStack = 1).
  itemInfo?: (itemId: string) => ItemInfo;
  // Optional notification when slots change.
  onChanged?: (slotIndex: number) => void;
}

export interface AddResult {
  // Number of units actually added to the inventory.
  added: number;
  // Number of units that didn't fit (full inventory or no slot).
  overflow: number;
}
...
export class InventoryGrid {
  ...
  private itemInfoFn: ((itemId: string) => ItemInfo) | null;
  private onChanged: ((slotIndex: number) => void) | null;
  private disposed: boolean = false;

  private constructor(opts: InventoryGridOptions) {
  ...
  getSlot(index: number): InventorySlot | null {
    ...
    if (index < 0 || index >= this.capacityNum) return null;
  ...
  add(itemId: string, count: number = 1): AddResult {
    if (this.disposed) return { added: 0, overflow: count };
    if (typeof itemId !== 'string' || itemId.length === 0) {
      return { added: 0, overflow: count };
    }
    var c = Math.floor(count);
    if (c <= 0) return { added: 0, overflow: 0 };
  ...
  remove(itemId: string, count: number = 1): number {
    ...
    if (typeof itemId !== 'string' || itemId.length === 0) return 0;
    var c = Math.floor(count);
    if (c <= 0) return 0;
    var removed = 0;
    var remaining = c;
  ...
  move(from: number, to: number): boolean {
    ...
    if (from < 0 || from >= this.capacityNum) return false;
    if (to < 0 || to >= this.capacityNum) return false;
    ...
    if (dst.itemId === src.itemId) {
      ...
      if (room <= 0) return false; // dst already full of same item
  ...
  toSnapshot(): Array<InventorySlot | null> {
    var out: Array<InventorySlot | null> = new Array(this.capacityNum);
  ...
  private maxStackOf(itemId: string): number {
    if (!this.itemInfoFn) return 1;
    var info: ItemInfo;
    try { info = this.itemInfoFn(itemId); } catch { return 1; }
    var ms = info && info.maxStack !== undefined ? info.maxStack : 1;
    return ms > 0 ? Math.floor(ms) : 1;
  }

  private fireChanged(idx: number): void {
    if (!this.onChanged) return;
    try { this.onChanged(idx); } catch {
      // Best-effort.
    }
  }
}
