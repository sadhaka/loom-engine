...
// A packed (generation, slot) reference to a material, the way an
...
export function makeMaterialHandle(slot: number, generation: number): MaterialHandle {
  return ((generation & MATERIAL_GENERATION_MASK) << MATERIAL_GENERATION_SHIFT)
    | (slot & MATERIAL_INDEX_MASK);
}

export function materialSlot(handle: MaterialHandle): number {
  return handle & MATERIAL_INDEX_MASK;
}

export function materialGeneration(handle: MaterialHandle): number {
  return (handle >>> MATERIAL_GENERATION_SHIFT) & MATERIAL_GENERATION_MASK;
}

// One row of the transition table. A material of `fromType` whose
// fatigue reaches `fatigueThreshold` fires this rule; when several
// rules fire for the same material, the highest `priority` wins.
export interface TransitionRule {
  fromType: number;
  fatigueThreshold: number;
  // The material's new type. Ignored when `recycle` is true.
  toType: number;
  priority: number;
  // When true the material is recycled (its slot freed, generation
  // bumped) instead of transformed.
  recycle: boolean;
}

export interface DecayStats {
  // Materials whose fatigue advanced this applyDecay call.
  decayed: number;
  // Phase-change commands emitted this call (queued, not yet committed).
  transitioned: number;
}

export interface CommitStats {
  // Phase-change commands applied.
  applied: number;
  // Commands rejected because the slot's generation no longer matched
  // (it was recycled and reused) or the slot is no longer active.
  rejected: number;
}
...
export class LoomDecay {
  ...
  private readonly matType: Uint16Array;
  private readonly matFatigue: Uint16Array;
  private readonly matFlags: Uint8Array;
  private readonly matGeneration: Uint8Array;

  // Transition table columns (gate 4), indexed by rule.
  private readonly ruleFromType: Uint16Array;
  private readonly ruleThreshold: Uint16Array;
  private readonly ruleToType: Uint16Array;
  private readonly rulePriority: Int32Array;
  private readonly ruleRecycle: Uint8Array;
  ...
  private readonly cmdSlot: Int32Array;
  private readonly cmdGeneration: Uint8Array;
  private readonly cmdToType: Uint16Array;
  private readonly cmdRecycle: Uint8Array;
  private cmdCount: number = 0;

  // Per-chunk hibernation tracking (gate 5).
  private readonly lastDecayTick: Uint32Array;
  private readonly chunkEverDecayed: Uint8Array;
  ...
  constructor(chunkCount: number, chunkSize: number, transitionRules: readonly TransitionRule[]) {
    ...
    if (capacity > MAX_CAPACITY) {
      throw new RangeError(
        'LoomDecay: chunkCount * chunkSize = ' + capacity + ' exceeds the cap ' + MAX_CAPACITY,
      );
    }
    if (!Array.isArray(transitionRules) || transitionRules.length > MAX_RULES) {
      throw new RangeError(
        'LoomDecay: transitionRules must be an array of at most ' + MAX_RULES + ' rules',
      );
    }
  ...
  isAlive(handle: MaterialHandle): boolean {
    ...
    if (((this.matFlags[slot] ?? 0) & MATERIAL_FLAG_ACTIVE) === 0) return false;
  ...
  getType(handle: MaterialHandle): number {
    if (!this.isAlive(handle)) return -1;
  ...
  applyDecay(
    ...
    for (let i = 0; i < this.chunkSize; i++) {
      const slot = base + i;
      if (((this.matFlags[slot] ?? 0) & MATERIAL_FLAG_ACTIVE) === 0) continue;
      let decaySteps = 0;
      for (let step = 0; step < catchup; step++) {
        if (entropy.random() < factor) decaySteps++;
      }
      if (decaySteps === 0) continue;
      decayed++;
      let nextFatigue = (this.matFatigue[slot] ?? 0) + decaySteps;
      if (nextFatigue > MAX_FATIGUE) nextFatigue = MAX_FATIGUE;
  ...
  private findTransition(type: number, fatigue: number): number {
    let best = -1;
    let bestPriority = 0;
    for (let r = 0; r < this.ruleCount; r++) {
      if ((this.ruleFromType[r] ?? 0) !== type) continue;
      if (fatigue < (this.ruleThreshold[r] ?? 0)) continue;
  ...
  private emitCommand(slot: number, generation: number, toType: number, recycle: boolean): void {
