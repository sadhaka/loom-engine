...
export interface VoxelComputeConfig {
  // Number of chunks the kernel manages. Each consumes
  // chunkSize^3 * 2 bytes (density front+back) + chunkSize^3 bytes
  // (material) + vertexCapacity * VERTEX_STRIDE * 4 bytes.
  maxChunks: number;
  // Per-chunk edge length in voxels. Total cells per chunk =
  // (chunkSize - 1)^3 (cells span between voxel corners).
  chunkSize: number;
  // Per-chunk vertex buffer capacity. <= MAX_VERTEX_CAPACITY.
...
export class VoxelComputeSystem {
  ...
  private readonly densityBuf0: Int8Array;
  private readonly densityBuf1: Int8Array;
  private readonly frontIsBuf0: Uint8Array;            // [chunk]
  private readonly material: Uint8Array;
  private readonly chunkEpoch: Uint32Array;            // [chunk] - bumped on promote

  // Per-chunk vertex buffer (gate 1, 3). SoA Int32 fp.
  private readonly vertexBuffer: Int32Array;
  private readonly vertexCount: Uint32Array;           // [chunk]

  // Marching cubes tables (gate 2). Loaded externally; meshChunk
  // throws RangeError until both have been loaded.
  private readonly edgeTable: Uint16Array;
  private readonly triTable: Int8Array;
  private edgeTableLoaded: boolean = false;
  private triTableLoaded: boolean = false;
  ...
  private readonly counterResetBuffer: Uint32Array;

  // Counters.
  private currentTick: number = 0;
  private vertexOverflowTotal: number = 0;
  private cellsMeshedTotal: number = 0;
  private trianglesEmittedTotal: number = 0;
  ...
  getVertexCount(chunkId: number): number {
    if (!this.requireChunk(chunkId)) return 0;
  ...
  getVertexOverflowTotal(): number { return this.vertexOverflowTotal; }
  getCellsMeshedTotal(): number { return this.cellsMeshedTotal; }
  getTrianglesEmittedTotal(): number { return this.trianglesEmittedTotal; }
  isReady(): boolean { return this.edgeTableLoaded && this.triTableLoaded; }
  ...
  setEdgeTable(table: Uint16Array | number[]): boolean {
    if (!table || table.length !== MC_EDGE_TABLE_SIZE) return false;
    for (let i = 0; i < MC_EDGE_TABLE_SIZE; i++) {
      const v = table[i] ?? 0;
      if (!Number.isInteger(v) || v < 0 || v > 0xffff) return false;
  ...
  setTriTable(table: Int8Array | number[]): boolean {
    if (!table || table.length !== MC_TRI_TABLE_SIZE) return false;
    for (let i = 0; i < MC_TRI_TABLE_SIZE; i++) {
      const v = table[i] ?? 0;
      if (!Number.isInteger(v) || v < -1 || v > 11) return false;
      this.triTable[i] = v | 0;
    }
  ...
  setVoxelDensity(chunkId: number, x: number, y: number, z: number, density: number): boolean {
    ...
    if (!this.requireVoxelCoord(x, y, z)) return false;
    if (!Number.isInteger(density) || density < -128 || density > 127) return false;
  ...
  setVoxelMaterial(chunkId: number, x: number, y: number, z: number, materialId: number): boolean {
    if (!this.requireChunk(chunkId)) return false;
    if (!this.requireVoxelCoord(x, y, z)) return false;
    if (!Number.isInteger(materialId) || materialId < 0 || materialId > 255) return false;
  ...
  getFrontDensity(chunkId: number, x: number, y: number, z: number): number {
    if (!this.requireChunk(chunkId)) return 0;
    if (!this.requireVoxelCoord(x, y, z)) return 0;
    const idx = this.voxelIdx(chunkId, x, y, z);
    ...
    for (let z = 0; z < cs - 1; z++) {
      for (let y = 0; y < cs - 1; y++) {
        for (let x = 0; x < cs - 1; x++) {
          ...
          if (cornerD0 < 0) cornerMask |= 1;
          if (cornerD1 < 0) cornerMask |= 2;
          if (cornerD2 < 0) cornerMask |= 4;
          if (cornerD3 < 0) cornerMask |= 8;
          if (cornerD4 < 0) cornerMask |= 16;
          if (cornerD5 < 0) cornerMask |= 32;
          if (cornerD6 < 0) cornerMask |= 64;
          if (cornerD7 < 0) cornerMask |= 128;
          ...
          for (let e = 0; e < 12; e++) {
            ...
            if (db !== da) {
              ...
              if (tFp < 0) tFp = 0;
              if (tFp > VOXEL_FP_ONE) tFp = VOXEL_FP_ONE;
            }
          ...
          for (let i = 0; i < 16; i += 3) {
            ...
            const vxn = v2x - v0x; const vyn = v2y - v0y; const vzn = v2z - v0z;
  ...
  private emitVertex(chunkId: number, px: number, py: number, pz: number,
  ...
  readVertex(chunkId: number, vertexIdx: number, out: Int32Array, outOffset: number = 0): boolean {
    if (!this.requireChunk(chunkId)) return false;
    if (!Number.isInteger(vertexIdx) || vertexIdx < 0
      || vertexIdx >= (this.vertexCount[chunkId] ?? 0)) return false;
    if (outOffset < 0 || outOffset + VOXEL_VERTEX_STRIDE > out.length) return false;
  ...
  getVertexBufferView(chunkId: number): Int32Array | null {
    if (!this.requireChunk(chunkId)) return null;
  ...
  private requireVoxelCoord(x: number, y: number, z: number): boolean {
    return Number.isInteger(x) && Number.isInteger(y) && Number.isInteger(z)
      && x >= 0 && y >= 0 && z >= 0
      && x < this.chunkSize && y < this.chunkSize && z < this.chunkSize;
  }

  private voxelIdx(chunkId: number, x: number, y: number, z: number): number {
    return chunkId * this.chunkVoxels + this.localIdx(x, y, z, this.chunkSize);
  }

  private localIdx(x: number, y: number, z: number, cs: number): number {
