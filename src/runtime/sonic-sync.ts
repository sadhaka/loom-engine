...
// Pack three small ints into a 64-bit-ish composite key. sourceSlot
// and listenerSlot are <= 16 bits; semanticId is <= 16 bits.
// hi = (sourceSlot << 16) | listenerSlot; lo = semanticId. The
// (0, 0, 0) triple is reserved as the EMPTY sentinel; addSource /
// addListener guarantee slot 0 is never (source=0, listener=0,
// semantic=0) by reserving (0,0,0) as a forbidden combo (we still
// allow source 0 and listener 0 individually - just not all-zero
// together with semantic 0).

export interface SonicSyncConfig {
  // Source pool capacity. Sources are SoA-stored; a source's slot is
  // assigned by addSource and stable until removeSource. <=
  // MAX_SOURCE_CAPACITY.
  maxSources: number;
  // Listener pool capacity, same semantics. <= MAX_LISTENER_CAPACITY.
  maxListeners: number;
  // Per-axis voxel count. The occlusion grid is voxelGridSize ** 3 u8
  // cells, indexed by [x + y*size + z*size*size]. The world fits in
  // [0, voxelGridSize * FP_ONE) on each axis.
  voxelGridSize: number;
  // Maximum ray length in voxels - DDA terminates after this many
  // voxel-boundary crossings even if the ray has not exited the grid.
  // <= MAX_RAY_LENGTH.
  maxRayLength: number;
  // Semantic ID space upper bound. semanticId < maxSemanticId; the
  // listener mask covers semanticId's low 32 categories (a listener
  // can opt-in to up to 32 distinct categories at once via the mask).
  // <= MAX_SEMANTIC_ID.
  maxSemanticId: number;
  // Output ring capacity per buffer. eventsDropped is incremented
  // when the back ring is full. <= MAX_EVENT_CAPACITY.
  eventCapacity: number;
  // Cooldown window in ticks. A (source, listener, semanticId) emit
  // is suppressed until this many ticks have elapsed since the prior
  // emit. 0 disables the cooldown (every audible pair emits every
  // call). <= MAX_COOLDOWN_TICKS.
...
export class SonicSync {
  ...
  private readonly cooldownMask: number;

  // Source SoA. positionX/Y/Z are Q16.16 Int32 fp; semanticId u16;
  // intensity u8 (0..255 = silent..maximum); active u8 (0/1).
  private readonly sourcePositionX: Int32Array;
  private readonly sourcePositionY: Int32Array;
  private readonly sourcePositionZ: Int32Array;
  private readonly sourceSemanticId: Uint16Array;
  private readonly sourceIntensity: Uint8Array;
  ...
  private readonly listenerPositionX: Int32Array;
  private readonly listenerPositionY: Int32Array;
  private readonly listenerPositionZ: Int32Array;
  private readonly listenerHearingRadius: Int32Array;
  private readonly listenerSemanticMask: Uint32Array;
  private readonly listenerActive: Uint8Array;
  ...
  // these along the ray.
  private readonly voxelGrid: Uint8Array;
  ...
  private readonly eventRing0: Int32Array;
  private readonly eventRing1: Int32Array;
  private frontIsRing0: boolean = true;
  ...
  private readonly cooldownKeyHi: Uint32Array;
  private readonly cooldownKeyLo: Uint32Array;
  private readonly cooldownTick: Uint32Array;
  private cooldownEntryCount: number = 0;

  // Per-octant DDA step direction LUTs (gate 1). Filled in the
  // constructor; never re-allocated. Indexed by octant in [0, 8).
  private readonly octantStepX: Int8Array;
  private readonly octantStepY: Int8Array;
  private readonly octantStepZ: Int8Array;

  // Preallocated per-pair working buffer (gate 1). Holds the current
  // ray's direction Int32 dx/dy/dz - never freshly allocated.
  private readonly rayDirection: Int32Array;
  ...
  constructor(config: SonicSyncConfig) {
    ...
    if (!Number.isInteger(maxSources) || maxSources < 1 || maxSources > MAX_SOURCE_CAPACITY) {
      throw new RangeError(
        'SonicSync: maxSources must be an integer in [1, ' + MAX_SOURCE_CAPACITY + '], got ' + maxSources,
      );
    }
    if (!Number.isInteger(maxListeners) || maxListeners < 1 || maxListeners > MAX_LISTENER_CAPACITY) {
      throw new RangeError(
        'SonicSync: maxListeners must be an integer in [1, ' + MAX_LISTENER_CAPACITY + '], got ' + maxListeners,
      );
    }
    if (!Number.isInteger(voxelGridSize) || voxelGridSize < 2 || voxelGridSize > MAX_VOXEL_GRID_SIZE) {
      throw new RangeError(
        'SonicSync: voxelGridSize must be an integer in [2, ' + MAX_VOXEL_GRID_SIZE + '], got ' + voxelGridSize,
      );
    }
    if (!Number.isInteger(maxRayLength) || maxRayLength < 1 || maxRayLength > MAX_RAY_LENGTH) {
      throw new RangeError(
        'SonicSync: maxRayLength must be an integer in [1, ' + MAX_RAY_LENGTH + '], got ' + maxRayLength,
      );
    }
    if (!Number.isInteger(maxSemanticId) || maxSemanticId < 1 || maxSemanticId > MAX_SEMANTIC_ID) {
      throw new RangeError(
        'SonicSync: maxSemanticId must be an integer in [1, ' + MAX_SEMANTIC_ID + '], got ' + maxSemanticId,
      );
    }
    if (!Number.isInteger(eventCapacity) || eventCapacity < 1 || eventCapacity > MAX_EVENT_CAPACITY) {
      throw new RangeError(
        'SonicSync: eventCapacity must be an integer in [1, ' + MAX_EVENT_CAPACITY + '], got ' + eventCapacity,
      );
    }
    if (!Number.isInteger(cooldownTicks) || cooldownTicks < 0 || cooldownTicks > MAX_COOLDOWN_TICKS) {
      throw new RangeError(
        'SonicSync: cooldownTicks must be an integer in [0, ' + MAX_COOLDOWN_TICKS + '], got ' + cooldownTicks,
      );
    }
  ...
  getSourceCount(): number { return this.sourceCount; }
  getListenerCount(): number { return this.listenerCount; }
  getEventsDroppedTotal(): number { return this.eventsDroppedTotal; }
  getFrontEventCount(): number { return this.frontCount; }
  getBackEventCount(): number { return this.backCount; }
  getCurrentTick(): number { return this.currentTick; }
  getCooldownEntryCount(): number { return this.cooldownEntryCount; }
  ...
  addSource(
    ...
    if (this.sourceCount >= this.maxSources) return SOURCE_SLOT_INVALID;
  ...
  updateSource(
    ...
    if (!this.requireSourceSlot(slot)) return false;
    if (!Number.isInteger(positionX)
      || !Number.isInteger(positionY)
      || !Number.isInteger(positionZ)) {
      return false;
    }
    if (!Number.isInteger(intensity) || intensity < 0 || intensity > 255) return false;
    this.sourcePositionX[slot] = positionX | 0;
    this.sourcePositionY[slot] = positionY | 0;
    this.sourcePositionZ[slot] = positionZ | 0;
    this.sourceIntensity[slot] = intensity;
  ...
  addListener(
    ...
    if (this.listenerCount >= this.maxListeners) return LISTENER_SLOT_INVALID;
    if (!Number.isInteger(positionX)
      || !Number.isInteger(positionY)
      || !Number.isInteger(positionZ)) {
  ...
  updateListener(
    ...
    if (!this.requireListenerSlot(slot)) return false;
    if (!Number.isInteger(positionX)
      || !Number.isInteger(positionY)
      || !Number.isInteger(positionZ)) {
      return false;
  ...
  setVoxel(x: number, y: number, z: number, occlusion: number): boolean {
    if (!this.requireVoxelCoords(x, y, z)) return false;
    if (!Number.isInteger(occlusion) || occlusion < 0 || occlusion > 255) return false;
  ...
  getVoxel(x: number, y: number, z: number): number {
    if (!Number.isInteger(x) || x < 0 || x >= this.voxelGridSize) return 0;
    if (!Number.isInteger(y) || y < 0 || y >= this.voxelGridSize) return 0;
    if (!Number.isInteger(z) || z < 0 || z >= this.voxelGridSize) return 0;
  ...
  fillVoxelRegion(
    ...
    if (!Number.isInteger(occlusion) || occlusion < 0 || occlusion > 255) return;
  ...
  traceOcclusion(sourceSlot: number, listenerSlot: number): number {
    if (!this.requireSourceSlot(sourceSlot)) return TRACE_INAUDIBLE;
    if (!this.requireListenerSlot(listenerSlot)) return TRACE_INAUDIBLE;
    ...
    if ((semMask & (1 << (semId & 31))) === 0) return TRACE_INAUDIBLE;
    ...
    if (cheb > radius) return TRACE_INAUDIBLE;
  ...
  private dda(
    ...
    if (dx === 0 && dy === 0 && dz === 0) return ATTENUATION_NONE;
    ...
    if (this.inGrid(vx, vy, vz)) {
      ...
      if (attenuation >= ATTENUATION_FULL) return ATTENUATION_FULL;
    }
    ...
    while (steps < this.maxRayLength) {
      ...
      if (vx === vxEnd && vy === vyEnd && vz === vzEnd) break;

      // Pick the axis with the smallest tNext - that boundary is hit
      // first. Ties broken X > Y > Z (deterministic).
      let axis: number;
      if (tNextX <= tNextY && tNextX <= tNextZ) axis = AXIS_X;
      else if (tNextY <= tNextZ) axis = AXIS_Y;
      else axis = AXIS_Z;

      if (axis === AXIS_X) { vx += stepX; tNextX += tDeltaX; }
      else if (axis === AXIS_Y) { vy += stepY; tNextY += tDeltaY; }
      else { vz += stepZ; tNextZ += tDeltaZ; }

      steps++;

      // Check exit BEFORE accumulation - a ray that has stepped out of
      // the grid has hit no more occlusion.
      if (!this.inGrid(vx, vy, vz)) break;

      attenuation += this.voxelGrid[this.voxelIndex(vx, vy, vz)] ?? 0;
      if (attenuation >= ATTENUATION_FULL) return ATTENUATION_FULL;
    }

    if (attenuation > ATTENUATION_FULL) attenuation = ATTENUATION_FULL;
    return attenuation;
  }
  ...
  producePerceptionEvents(): number {
    ...
    for (let s = 0; s < this.sourceCount; s++) {
      if (!this.sourceActive[s]) continue;
      for (let l = 0; l < this.listenerCount; l++) {
        if (!this.listenerActive[l]) continue;
        const att = this.traceOcclusion(s, l);
        if (att === TRACE_INAUDIBLE) continue;
        if (att >= ATTENUATION_FULL) continue;        // fully blocked - no event
        const semId = this.sourceSemanticId[s] ?? 0;
        // Cooldown check (gate 7).
        if (!this.cooldownAllow(s, l, semId, this.currentTick)) continue;
  ...
  readEvent(i: number, out: Int32Array, outOffset: number = 0): boolean {
    if (!Number.isInteger(i) || i < 0 || i >= this.frontCount) return false;
    if (outOffset < 0 || outOffset + PERCEPTION_EVENT_STRIDE > out.length) return false;
  ...
  private cooldownAllow(
    ...
    if (slot < 0) return true;        // never seen - allow
  ...
  private cooldownStamp(
    ...
    if (slot < 0) return;             // table full of valid entries - drop
  ...
  private cooldownProbe(
    ...
    for (let probe = 0; probe < this.cooldownTableSize - 1; probe++) {
      ...
      if (slot === this.cooldownTableSize - 1) continue;        // skip reserved
      ...
      if (eHi === keyHi && eLo === keyLo) return slot;
    }
  ...
  private inGrid(x: number, y: number, z: number): boolean {
    return x >= 0 && y >= 0 && z >= 0
  ...
  private requireSourceSlot(slot: number): boolean {
    return Number.isInteger(slot) && slot >= 0 && slot < this.sourceCount;
  }

  private requireListenerSlot(slot: number): boolean {
    return Number.isInteger(slot) && slot >= 0 && slot < this.listenerCount;
  }

  private requireVoxelCoords(x: number, y: number, z: number): boolean {
  ...
  private backIsRing0(): boolean { return !this.frontIsRing0; }
