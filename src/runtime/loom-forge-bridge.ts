...
export interface LoomForgeBuildContract {
  // The Wasm module must IMPORT its memory (so the JS side allocates
  // and shares the SAB). Sketch had this implicit; here it's
  // required.
  importedSharedMemory: boolean;
  // Minimum + maximum Wasm pages the module declares.
  minPages: number;
  maxPages: number;
  // The Wasm module must be built with SIMD enabled
  // (-msimd128).
  simdEnabled: boolean;
  // Optional explicit feature-detection probe value. The deferred
  // browser-side bootstrap reads this to confirm the runtime
  // supports wasm SIMD before instantiating.
  probeOpcode?: number;
}
...
export class LoomForgeBridge {
  ...
  private readonly contractValid: boolean;

  private state: number = FORGE_STATE_UNINITIALIZED;
  private stepCallback: StepCallback | null = null;
  private currentTick: number = 0;
  private stepsTotal: number = 0;
  private invalidStepsTotal: number = 0;

  // Front-vs-back flag for the position double-buffer (gate 5).
  // false = "front is base"; true = "front is back-buffer".
  private frontIsBack: boolean = false;
  ...
  isInitialized(): boolean { return this.state === FORGE_STATE_READY; }
  isContractValid(): boolean { return this.contractValid; }
  getStepsTotal(): number { return this.stepsTotal; }
  getInvalidStepsTotal(): number { return this.invalidStepsTotal; }
  getFrontIsBack(): boolean { return this.frontIsBack; }
  ...
  completeInit(callback: StepCallback): number {
    if (!this.contractValid) return FORGE_REASON_BAD_CONTRACT;
    if (typeof callback !== 'function') return FORGE_REASON_NO_CALLBACK;
