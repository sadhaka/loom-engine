...
var ONES: { [k: string]: number } = {
  zero: 0, one: 1, two: 2, three: 3, four: 4, five: 5, six: 6, seven: 7,
  eight: 8, nine: 9, ten: 10, eleven: 11, twelve: 12, thirteen: 13,
  fourteen: 14, fifteen: 15, sixteen: 16, seventeen: 17, eighteen: 18,
  nineteen: 19,
};
var TENS: { [k: string]: number } = {
...
export function parseNumberWord(token: string): number | null {
  ...
  if (!t) return null;
  if (Object.prototype.hasOwnProperty.call(ONES, t)) return ONES[t] as number;
  if (Object.prototype.hasOwnProperty.call(TENS, t)) return TENS[t] as number;
  // "twenty-one" / "twenty one"
  var parts = t.split(/[\s-]+/);
  if (parts.length === 2) {
    var tens = TENS[parts[0] as string];
    var ones = ONES[parts[1] as string];
...
// cares). Deterministic.
export function extractCandidateNumbers(text: string): number[] {
  var out: number[] = [];
  ...
  for (var i = 0; i < words.length; i++) {
    var w = words[i] as string;
    if (!w) continue;
    if (TENS[w] !== undefined) {
      var next = (i + 1 < words.length ? words[i + 1] : '') as string;
      var ones = ONES[next];
      if (ones !== undefined && ones >= 1 && ones <= 9) {
        out.push((TENS[w] as number) + ones);
        i++; // consume the ones word
      } else {
        out.push(TENS[w] as number);
      }
    } else if (ONES[w] !== undefined) {
      out.push(ONES[w] as number);
    }
  }
  return out;
}
...
export interface NarrationContractOptions {
  // Numbers at or below this are treated as ambiguous flavor, not mechanics
  // (default 2 - "the two guards"). Set to 0 to scrutinize every number.
...
export function findInventedNumber(
  text: string,
  attested: Iterable<number>,
  opts?: NarrationContractOptions,
): number | null {
  var floor = opts && typeof opts.ignoreAtOrBelow === 'number' ? opts.ignoreAtOrBelow : 2;
  var allowed: Set<number> = new Set();
  if (attested) {
    for (var a of attested) {
      if (typeof a === 'number' && !isNaN(a)) allowed.add(a);
    }
  }
  var candidates = extractCandidateNumbers(text);
  for (var c of candidates) {
    if (c <= floor) continue;        // ambiguous flavor count - not a claim
    if (!allowed.has(c)) return c;   // a number the engine never produced
  }
  return null;
}
