...
export interface ScreenShakeOptions {
  // Trauma decay per second (linear). 1.5 = full decay in ~0.67s.
  // Default 1.5.
  decayPerSecond?: number;
  // Max position offset in pixels at trauma=1. Default 16.
  maxOffsetPx?: number;
  // Max rotation offset in radians at trauma=1. Default 0.05.
  maxAngleRad?: number;
  // RNG callable returning [0, 1). Default Math.random; replays
  // pass an entropy-bound RNG for deterministic jitter.
  rng?: () => number;
}
...
export class ScreenShake {
  ...
  private constructor(opts: ScreenShakeOptions) {
  ...
  isShaking(): boolean { return this.trauma > 0; }
