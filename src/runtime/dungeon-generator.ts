...
export interface DungeonResult {
  width: number;
  height: number;
  // 0 = wall, 1 = floor. Row-major. tiles[y * width + x].
  tiles: Uint8Array;
  rooms: DungeonRoom[];
...
export interface DungeonGeneratorOptions {
  // Seed for the deterministic RNG. Default 'dungeon-seed'.
  seed?: number | string;
  // Map dimensions. Required, > 0.
  width: number;
  height: number;
  // BSP partition limits. Splitting stops when a leaf is smaller
  // than minLeafSize on either axis. Default 8.
  minLeafSize?: number;
  // Room size bounds (inclusive). Each room is placed inside its
  // leaf with random size + offset. Defaults: min 4, max 10.
  minRoomSize?: number;
  maxRoomSize?: number;
  // Maximum BSP recursion depth. Default 6 (gives up to 64 leaves).
  maxDepth?: number;
}
...
export class DungeonGenerator {
  ...
  private minLeafSize: number;
  private minRoomSize: number;
  private maxRoomSize: number;
  private maxDepth: number;
  private rng: () => number;

  private constructor(opts: DungeonGeneratorOptions) {
    if (!(opts.width > 0)) throw new Error('DungeonGenerator: width must be > 0');
    if (!(opts.height > 0)) throw new Error('DungeonGenerator: height must be > 0');
  ...
  private split(node: BSPNode): void {
    ...
    if (node.w <= min * 2 && node.h <= min * 2) return;
    ...
    this.split(node.left as BSPNode);
    this.split(node.right as BSPNode);
  }

  private placeRooms(node: BSPNode, out: DungeonRoom[]): void {
    if (node.left || node.right) {
      if (node.left)  this.placeRooms(node.left,  out);
      if (node.right) this.placeRooms(node.right, out);
      return;
    }
    ...
    if (maxRoomW < this.minRoomSize || maxRoomH < this.minRoomSize) return;
    ...
    var room: DungeonRoom = { x: rx, y: ry, w: rw, h: rh };
    node.room = room;
    out.push(room);
  }

  private connect(node: BSPNode, out: DungeonCorridor[]): void {
    if (!node.left && !node.right) return;
    if (node.left)  this.connect(node.left,  out);
    if (node.right) this.connect(node.right, out);
  ...
  private pickRoom(node: BSPNode): DungeonRoom | null {
  ...
  private carveRooms(rooms: DungeonRoom[], tiles: Uint8Array): void {
    for (var i = 0; i < rooms.length; i++) {
      var r = rooms[i] as DungeonRoom;
  ...
  private carveCorridors(corridors: DungeonCorridor[], tiles: Uint8Array): void {
    for (var i = 0; i < corridors.length; i++) {
      var c = corridors[i] as DungeonCorridor;
