...
export interface AttackerStats {
  // Base attack power.
  attackPower: number;
  // Crit chance in [0, 1]. Default 0 (no crits).
  critChance?: number;
  // Crit multiplier applied to base damage on crit. Default 1.5.
  critMultiplier?: number;
  // Damage variance fraction in [0, 1]. e.g. 0.1 = ±10%. Default 0.
  variance?: number;
  // Optional armor pen (flat reduction of defender armor before
  // mitigation). Default 0.
  armorPen?: number;
  // Optional damage type for type-vs-resist look up (consumer-
  // driven; the formula doesn't interpret this).
  type?: string;
}

export interface DefenderStats {
  // Armor / defense. Mitigation curve: armor / (armor + armorK).
  armor: number;
  // Optional flat damage reduction applied AFTER percentage
  // mitigation. Default 0.
  flatReduction?: number;
  // Optional per-type resist fraction in [0, 1]. Looked up by
  // attacker.type (if both are set). The fraction is subtracted
  // from final damage AFTER armor mitigation.
  resists?: Record<string, number>;
}

export interface DamageOptions {
  // Curve shape constant for armor mitigation. Default 100.
  armorK?: number;
  // Minimum final damage before flat reduction (so armor can't
  // reduce damage to 0). Default 1.
  minDamage?: number;
  // RNG callable returning [0, 1). Default Math.random.
  rng?: () => number;
}

export interface DamageResult {
  // Final damage applied to defender hp.
  final: number;
  // Damage before any mitigation (post-crit, post-variance).
  raw: number;
  // Damage after mitigation but before flat reduction / resist.
  mitigated: number;
  // True if the hit was a critical strike.
  isCrit: boolean;
  // Mitigation fraction applied [0, 1]. raw * (1 - mitigationPct)
  // = mitigated.
...
export function computeDamage(
  ...
  if (mitigationPct < 0) mitigationPct = 0;
  if (mitigationPct > 1) mitigationPct = 1;
  ...
  if (attacker.type && defender.resists && Object.prototype.hasOwnProperty.call(defender.resists, attacker.type)) {
    resistPct = clamp01(defender.resists[attacker.type] as number);
  }
