...
export interface CognitiveMapOptions {
  // World-state slot count.
  stateSize: number;
  // Total task ids the domain may use.
  taskCount: number;
  // Total method ids the domain may use.
  methodCount: number;
  // Capacity of the flat method-subtask list (sum of subtask lengths
  // across all methods).
  totalMethodSubtasks: number;
  // Capacity of the flat method-precondition list.
  totalMethodPreconds: number;
  // Capacity of the flat primitive-precondition list.
  totalPrimPreconds: number;
  // Capacity of the flat primitive-effect list.
  totalPrimEffects: number;
  // Open-task stack capacity (a deeply nested decomposition can fill
  // it; usually a small multiple of the deepest method).
  maxStackDepth: number;
  // Decision-frame stack capacity (one frame per active compound
  // decomposition; the planner never has more than the current DFS
  // depth in compound tasks).
  maxDecisionDepth: number;
  // Undo-log capacity in entries (one entry per applied effect, for
  // every effect on the current attempt path).
  maxUndoLog: number;
  // Maximum plan length (total primitive count produced by findPlan).
  maxPlanLength: number;
  // Goal queue capacity. Default 16.
...
export type PlanResult =
  | { ok: true; planGen: number; planLength: number }
  | {
      ok: false;
      planGen: number;
...
export class CognitiveMap {
  ...
  // ---- domain tables ----
  private readonly taskKind: Uint8Array;
  // Per-primitive slices into the flat precond / effect lists.
  private readonly primPrecondStart: Int32Array;
  private readonly primPrecondCount: Int32Array;
  private readonly primEffectStart: Int32Array;
  private readonly primEffectCount: Int32Array;
  // Flat (slot, value) pairs - parallel arrays for the precondition
  // and effect lists.
  private readonly primPrecondSlots: Int32Array;
  private readonly primPrecondValues: Int32Array;
  private readonly primEffectSlots: Int32Array;
  private readonly primEffectValues: Int32Array;
  ...
  // during planning.
  private primPrecondLen = 0;
  private primEffectLen = 0;

  // Per-method tables.
  private readonly methodTask: Int32Array;
  private readonly methodSubtaskStart: Int32Array;
  private readonly methodSubtaskCount: Int32Array;
  private readonly methodPrecondStart: Int32Array;
  private readonly methodPrecondCount: Int32Array;
  // Flat method-subtask and method-precondition lists.
  private readonly methodSubtaskList: Int32Array;
  private readonly methodPrecondSlots: Int32Array;
  private readonly methodPrecondValues: Int32Array;
  private methodSubtaskLen = 0;
  private methodPrecondLen = 0;
  // How many methods have been defined so far.
  private definedMethods = 0;
  ...
  private readonly taskMethodStart: Int32Array;
  private readonly taskMethodCount: Int32Array;
  private readonly methodIndex: Int32Array;
  ...
  private stackTop = 0;
  private readonly decision: Int32Array;
  private decisionTop = 0;
  private readonly undoLog: Int32Array;
  private undoTop = 0;
  private readonly plan: Int32Array;
  private planLen = 0;

  // ---- output ----
  private _planGen = 0;
  private _failedTaskId = NO_FAILED_TASK;
  private _failedDepth = 0;

  // ---- goal queue ----
  private readonly goalQueue: Int32Array;
  private goalQueueCount = 0;
  ...
  definePrimitive(taskId: number, def: PrimitiveDefinition): void {
    ...
    if (this.taskKind[taskId] !== TASK_UNDEFINED) {
      throw new Error(
        'CognitiveMap.definePrimitive: task ' + taskId + ' is already defined as '
    ...
    if (this.primPrecondLen + preconds.length > this.primPrecondSlots.length) {
      throw new RangeError(
        'CognitiveMap.definePrimitive: totalPrimPreconds capacity '
        + this.primPrecondSlots.length + ' exceeded',
      );
    }
    if (this.primEffectLen + effects.length > this.primEffectSlots.length) {
      throw new RangeError(
        'CognitiveMap.definePrimitive: totalPrimEffects capacity '
    ...
    for (let i = 0; i < preconds.length; i++) {
      const p = preconds[i] as DomainSpec;
    ...
    for (let i = 0; i < effects.length; i++) {
      const e = effects[i] as DomainSpec;
  ...
  defineMethod(def: MethodDefinition): void {
    ...
    if (this.taskKind[def.taskId] === TASK_PRIMITIVE) {
      throw new Error(
        'CognitiveMap.defineMethod: task ' + def.taskId
        + ' is already defined as a primitive',
      );
    }
    if (this.definedMethods >= this.methodCount) {
      throw new RangeError(
        'CognitiveMap.defineMethod: methodCount capacity ' + this.methodCount + ' exceeded',
      );
    }
  ...
  findPlan(goalId: number, stepBudget: number): PlanResult {
    ...
    if (!Number.isInteger(stepBudget) || stepBudget < 1) {
      throw new RangeError(
        'CognitiveMap.findPlan: stepBudget must be a positive integer, got ' + stepBudget,
      );
    }
    this._planGen++;
    this.stackTop = 0;
  ...
  planStep(index: number): number {
    if (!Number.isInteger(index) || index < 0 || index >= this.planLen) {
      throw new RangeError(
        'CognitiveMap.planStep: index ' + index + ' out of [0, ' + this.planLen + ')',
      );
    }
  ...
  enqueueGoal(goalTaskId: number, priority: number): boolean {
    ...
    if (!Number.isInteger(priority)) {
      throw new RangeError(
        'CognitiveMap.enqueueGoal: priority must be an integer, got ' + priority,
      );
    }
    if (this.goalQueueCount >= this.maxQueuedGoals) return false;
  ...
  runScheduler(stepBudget: number): PlanResult | null {
    if (this.goalQueueCount === 0) return null;
  ...
  private tryApplyPrimitive(task: number): boolean {
    if (!this.checkPreconds(
      this.primPrecondStart[task] ?? 0,
      this.primPrecondCount[task] ?? 0,
      this.primPrecondSlots,
      this.primPrecondValues,
    )) {
      return false;
    }
    if (this.planLen >= this.maxPlanLength) return false;
    const effStart = this.primEffectStart[task] ?? 0;
    const effCount = this.primEffectCount[task] ?? 0;
    if (this.undoTop + effCount > this.maxUndoLog) return false;
  ...
  private tryMethod(task: number, methodIdx: number, m: number): boolean {
    if (m < 0) return false;
    if (!this.checkPreconds(
      this.methodPrecondStart[m] ?? 0,
      this.methodPrecondCount[m] ?? 0,
      this.methodPrecondSlots,
      this.methodPrecondValues,
    )) {
      return false;
    }
    ...
    // entries.
    if (this.stackTop + subCount > this.maxStackDepth) return false;
    if (this.decisionTop >= this.maxDecisionDepth) return false;
  ...
  private pushTask(task: number): boolean {
    if (this.stackTop >= this.maxStackDepth) return false;
  ...
  private backtrack(blamedTask: number): boolean {
  ...
  // rollbackAll.
  private rollbackTo(targetLen: number): void {
  ...
  private checkPreconds(
    ...
    slots: Int32Array, values: Int32Array,
  ): boolean {
    for (let i = 0; i < count; i++) {
      const slot = slots[start + i] ?? 0;
      const required = values[start + i] ?? 0;
      if ((this.state[slot] ?? 0) < required) return false;
    }
    return true;
  }

  private failureResult(reason: PlanFailureReason): PlanResult {
    this.rollbackAll();
    return {
      ok: false,
  ...
  private requireMutable(op: string): void {
    if (this.finalized) {
      throw new Error('CognitiveMap.' + op + ': domain is finalized; clear() to redefine');
    }
  }

  private requireFinalized(op: string): void {
    if (!this.finalized) {
      throw new Error('CognitiveMap.' + op + ': call finalize() before planning');
    }
  }

  private requireTask(taskId: number, op: string): void {
    if (!Number.isInteger(taskId) || taskId < 0 || taskId >= this.taskCount) {
      throw new RangeError(
        'CognitiveMap.' + op + ': taskId ' + taskId + ' out of [0, ' + this.taskCount + ')',
      );
    }
  }

  private requireSlot(slot: number, op: string): void {
    if (!Number.isInteger(slot) || slot < 0 || slot >= this.stateSize) {
      throw new RangeError(
        'CognitiveMap.' + op + ': slot ' + slot + ' out of [0, ' + this.stateSize + ')',
      );
    }
  }

  private requireValue(value: number, op: string): void {
    if (!Number.isInteger(value)) {
      throw new RangeError(
        'CognitiveMap.' + op + ': value must be an integer, got ' + value,
      );
    }
  }
}
...
const EMPTY_SPEC_LIST: ReadonlyArray<DomainSpec> = [];

function requireOpts(opts: CognitiveMapOptions): void {
  if (!opts) {
...
function requireCap(name: string, value: number, lo: number, hi: number): void {
  if (!Number.isInteger(value) || value < lo || value > hi) {
    throw new RangeError(
      'CognitiveMap: ' + name + ' must be an integer in [' + lo + ', ' + hi + '], got ' + value,
    );
  }
}

function kindName(kind: number): string {
  if (kind === TASK_PRIMITIVE) return 'primitive';
  if (kind === TASK_COMPOUND) return 'compound';
  return 'undefined';
}
