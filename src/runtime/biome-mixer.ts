...
export interface BiomeSpec<T = Record<string, unknown>> {
  id: string;
  // Elevation range [min, max]. Defaults: [-Infinity, +Infinity]
  // (i.e. accepts any elevation).
  minElev?: number;
  maxElev?: number;
  // Moisture range [min, max]. Defaults: [-Infinity, +Infinity].
  minMoist?: number;
  maxMoist?: number;
  // Optional payload for the consumer (color, asset id, label).
  data?: T;
}

interface InternalBiome<T> {
  id: string;
  minElev: number;
  maxElev: number;
  minMoist: number;
...
export class BiomeMixer<T = Record<string, unknown>> {
  private biomes: InternalBiome<T>[] = [];
  private byId: Map<string, InternalBiome<T>> = new Map();
  private fallbackId: string | null = null;

  private constructor() {}

  static create<T = Record<string, unknown>>(): BiomeMixer<T> {
    return new BiomeMixer<T>();
  }
  ...
  defineBiome(spec: BiomeSpec<T>): boolean {
    ...
    if (maxE < minE || maxM < minM) return false;
    var b: InternalBiome<T> = {
      id: spec.id,
      minElev: minE, maxElev: maxE,
      minMoist: minM, maxMoist: maxM,
    };
    if (spec.data !== undefined) b.data = spec.data;
    this.biomes.push(b);
  ...
  removeBiome(id: string): boolean {
    ...
    if (idx >= 0) this.biomes.splice(idx, 1);
    this.byId.delete(id);
  ...
  classify(elevation: number, moisture: number): string | null {
    for (var i = 0; i < this.biomes.length; i++) {
      var b = this.biomes[i] as InternalBiome<T>;
  ...
  classifyFull(elevation: number, moisture: number): { id: string; data?: T } | null {
    var id = this.classify(elevation, moisture);
    if (id === null) return null;
    var b = this.byId.get(id);
    if (!b) return null;
    var out: { id: string; data?: T } = { id: b.id };
  ...
  list(): string[] {
    var out: string[] = [];
    for (var i = 0; i < this.biomes.length; i++) out.push((this.biomes[i] as InternalBiome<T>).id);
    return out;
  }

  count(): number { return this.biomes.length; }
  hasBiome(id: string): boolean { return this.byId.has(id); }
  getFallback(): string | null { return this.fallbackId; }
