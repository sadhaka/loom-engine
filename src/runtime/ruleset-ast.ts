...
export type MutationNode =
  ...
  | { type: 'if'; condition: DegreeCond; then: MutationNode[]; else?: MutationNode[] }
  | { type: 'foreach_target'; select: { tag: string; limit?: number }; mutations: MutationNode[] }
  | { type: 'repeat'; count: number; mutations: MutationNode[] };

export type CompareOp = 'gt' | 'gte' | 'lt' | 'lte' | 'eq' | 'ne';

// A compare operand - deliberately a CLOSED, RNG-free set (spec 4.1). dice and
// math are NOT permitted: conditions are evaluated zero or more times during
// the fixed-order degree walk, so any PRNG draw inside a condition would make
// PRNG consumption depend on which earlier branches were tested.
export type CompareOperand =
  | { source: 'roll' }
  | { source: 'dc' }
  | { source: 'delta' }
  | { source: 'natural' }
  | { source: 'prop'; target: string; property: string }
...
export type DegreeCond =
  ...
  | { type: 'nat_roll_gte'; value: number }
  | { type: 'nat_roll_lte'; value: number }
  | { type: 'or'; conditions: DegreeCond[] }
  | { type: 'and'; conditions: DegreeCond[] }
  | { type: 'compare'; op: CompareOp; left: CompareOperand; right: CompareOperand }
  | { type: 'has_tag'; target: string; tag: string };
...
export interface EvalContext {
  ...
  // v2: the entity bound by the innermost executing foreach_target ('each').
  // Saved/restored around each foreach execution; undefined outside any.
...
// them). null in trigger context, where they DO NOT EXIST (spec 1.1).
interface CheckQuantities { roll: number; dc: number; delta: number; natural: number | null; }
...
function resolveTarget(t: string, ctx: EvalContext): string {
  ...
  if (t === 'each') {
    ...
    // alias an entity.
    if (ctx.eachId === undefined) throw new Error('AST: target ref each has no foreach_target binding');
    return ctx.eachId;
  }
...
function requireQuantities(q: CheckQuantities | null, what: string): CheckQuantities {
  if (q === null) throw new Error('AST: ' + what + ' is not valid in trigger context');
  return q;
}
...
function resolveOperand(opnd: CompareOperand, ctx: EvalContext, q: CheckQuantities | null): number | null {
  if (!opnd || typeof opnd !== 'object' || typeof (opnd as { source?: unknown }).source !== 'string') {
  ...
  switch (opnd.source) {
    ...
    default:
      throw new Error('AST: unknown compare operand source: ' + (opnd as { source: string }).source);
  }
}
...
function matchCondition(cond: DegreeCond, ctx: EvalContext, q: CheckQuantities | null, depth: number): boolean {
  ...
  switch (cond.type) {
    ...
    case 'or': {
      ...
      for (var i = 0; i < cond.conditions.length; i++) {
        if (matchCondition(cond.conditions[i] as DegreeCond, ctx, q, depth + 1)) return true; // short-circuit on first TRUE child
      }
      return false; // empty `or` evaluates FALSE (v1 behavior, kept - spec 3.2)
    }
    case 'and': {
      if (!Array.isArray(cond.conditions)) throw new Error('AST: and requires a non-empty conditions array');
      for (var j = 0; j < cond.conditions.length; j++) {
        if (!matchCondition(cond.conditions[j] as DegreeCond, ctx, q, depth + 1)) return false; // short-circuit on first FALSE child
      }
    ...
    case 'compare': {
      ...
      if (cond.op === 'gt') return lv > rv;
      if (cond.op === 'gte') return lv >= rv;
      if (cond.op === 'lt') return lv < rv;
      if (cond.op === 'lte') return lv <= rv;
      if (cond.op === 'eq') return lv === rv;
      if (cond.op === 'ne') return lv !== rv;
      throw new Error('AST: unknown compare op: ' + (cond as { op: string }).op);
    }
    case 'has_tag': {
      ...
      if (!tent || !Array.isArray(tent.tags)) return false;
...
function applyMutationInto(node: MutationNode, ctx: EvalContext, q: CheckQuantities | null, applied: AppliedMutation[]): void {
  ...
  switch (node.type) {
    ...
    case 'if': {
      ...
      var taken: MutationNode[];
      if (matchCondition(node.condition, ctx, q, 0)) taken = node.then;
      else taken = node.else === undefined ? [] : node.else;
      for (var bi = 0; bi < taken.length; bi++) {
        applyMutationInto(taken[bi] as MutationNode, ctx, q, applied);
      }
      return;
    }
    case 'foreach_target': {
      ...
      for (var si = 0; si < allIds.length; si++) {
        var candId = allIds[si] as string;
        var cand = ctx.state.entities[candId];
        if (cand && Array.isArray(cand.tags) && cand.tags.indexOf(node.select.tag) >= 0) matched.push(candId);
      }
      ...
      if (matched.length > lim) matched.length = lim;
      ...
      for (var ti = 0; ti < matched.length; ti++) {
        ctx.eachId = matched[ti] as string;
        for (var fmi = 0; fmi < node.mutations.length; fmi++) {
          applyMutationInto(node.mutations[fmi] as MutationNode, ctx, q, applied);
        }
      }
      ctx.eachId = prevEach;
      return;
    ...
    case 'repeat': {
      ...
      for (var it = 0; it < node.count; it++) {
        for (var rmi = 0; rmi < node.mutations.length; rmi++) {
          applyMutationInto(node.mutations[rmi] as MutationNode, ctx, q, applied);
        }
      }
      return;
    }
...
// and.conditions (vacuous truth is fail-open - spec 3.2), malformed dice,
// unclean / __proto__ names, check-only quantities in trigger context (spec 1.1),
// `each` outside any foreach_target body (static scope rule - spec 6.3), bad
// repeat counts / select limits, and the document-global budgets: nodes, dice
// (charged count * M), applied mutations (charged M per leaf), and the
// multiplicity cap M' <= MAX_APPLIED_MUTATIONS at every body entry (spec 8.2).
// Because it runs before evaluateAction/applyTriggeredMutations touch the PRNG,
// a rejected AST advances neither rng nor state - so the reject boundary is
// byte-identical across TS / Rust / Python. Extra (unlisted) fields are IGNORED:
// not walked, not validated, charging no budget (spec 1.4, vector D5).

interface ValidateBudget { nodes: number; dice: number; applied: number; }
...
interface WalkCtx { b: ValidateBudget; trigger: boolean; }
...
function chargeDice(w: WalkCtx, count: number, m: number): void {
  w.b.dice += count * m;
  if (w.b.dice > MAX_DICE_TOTAL) throw new Error('AST: total dice count exceeds budget ' + MAX_DICE_TOTAL);
}

function chargeApplied(w: WalkCtx, m: number): void {
  w.b.applied += m;
  if (w.b.applied > MAX_APPLIED_MUTATIONS) throw new Error('AST: applied-mutation budget exceeded (max ' + MAX_APPLIED_MUTATIONS + ')');
}
...
function validateTargetRef(t: string, eachOk: boolean): void {
  if (t === 'actor' || t === 'self' || t === 'target') return;
  if (t === 'each') {
    if (eachOk) return;
...
function validateExpr(node: ExprNode, w: WalkCtx, depth: number, m: number, eachOk: boolean): void {
  bumpNode(w.b);
  if (depth > MAX_EXPR_DEPTH) throw new Error('AST: expression exceeds max depth ' + MAX_EXPR_DEPTH);
  if (!node || typeof node.type !== 'string') throw new Error('AST: malformed expression node');
  switch (node.type) {
...
function validateOperand(opnd: CompareOperand, w: WalkCtx, eachOk: boolean): void {
  bumpNode(w.b);
  if (!opnd || typeof opnd !== 'object' || typeof (opnd as { source?: unknown }).source !== 'string') {
    throw new Error('AST: malformed compare operand');
  }
  switch (opnd.source) {
    ...
    case 'delta':
    case 'natural':
      if (w.trigger) throw new Error('AST: compare operand source ' + opnd.source + ' is not valid in trigger context');
      return;
    case 'prop':
...
function validateDegreeCond(cond: DegreeCond, w: WalkCtx, depth: number, eachOk: boolean): void {
  bumpNode(w.b);
  if (depth > MAX_EXPR_DEPTH) throw new Error('AST: degree condition exceeds max depth ' + MAX_EXPR_DEPTH);
  if (!cond || typeof cond.type !== 'string') throw new Error('AST: malformed degree condition');
  switch (cond.type) {
    ...
    case 'nat_roll_lte':
      ...
      if (w.trigger) throw new Error('AST: ' + cond.type + ' is not valid in trigger context');
    ...
    case 'or':
      ...
      if (!Array.isArray(cond.conditions)) throw new Error('AST: or condition requires a conditions array');
      for (var i = 0; i < cond.conditions.length; i++) {
        validateDegreeCond(cond.conditions[i] as DegreeCond, w, depth + 1, eachOk);
      }
      return;
    case 'and':
      ...
      for (var j = 0; j < cond.conditions.length; j++) {
        validateDegreeCond(cond.conditions[j] as DegreeCond, w, depth + 1, eachOk);
      }
      return;
    case 'compare':
...
function validateMutation(node: MutationNode, w: WalkCtx, mdepth: number, m: number, eachOk: boolean): void {
  bumpNode(w.b);
  if (!node || typeof node.type !== 'string') throw new Error('AST: malformed mutation node');
  switch (node.type) {
    case 'set_prop':
    ...
    case 'if':
      ...
      if (!Array.isArray(node.then)) throw new Error('AST: if.then must be a mutation array');
      validateMutationList(node.then, w, mdepth + 1, m, eachOk);
      if (node.else !== undefined) {
        if (!Array.isArray(node.else)) throw new Error('AST: if.else must be a mutation array');
        validateMutationList(node.else, w, mdepth + 1, m, eachOk);
      }
      return;
    case 'foreach_target': {
      var sel = node.select;
      if (!sel || typeof sel !== 'object') throw new Error('AST: foreach_target.select must be an object');
      ...
      if (!Array.isArray(node.mutations)) throw new Error('AST: foreach_target.mutations must be an array');
    ...
    case 'repeat': {
      ...
      if (!Array.isArray(node.mutations)) throw new Error('AST: repeat.mutations must be an array');
...
function validateMutationList(mutations: MutationNode[], w: WalkCtx, mdepth: number, m: number, eachOk: boolean): void {
  if (mdepth > MAX_EXPR_DEPTH) throw new Error('AST: mutation structure exceeds max depth ' + MAX_EXPR_DEPTH);
  if (!Array.isArray(mutations)) throw new Error('AST: mutations must be an array');
  for (var i = 0; i < mutations.length; i++) validateMutation(mutations[i] as MutationNode, w, mdepth, m, eachOk);
}
...
export function validateCheck(check: CheckNode): void {
  ...
  var w: WalkCtx = { b: { nodes: 0, dice: 0, applied: 0 }, trigger: false };
...
export function validateTriggeredMutations(mutations: MutationNode[]): void {
  var w: WalkCtx = { b: { nodes: 0, dice: 0, applied: 0 }, trigger: true };
...
export function applyTriggeredMutations(
  ...
  var ctx2: EvalContext = { state: work, actorId: ctx.actorId, targetId: ctx.targetId, rng: ctx.rng, naturalRoll: null, eachId: undefined };
  var applied: AppliedMutation[] = [];
  for (var i = 0; i < mutations.length; i++) {
    applyMutationInto(mutations[i] as MutationNode, ctx2, null, applied);
  }
...
export function evaluateAction(state: WorldState, check: CheckNode, ctx: EvalContext): ActionResult {
  ...
  var q: CheckQuantities = { roll: roll, dc: dc, delta: delta, natural: natural };
  var chosen = 'none';
  ...
  for (var d = 0; d < DEGREE_ORDER.length; d++) {
    ...
    if (!branch) continue;
    if (matchCondition(branch.condition, ctx2, q, 0)) { chosen = name; muts = branch.mutations; break; }
  }
  var applied: AppliedMutation[] = [];
  for (var i = 0; i < muts.length; i++) {
    applyMutationInto(muts[i] as MutationNode, ctx2, q, applied);
  }
