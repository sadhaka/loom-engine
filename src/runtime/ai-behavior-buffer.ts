...
// A packed (generation, slot) reference to an observer, the way an
// EntityId packs (generation, index). A handle to a released observer
// fails generation validation.
export type ObserverHandle = number;

export function makeObserverHandle(slot: number, generation: number): ObserverHandle {
  return ((generation & OBSERVER_GENERATION_MASK) << OBSERVER_GENERATION_SHIFT)
    | (slot & OBSERVER_INDEX_MASK);
}

export function observerSlot(handle: ObserverHandle): number {
  return handle & OBSERVER_INDEX_MASK;
}

export function observerGeneration(handle: ObserverHandle): number {
...
export class AIBehaviorBuffer {
  ...
  private readonly observerActive: Uint8Array;       // 1 = slot holds a live observer
  private readonly observerGen: Uint8Array;          // bumped on release (handle guard)
  private readonly observerLastSeen: Uint32Array;    // maxObservers * capacity
  ...
  constructor(
    ...
    if (!Number.isInteger(capacity) || capacity < 1 || capacity > MAX_CAPACITY) {
      throw new RangeError(
        'AIBehaviorBuffer: capacity must be an integer in [1, ' + MAX_CAPACITY + '], got ' + capacity,
      );
    }
    if (!Number.isInteger(payloadLength) || payloadLength < 1 || payloadLength > MAX_PAYLOAD_LENGTH) {
      throw new RangeError(
        'AIBehaviorBuffer: payloadLength must be an integer in [1, ' + MAX_PAYLOAD_LENGTH + '], got '
        + payloadLength,
      );
    }
    if (!Number.isInteger(maxObservers) || maxObservers < 1 || maxObservers > MAX_OBSERVERS) {
      throw new RangeError(
        'AIBehaviorBuffer: maxObservers must be an integer in [1, ' + MAX_OBSERVERS + '], got '
        + maxObservers,
      );
    }
    ...
    if (totalSlots > MAX_TOTAL_SLOTS) {
      throw new RangeError(
        'AIBehaviorBuffer: capacity * stride = ' + totalSlots + ' exceeds the cap ' + MAX_TOTAL_SLOTS,
      );
    }
    ...
    if (buffer === undefined) {
      ...
      if (buffer.byteLength < byteLength) {
        throw new RangeError(
          'AIBehaviorBuffer: provided buffer byteLength ' + buffer.byteLength
          + ' < required ' + byteLength,
        );
      }
      backing = buffer;
  ...
  writeSnapshot(slot: number, values: ArrayLike<number>, count?: number): number {
    ...
    if (n > this.payloadLength) {
      throw new RangeError(
        'AIBehaviorBuffer.writeSnapshot: count ' + n + ' exceeds payloadLength ' + this.payloadLength,
      );
    }
    if (n > values.length) {
      throw new RangeError(
        'AIBehaviorBuffer.writeSnapshot: count ' + n + ' exceeds values.length ' + values.length,
      );
    }
  ...
  readSnapshot(slot: number, out: Float32Array, attempts?: number): number {
    ...
    for (let attempt = 0; attempt < tries; attempt++) {
      ...
      if (v1 === 0) return SNAPSHOT_NEVER_WRITTEN;
      if ((v1 & 1) === 1) continue;   // a write is in progress - retry
      for (let i = 0; i < copyLen; i++) {
        out[i] = this.f32[base + i] ?? 0;
      }
      const v2 = this.u32[versionIdx] ?? 0;
      if (v1 === v2) return v1;       // consistent
  ...
  createObserver(): ObserverHandle {
    ...
    throw new Error(
      'AIBehaviorBuffer.createObserver: observer registry full (maxObservers=' + this.maxObservers + ')',
    );
  }
  ...
  releaseObserver(handle: ObserverHandle): boolean {
    if (!this.isObserver(handle)) return false;
  ...
  isObserver(handle: ObserverHandle): boolean {
    const s = observerSlot(handle);
    if (s >= this.maxObservers) return false;
    if ((this.observerActive[s] ?? 0) === 0) return false;
  ...
  readChanged(observer: ObserverHandle, slot: number, out: Float32Array, attempts?: number): number {
    ...
    for (let attempt = 0; attempt < tries; attempt++) {
      ...
      if (v1 === 0) return SNAPSHOT_NEVER_WRITTEN;
      if ((v1 & 1) === 1) continue;   // a write is in progress - retry
      if (v1 === (this.observerLastSeen[lastSeenIdx] ?? 0)) return SNAPSHOT_UNCHANGED;
      for (let i = 0; i < copyLen; i++) {
        out[i] = this.f32[base + i] ?? 0;
      }
      const v2 = this.u32[versionIdx] ?? 0;
  ...
  private requireSlot(slot: number, op: string): void {
    if (!Number.isInteger(slot) || slot < 0 || slot >= this.capacity) {
      throw new RangeError(
        'AIBehaviorBuffer.' + op + ': slot ' + slot + ' out of [0, ' + this.capacity + ')',
      );
    }
  }

  private resolveAttempts(attempts: number | undefined, op: string): number {
    if (attempts === undefined) return DEFAULT_READ_ATTEMPTS;
    if (!Number.isInteger(attempts) || attempts < 1 || attempts > MAX_READ_ATTEMPTS) {
      throw new RangeError(
        'AIBehaviorBuffer.' + op + ': attempts must be an integer in [1, ' + MAX_READ_ATTEMPTS
        + '], got ' + attempts,
      );
    }
    return attempts;
  ...
  private requireObserver(handle: ObserverHandle, op: string): number {
    const s = observerSlot(handle);
    if (s >= this.maxObservers
      || (this.observerActive[s] ?? 0) === 0
      || (this.observerGen[s] ?? 0) !== observerGeneration(handle)) {
      throw new RangeError(
        'AIBehaviorBuffer.' + op + ': observer handle is invalid or released',
      );
    }
    return s;
  }
