...
// A packed (generation, slot) reference to a proposed event. A handle
// to a slot that has since been reused fails generation validation -
// the generation is the idempotency epoch (gate 6).
export type EventHandle = number;

export function makeEventHandle(slot: number, generation: number): EventHandle {
  return ((generation & EVENT_GENERATION_MASK) << EVENT_GENERATION_SHIFT)
    | (slot & EVENT_INDEX_MASK);
}

export function eventSlot(handle: EventHandle): number {
  return handle & EVENT_INDEX_MASK;
}

export function eventGeneration(handle: EventHandle): number {
  return (handle >>> EVENT_GENERATION_SHIFT) & EVENT_GENERATION_MASK;
}

// BlackSwan construction parameters. All required - an options object
// rather than six positional numbers so the call site is unambiguous.
export interface BlackSwanConfig {
  // Entropy window length in ticks (the ΔT of the rate).
  entropyWindow: number;
  // Entropy rate at or below which the world is "calm";
  // entropyExceedsThreshold() is true strictly above it.
  chaosThreshold: number;
  // Event-table capacity.
  maxEvents: number;
  // Event-kind enum space: a kind is in [0, maxKinds).
  maxKinds: number;
  // Proposer / provenance id space: a proposer is in [0, maxProposers).
  maxProposers: number;
  // Audit-ring capacity (oldest records are overwritten once full).
  auditLogSize: number;
}
...
export class BlackSwan {
  ...
  private readonly entropyBuckets: Uint32Array;
  // The bucket addEntropy() currently accumulates into.
  private entropyCursor: number = 0;
  // Running sum of entropyBuckets - kept incrementally so entropy() is
  // O(1). A JS number (<= maxEvents-window * U32_MAX, well under 2^53).
  private entropySum: number = 0;

  // --- event table (gate 1 active-slot metadata is evState) ---
  private readonly evState: Uint8Array;          // EVENT_STATE_*
  private readonly evKind: Uint16Array;          // bounded enum (gate 3)
  private readonly evProposer: Uint32Array;      // provenance (gate 5)
  private readonly evScopeRadius: Uint32Array;   // scope radius (gate 5)
  private readonly evTtl: Uint32Array;           // ticks-to-live (gate 5)
  private readonly evActivationTick: Uint32Array; // tick it went live (gate 6)
  private readonly evExpiryTick: Uint32Array;    // activationTick + ttl
  private readonly evGeneration: Uint8Array;     // bumped on reuse - the epoch (gate 6)

  // --- audit ring (gate 5) ---
  // auditCount records ever written, monotonic; auditCount %
  // auditLogSize is the next write position.
  private readonly auditRing: Uint32Array;
  ...
  constructor(config: BlackSwanConfig) {
    ...
    if (!Number.isInteger(entropyWindow) || entropyWindow < 1 || entropyWindow > MAX_ENTROPY_WINDOW) {
      throw new RangeError(
        'BlackSwan: entropyWindow must be an integer in [1, ' + MAX_ENTROPY_WINDOW + '], got ' + entropyWindow,
      );
    }
    if (!Number.isFinite(chaosThreshold) || chaosThreshold < 0) {
      throw new RangeError(
        'BlackSwan: chaosThreshold must be a finite number >= 0, got ' + chaosThreshold,
      );
    }
    if (!Number.isInteger(maxEvents) || maxEvents < 1 || maxEvents > MAX_EVENTS) {
      throw new RangeError(
        'BlackSwan: maxEvents must be an integer in [1, ' + MAX_EVENTS + '], got ' + maxEvents,
      );
    }
    if (!Number.isInteger(maxKinds) || maxKinds < 1 || maxKinds > MAX_KINDS) {
      throw new RangeError(
        'BlackSwan: maxKinds must be an integer in [1, ' + MAX_KINDS + '], got ' + maxKinds,
      );
    }
    if (!Number.isInteger(maxProposers) || maxProposers < 1 || maxProposers > MAX_PROPOSERS) {
      throw new RangeError(
        'BlackSwan: maxProposers must be an integer in [1, ' + MAX_PROPOSERS + '], got ' + maxProposers,
      );
    }
    if (!Number.isInteger(auditLogSize) || auditLogSize < 1 || auditLogSize > MAX_AUDIT_LOG) {
      throw new RangeError(
        'BlackSwan: auditLogSize must be an integer in [1, ' + MAX_AUDIT_LOG + '], got ' + auditLogSize,
      );
    }
  ...
  tick(currentTick: number): number {
    if (!Number.isInteger(currentTick) || currentTick < 0 || currentTick > U32_MAX) {
      throw new RangeError(
        'BlackSwan.tick: currentTick must be an integer in [0, ' + U32_MAX + '], got ' + currentTick,
      );
    }
    ...
    for (let slot = 0; slot < this.maxEvents; slot++) {
      ...
      if (state !== EVENT_STATE_CANARY && state !== EVENT_STATE_ACTIVE) continue;
  ...
  approve(handle: EventHandle): boolean {
    ...
    if (slot < 0 || (this.evState[slot] ?? EVENT_STATE_NONE) !== EVENT_STATE_PROPOSED) return false;
  ...
  activate(handle: EventHandle, asCanary: boolean): boolean {
    const slot = this.resolveSlot(handle);
    if (slot < 0 || (this.evState[slot] ?? EVENT_STATE_NONE) !== EVENT_STATE_APPROVED) return false;
    this.evActivationTick[slot] = this.currentTick;
    let expiry = this.currentTick + (this.evTtl[slot] ?? 0);
    if (expiry > U32_MAX) expiry = U32_MAX;
  ...
  promote(handle: EventHandle): boolean {
    const slot = this.resolveSlot(handle);
    if (slot < 0 || (this.evState[slot] ?? EVENT_STATE_NONE) !== EVENT_STATE_CANARY) return false;
  ...
  exportEventsByState(state: number, outHandles: Int32Array): number {
    if (!Number.isInteger(state) || state < EVENT_STATE_PROPOSED || state > EVENT_STATE_REVOKED) {
      throw new RangeError(
        'BlackSwan.exportEventsByState: state ' + state + ' is not a lifecycle state ['
    ...
    for (let slot = 0; slot < this.maxEvents && count < cap; slot++) {
      if ((this.evState[slot] ?? EVENT_STATE_NONE) !== state) continue;
  ...
  readAuditRecord(index: number, out: Uint32Array): void {
    ...
    if (out.length < AUDIT_RECORD_STRIDE) {
      throw new RangeError(
        'BlackSwan.readAuditRecord: out must hold at least ' + AUDIT_RECORD_STRIDE + ' entries, got ' + out.length,
      );
    }
  ...
  private resolveSlot(handle: EventHandle): number {
    if (!Number.isInteger(handle)) return -1;
    const slot = eventSlot(handle);
    if (slot >= this.maxEvents) return -1;
    if ((this.evState[slot] ?? EVENT_STATE_NONE) === EVENT_STATE_NONE) return -1;
    if ((this.evGeneration[slot] ?? 0) !== eventGeneration(handle)) return -1;
    return slot;
  }

  // Set a slot's state and write the transition to the audit ring.
  private applyTransition(slot: number, toState: number): void {
