...
export interface LoomPulseConfig {
  // Number of distinct vibes (e.g. happy / tense / focused / bored).
  // <= MAX_VIBES.
  maxVibes: number;
  // EMA smoothing factor in Q16.16 [0, PULSE_FP_ONE]. Higher = more
  // weight on history (heavier smoothing). Typical: 0.9 * PULSE_FP_ONE.
  smoothing: number;
  // Per-tick decay applied to vibeValue. fp [0, PULSE_FP_ONE]. 0 =
  // no decay. Default: ~0.01 * PULSE_FP_ONE per tick.
  valueDecayPerTick: number;
  // Per-tick decay applied to vibeConfidence. Without ongoing
  // signals, confidence rots down. Default: ~0.005 * PULSE_FP_ONE.
  confidenceDecayPerTick: number;
  // Per-signal confidence gain (added on injectSignal, capped at
  // PULSE_FP_ONE). Smaller = more signals required before the
  // kernel "trusts" a vibe. Typical: ~0.05 * PULSE_FP_ONE.
  confidenceGainPerSignal: number;
  // Hysteresis activation threshold (the value below which a
  // currently-INACTIVE vibe stays inactive). fp.
  activationThreshold: number;
  // Hysteresis deactivation threshold (the value below which a
  // currently-ACTIVE vibe goes inactive). fp; MUST be <
  // activationThreshold (the gap is the dead zone).
  deactivationThreshold: number;
  // Atmosphere-impact ceiling for clampAtmosphereImpact (gate 7).
  // fp; the effective vibe seen by the atmosphere driver is capped
  // at this. Typical: 0.5 * PULSE_FP_ONE for "subtle local effects."
  maxAtmosphereImpact: number;
  // Per-vibe audit-ring size. Last N raw signals are kept for
  // offline bias / misclassification analysis. <= MAX_AUDIT_RING.
...
export class LoomPulse {
  ...
  private readonly frontVibeValue: Int32Array;
  private readonly frontVibeConfidence: Int32Array;
  private readonly frontActiveFlag: Uint8Array;

  // Back buffer - producers write here; tick() promotes to front.
  private readonly backVibeValue: Int32Array;
  private readonly backVibeConfidence: Int32Array;
  private readonly backActiveFlag: Uint8Array;
  ...
  private readonly auditHead: Uint32Array;
  private readonly auditCount: Uint32Array;
  ...
  // explicitly opt in.
  private playerConsent: boolean = false;

  // Set on consent toggle to clear back state at next tick.
  private pendingConsentClear: boolean = false;

  constructor(config: LoomPulseConfig) {
    const {
      maxVibes, smoothing, valueDecayPerTick, confidenceDecayPerTick,
      confidenceGainPerSignal, activationThreshold, deactivationThreshold,
      maxAtmosphereImpact, auditRingSize,
    } = config;
    ...
    if (deactivationThreshold >= activationThreshold) {
      throw new RangeError(
        'LoomPulse: deactivationThreshold (' + deactivationThreshold
  ...
  injectSignal(vibeId: number, intensity: number): boolean {
    if (!this.requireVibeId(vibeId)) return false;
    if (!Number.isInteger(intensity) || intensity < 0 || intensity > PULSE_FP_ONE) return false;
    if (!this.playerConsent) return true;       // silent drop with consent denied
  ...
  corroborateWithGameplay(vibeId: number, score: number): boolean {
    if (!this.requireVibeId(vibeId)) return false;
    if (!Number.isInteger(score) || score < 0 || score > PULSE_FP_ONE) return false;
    if (!this.playerConsent) return true;       // silent drop
  ...
  getEffectiveVibe(vibeId: number): number {
    if (!this.requireVibeId(vibeId)) return 0;
  ...
  getCorroboratedVibe(vibeId: number, minCorroboration: number): number {
    ...
    if (!Number.isInteger(minCorroboration) || minCorroboration < 0
      || minCorroboration > PULSE_FP_ONE) return 0;
    const corrob = this.corroborationScore[vibeId] ?? 0;
    if (corrob < minCorroboration) return 0;
  ...
  readAuditSample(vibeId: number, i: number, out: Int32Array, outOffset: number = 0): boolean {
    if (!this.requireVibeId(vibeId)) return false;
    if (!Number.isInteger(i) || i < 0) return false;
    const count = this.auditCount[vibeId] ?? 0;
    if (i >= count) return false;
    if (outOffset < 0 || outOffset + AUDIT_RECORD_STRIDE > out.length) return false;
  ...
  private requireVibeId(id: number): boolean {
