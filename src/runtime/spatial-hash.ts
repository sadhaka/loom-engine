...
interface CellList {
  // Entity ids in this cell. Order doesn't matter; we use swap-pop
  // for O(1) removal.
  entities: number[];
}

// Per-entity bookkeeping: the bucket key and the index within that
// cell's entities array. Lets remove() do O(1) swap-pop.
...
export class SpatialHash {
  ...
  private readonly buckets: Map<number, CellList> = new Map();
  private readonly entityIndex: Map<number, EntityRecord> = new Map();
  // Counters for tests.
  private inserts: number = 0;
  private removes: number = 0;
  private updates: number = 0;
  private queries: number = 0;

  constructor(cellSize: number = 32) {
    var s = +cellSize;
    if (!isFinite(s) || s <= 0) s = 32;
    this.cellSize = s;
  }
  ...
  remove(entity: number): boolean {
    ...
    if (cell) {
      ...
      if (rec.indexInBucket !== lastIdx) {
        ...
        if (movedRec) movedRec.indexInBucket = rec.indexInBucket;
      }
      cell.entities.pop();
  ...
  size(): number { return this.entityIndex.size; }
