...
export interface IMemorySource {
  // Returns approximate bytes this source owns. Called on every
  // report() - keep it cheap. Negative or NaN values are clamped
  // to 0 in the report.
  estimateBytes(): number;
}

export interface MemoryReport {
  // Per-source bytes, in registration order.
  bySource: Array<{ name: string; bytes: number }>;
  // Sum across every source.
  totalBytes: number;
  // Number of registered sources at report time.
  sourceCount: number;
}

export interface MemoryBudgetOptions {
  // Optional reporting callback fired on every report() call. Useful
  // for piping to a debug HUD without polling.
  onReport?: (report: MemoryReport) => void;
}

export class MemoryBudget {
  private sources: Map<string, IMemorySource> = new Map();
  private order: string[] = [];
  private onReport: ((r: MemoryReport) => void) | null;
  private disposed: boolean = false;

  private constructor(opts: MemoryBudgetOptions) {
  ...
  register(name: string, source: IMemorySource): void {
    ...
    var key = String(name);
    if (!this.sources.has(key)) this.order.push(key);
  ...
  totalBytes(): number {
    ...
    for (var i = 0; i < this.order.length; i++) {
      var src = this.sources.get(this.order[i] as string);
      if (src) total += safeBytes(src);
    }
    return total;
  }
  ...
  report(): MemoryReport {
    var bySource: Array<{ name: string; bytes: number }> = [];
    var total = 0;
    if (!this.disposed) {
      for (var i = 0; i < this.order.length; i++) {
        var key = this.order[i] as string;
...
function safeBytes(src: IMemorySource): number {
  var v: number;
  try { v = src.estimateBytes(); } catch { return 0; }
...
export function estimateTypedArrayBytes(
  ...arrs: ArrayBufferView[]
): number {
  var total = 0;
  for (var i = 0; i < arrs.length; i++) {
    var a = arrs[i];
    if (a && typeof a.byteLength === 'number') total += a.byteLength;
  }
  return total;
}
...
export function estimateMapBytes(map: Map<unknown, unknown> | null | undefined, perEntryBytes: number = 96): number {
  if (!map) return 0;
...
export function estimateSetBytes(set: Set<unknown> | null | undefined, perEntryBytes: number = 64): number {
  if (!set) return 0;
...
export function estimateArrayBytes(arr: unknown[] | null | undefined, perElementBytes: number): number {
  if (!arr) return 0;
...
export function estimateObjectBytes(obj: object | null | undefined, perPropertyBytes: number = 32): number {
  if (!obj) return 0;
