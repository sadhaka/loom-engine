...
// A packed (generation, slot) reference to a collider, the way an
// EntityId packs (generation, index). A handle to a slot that has
// since been recycled fails generation validation.
export type ColliderHandle = number;

export function makeColliderHandle(slot: number, generation: number): ColliderHandle {
  return ((generation & COLLIDER_GENERATION_MASK) << COLLIDER_GENERATION_SHIFT)
    | (slot & COLLIDER_INDEX_MASK);
}

export function colliderSlot(handle: ColliderHandle): number {
  return handle & COLLIDER_INDEX_MASK;
}

export function colliderGeneration(handle: ColliderHandle): number {
  return (handle >>> COLLIDER_GENERATION_SHIFT) & COLLIDER_GENERATION_MASK;
}

// Result of a step() - the canonical per-frame pipeline.
export interface PhysicsStepStats {
  // Overlapping pairs found by the detect() pass.
  contacts: number;
  // Push-apart operations applied across all resolve() iterations.
  resolved: number;
}
...
export class PhysicsSystem {
  ...
  private readonly contactA: Int32Array;
  private readonly contactB: Int32Array;
  ...
  private maxColliderExtent: number = 0;
  ...
  private positionsDirty: boolean = false;
  private lastSyncGrid: SpatialGrid | null = null;
  private lastSyncEpoch: number = 0;

  constructor(capacity: number, maxContacts: number) {
    if (!Number.isInteger(capacity) || capacity < 1 || capacity > MAX_CAPACITY) {
      throw new RangeError(
        'PhysicsSystem: capacity must be an integer in [1, ' + MAX_CAPACITY + '], got ' + capacity,
      );
    }
    if (!Number.isInteger(maxContacts) || maxContacts < 1 || maxContacts > MAX_CONTACTS) {
      throw new RangeError(
        'PhysicsSystem: maxContacts must be an integer in [1, ' + MAX_CONTACTS + '], got ' + maxContacts,
      );
    }
  ...
  spawn(
    ...
    if (!Number.isInteger(slot) || slot < 0 || slot >= this.capacity) {
      throw new RangeError(
        'PhysicsSystem.spawn: slot ' + slot + ' out of [0, ' + this.capacity + ')',
      );
    }
    ...
    if (!Number.isFinite(halfW) || halfW <= 0 || !Number.isFinite(halfH) || halfH <= 0) {
      throw new RangeError(
        'PhysicsSystem.spawn: halfW / halfH must be positive finite, got ' + halfW + ', ' + halfH,
      );
    }
    ...
    if (((this.flags[slot] ?? 0) & COLLIDER_FLAG_ACTIVE) !== 0) {
      throw new Error(
        'PhysicsSystem.spawn: slot ' + slot + ' is already active - recycle it first',
      );
    }
    this.posX[slot] = x;
    this.posY[slot] = y;
    ...
    const fullW = halfW * 2;
    const fullH = halfH * 2;
    if (fullW > this.maxColliderExtent) this.maxColliderExtent = fullW;
    if (fullH > this.maxColliderExtent) this.maxColliderExtent = fullH;
  ...
  isAlive(handle: ColliderHandle): boolean {
    ...
    if (((this.flags[slot] ?? 0) & COLLIDER_FLAG_ACTIVE) === 0) return false;
  ...
  getX(handle: ColliderHandle): number {
    if (!this.isAlive(handle)) return NaN;
  ...
  integrate(dt: number): void {
    ...
    for (let s = 0; s < this.capacity; s++) {
      ...
      if ((f & COLLIDER_FLAG_ACTIVE) === 0) continue;
      if ((f & COLLIDER_FLAG_STATIC) !== 0) continue;
  ...
  syncGrid(grid: SpatialGrid): void {
    if (grid.maxEntities < this.capacity) {
      throw new RangeError(
        'PhysicsSystem.syncGrid: grid.maxEntities ' + grid.maxEntities
    ...
    if (grid.cellSize < this.maxColliderExtent) {
      throw new RangeError(
        'PhysicsSystem.syncGrid: grid.cellSize ' + grid.cellSize
    ...
    grid.clear();
    for (let s = 0; s < this.capacity; s++) {
      if (((this.flags[s] ?? 0) & COLLIDER_FLAG_ACTIVE) === 0) continue;
  ...
  detect(grid: SpatialGrid): number {
    if (this.positionsDirty) {
      throw new Error(
        'PhysicsSystem.detect: collider positions changed since the last syncGrid() - '
    ...
    if (grid !== this.lastSyncGrid || grid.epoch !== this.lastSyncEpoch) {
      throw new Error(
        'PhysicsSystem.detect: the grid was modified since syncGrid() (or a different grid '
    ...
    for (let i = 0; i < this.capacity; i++) {
      if (((this.flags[i] ?? 0) & COLLIDER_FLAG_ACTIVE) === 0) continue;
      ...
      for (let r = rLo; r <= rHi; r++) {
        for (let c = cLo; c <= cHi; c++) {
          ...
          for (let k = 0; k < n; k++) {
            ...
            if (j <= i) continue;
            if (((this.flags[j] ?? 0) & COLLIDER_FLAG_ACTIVE) === 0) continue;
            ...
            if (adx < ihw + (this.halfW[j] ?? 0) && ady < ihh + (this.halfH[j] ?? 0)) {
              if (this.contactCount >= this.maxContacts) {
                throw new Error(
                  'PhysicsSystem.detect: contact buffer full (maxContacts=' + this.maxContacts
  ...
  resolve(iterations: number = 1): number {
    if (!Number.isInteger(iterations) || iterations < 1 || iterations > MAX_RESOLVE_ITERATIONS) {
      throw new RangeError(
        'PhysicsSystem.resolve: iterations must be an integer in [1, ' + MAX_RESOLVE_ITERATIONS
        + '], got ' + iterations,
      );
    }
    let pushes = 0;
    for (let iter = 0; iter < iterations; iter++) {
      for (let cc = 0; cc < this.contactCount; cc++) {
        ...
        if ((fa & COLLIDER_FLAG_ACTIVE) === 0 || (fb & COLLIDER_FLAG_ACTIVE) === 0) continue;
  ...
  step(dt: number, grid: SpatialGrid, iterations: number = 1): PhysicsStepStats {
    if (!Number.isFinite(dt) || dt < 0) {
  ...
  getContactA(index: number): number {
    if (!Number.isInteger(index) || index < 0 || index >= this.contactCount) {
      throw new RangeError(
        'PhysicsSystem.getContactA: index ' + index + ' out of [0, ' + this.contactCount + ')',
      );
    }
  ...
  getContactB(index: number): number {
    if (!Number.isInteger(index) || index < 0 || index >= this.contactCount) {
      throw new RangeError(
        'PhysicsSystem.getContactB: index ' + index + ' out of [0, ' + this.contactCount + ')',
      );
    }
