...
export interface TierSpec {
  // Stable tier id (e.g. 'common', 'rare', 'legendary').
  id: string;
  // Base selection weight (positive). Default 1.
  weight?: number;
}
...
export interface LootItem<T = Record<string, unknown>> {
  id: string;
  tier: string;
  // Selection weight within the tier. Default 1.
  weight?: number;
  // Optional tags for filtered rolls.
  tags?: string[];
  payload: T;
}
...
export interface RollContext {
  // Optional level passed to scale fn / tag filter.
  level?: number;
  // Optional tags filter; items with their `tags` array set must
  // intersect ctx.tags. Items with no tags are always eligible
  // unless `requireTagMatch: true`.
  tags?: string[];
  // If true, items without a tags array are excluded when ctx.tags
  // is set. Default false.
  requireTagMatch?: boolean;
  // Force a specific tier (skips tier roll).
  tier?: string;
}
...
export interface LootTierOptions {
  // RNG seam. Default seeded mulberry32.
  rng?: RngFn;
  // Seed for default mulberry32 PRNG. Default 1.
  seed?: number;
}
...
export class LootTier<T = Record<string, unknown>> {
  private tiers: Map<string, InternalTier> = new Map();
  private items: Map<string, InternalItem<T>> = new Map();
  // tier id -> array of item ids in that tier.
  private itemsByTierIndex: Map<string, string[]> = new Map();
  private rng: RngFn;
  private tierScaleFn: TierScaleFn | null = null;
  private disposed: boolean = false;

  private constructor(opts: LootTierOptions) {
  ...
  static create<T = Record<string, unknown>>(
    opts: LootTierOptions = {}): LootTier<T> {
    return new LootTier<T>(opts);
  }
  ...
  tierCount(): number { return this.tiers.size; }
  ...
  addItem(item: LootItem<T>): boolean {
    ...
    if (typeof item.tier !== 'string' || !this.tiers.has(item.tier)) return false;
    if (item.payload === undefined || item.payload === null) return false;
    ...
    if (existing && existing.tier !== item.tier) {
      ...
      if (oldArr) {
        ...
        if (ix >= 0) oldArr.splice(ix, 1);
      }
    }
    var internal: InternalItem<T> = {
      id: item.id,
      tier: item.tier,
      weight: weight,
    ...
    if (arr && arr.indexOf(item.id) < 0) arr.push(item.id);
    return true;
  }
  ...
  itemsByTier(tier: string): LootItem<T>[] {
    ...
    if (!ids) return out;
    for (var i = 0; i < ids.length; i++) {
      var item = this.items.get(ids[i] as string);
      if (item) out.push(this.publicItem(item));
    }
    return out;
  }
  ...
  effectiveTierWeights(ctx: RollContext = {}): { id: string; weight: number }[] {
    var out: { id: string; weight: number }[] = [];
  ...
  rollTier(ctx: RollContext = {}): string | null {
    ...
    for (var j = 0; j < weights.length; j++) {
      ...
      if (acc >= target) return weights[j]!.id;
    }
  ...
  private rollItemInTier(tier: string, ctx: RollContext): DropResult<T> | null {
    var ids = this.itemsByTierIndex.get(tier);
    if (!ids || ids.length === 0) return null;
    var requireMatch = ctx.requireTagMatch === true;
    var ctxTags = Array.isArray(ctx.tags) && ctx.tags.length > 0 ? ctx.tags : null;
    var matched: InternalItem<T>[] = [];
    var totalWeight = 0;
    for (var i = 0; i < ids.length; i++) {
      var item = this.items.get(ids[i] as string);
      if (!item) continue;
      if (item.tags === null) {
        if (ctxTags !== null && requireMatch) continue;
      } else {
        // Item has tags; if ctxTags set, require any-match.
        if (ctxTags !== null) {
          var anyMatch = false;
          for (var j = 0; j < (item.tags as string[]).length; j++) {
            if (ctxTags.indexOf((item.tags as string[])[j] as string) >= 0) {
              anyMatch = true;
              break;
            }
          }
    ...
    for (var k = 0; k < matched.length; k++) {
      acc += (matched[k] as InternalItem<T>).weight;
      if (acc >= target) {
        var pick = matched[k] as InternalItem<T>;
        return this.toDropResult(pick);
      }
    }
    var lastPick = matched[matched.length - 1] as InternalItem<T>;
  ...
  private toDropResult(item: InternalItem<T>): DropResult<T> {
    var out: DropResult<T> = {
      tier: item.tier,
      id: item.id,
      payload: item.payload,
    };
    if (item.tags !== null) out.tags = item.tags.slice();
    return out;
  }

  private publicItem(item: InternalItem<T>): LootItem<T> {
    var copy: LootItem<T> = {
      id: item.id,
      tier: item.tier,
      weight: item.weight,
    ...
    if (item.tags !== null) copy.tags = item.tags.slice();
    return copy;
  }
}
