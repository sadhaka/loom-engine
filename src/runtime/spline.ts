...
export interface SplineOptions {
  // Catmull-Rom tension (alpha). 0.5 = centripetal (default,
  // overshoot-free); 0 = uniform; 1 = chordal. Centripetal is the
  // best default for arbitrary point distributions.
  tension?: number;
  // Treat the path as a closed loop. Default false.
  closed?: boolean;
}
...
export function linearPath(points: ReadonlyArray<Vec2Like>, t: number): Vec2Like {
  if (!points || points.length === 0) return { x: 0, y: 0 };
  if (points.length === 1) {
    var only = points[0] as Vec2Like;
    return { x: only.x, y: only.y };
  }
  var ct = clampUnit(t);
  var n = points.length;
  if (ct >= 1) {
    var last = points[n - 1] as Vec2Like;
  ...
  var p0 = points[idx] as Vec2Like;
  var p1 = points[idx + 1] as Vec2Like;
...
export function catmullRomPath(
  ...
  if (ct >= 1) {
    if (!closed) {
      var lastPoint = points[n - 1] as Vec2Like;
...
function pickPoint(points: ReadonlyArray<Vec2Like>, idx: number, closed: boolean): Vec2Like {
  var n = points.length;
  if (closed) {
    var wrapped = ((idx % n) + n) % n;
    return points[wrapped] as Vec2Like;
  }
  if (idx < 0) {
    // Mirror the boundary segment: phantom = 2*p0 - p1.
    var p0 = points[0] as Vec2Like;
    var p1 = points[1] as Vec2Like;
    return { x: 2 * p0.x - p1.x, y: 2 * p0.y - p1.y };
  }
  if (idx >= n) {
    var pn1 = points[n - 1] as Vec2Like;
    var pn2 = points[n - 2] as Vec2Like;
    return { x: 2 * pn1.x - pn2.x, y: 2 * pn1.y - pn2.y };
  }
  return points[idx] as Vec2Like;
}

// Cubic Hermite path evaluator. Explicit tangents per keypoint.
// Each keypoint is { p, m } where p is the position and m is the
// outgoing tangent vector (incoming on segment N+1, outgoing on
// segment N).
...
export function hermitePath(keys: ReadonlyArray<HermiteKey>, t: number): Vec2Like {
  if (!keys || keys.length === 0) return { x: 0, y: 0 };
  if (keys.length === 1) {
    var only = keys[0] as HermiteKey;
    return { x: only.p.x, y: only.p.y };
  }
  var ct = clampUnit(t);
  var n = keys.length;
  if (ct >= 1) {
    var last = keys[n - 1] as HermiteKey;
  ...
  var k0 = keys[idx] as HermiteKey;
  var k1 = keys[idx + 1] as HermiteKey;
