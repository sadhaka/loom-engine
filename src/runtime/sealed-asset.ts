...
export class SealedAssetRegistry {
  ...
  // Per-asset state.
  private readonly assetState: Uint8Array;
  private readonly assetGeneration: Uint8Array;
  private readonly assetEventId: Uint32Array;
  private readonly assetVersion: Uint32Array;
  private readonly assetCdnHash: Uint32Array;
  private readonly assetEntitlementMask: Uint32Array;
  private readonly assetRegionMask: Uint32Array;
  private readonly assetContentHash: Uint8Array;        // sized maxAssets * AAD_HASH_BYTES
  private readonly assetDecryptedByteLength: Uint32Array;
  private readonly assetTransferred: Uint8Array;
  private nextAssetSlot: number = 0;

  // Per-event state.
  private readonly eventKeyDisclosed: Uint8Array;
  private readonly eventEntitlementMask: Uint32Array;
  private readonly eventRegionMask: Uint32Array;

  // Per-cdnHash dedup table - so a client doesn't register the same
  // asset twice. Open-addressed; key 0 means EMPTY.
  private readonly cdnDedupKey: Uint32Array;
  private readonly cdnDedupSlot: Int32Array;
  private readonly cdnDedupMask: number;

  private currentTick: number = 0;
  private failuresTotal: number = 0;
  private successesTotal: number = 0;
  ...
  constructor(config: SealedAssetConfig) {
    ...
    let dedupSize = 1;
    while (dedupSize < maxAssets * 2) dedupSize <<= 1;
  ...
  getFailuresTotal(): number { return this.failuresTotal; }
  getSuccessesTotal(): number { return this.successesTotal; }
  ...
  static validateEnvelope(envelope: Uint8Array): number {
    if (!envelope || envelope.length < ENVELOPE_MIN_BYTES) return SEALED_REASON_BAD_ENVELOPE;
  ...
  static readIV(envelope: Uint8Array): Uint8Array | null {
    if (SealedAssetRegistry.validateEnvelope(envelope) !== SEALED_REASON_NONE) return null;
  ...
  static buildAAD(eventId: number, assetId: number, version: number, contentHash: Uint8Array): Uint8Array | null {
    if (!Number.isInteger(eventId) || eventId < 0 || eventId > U32_MAX) return null;
    if (!Number.isInteger(assetId) || assetId < 0 || assetId > U32_MAX) return null;
    if (!Number.isInteger(version) || version < 0 || version > U32_MAX) return null;
    if (!contentHash || contentHash.length !== AAD_HASH_BYTES) return null;
  ...
  registerAsset(
    ...
    version: number,
    cdnHash: number,
    entitlementMask: number,
    regionMask: number,
    contentHash: Uint8Array,
  ): number {
    if (!this.requireEvent(eventId)) return SEALED_HANDLE_INVALID;
    if (!Number.isInteger(version) || version < 0 || version > U32_MAX) return SEALED_HANDLE_INVALID;
    if (!Number.isInteger(cdnHash) || cdnHash <= 0 || cdnHash > U32_MAX) return SEALED_HANDLE_INVALID;
    if (!Number.isInteger(entitlementMask) || entitlementMask < 0 || entitlementMask > U32_MAX) return SEALED_HANDLE_INVALID;
    if (!Number.isInteger(regionMask) || regionMask < 0 || regionMask > U32_MAX) return SEALED_HANDLE_INVALID;
    if (!contentHash || contentHash.length !== AAD_HASH_BYTES) return SEALED_HANDLE_INVALID;
    // Dedup on cdnHash.
    const existing = this.findCdnSlot(cdnHash);
    if (existing >= 0) return SEALED_HANDLE_INVALID;     // already registered

    const slot = this.allocAssetSlot();
    if (slot < 0) return SEALED_HANDLE_INVALID;
  ...
  findByCdnHash(cdnHash: number): number {
    if (!Number.isInteger(cdnHash) || cdnHash <= 0) return SEALED_HANDLE_INVALID;
  ...
  discloseEventKey(eventId: number, requiredEntitlementMask: number, requiredRegionMask: number): number {
    if (!this.requireEvent(eventId)) return SEALED_REASON_BAD_EVENT;
    if (!Number.isInteger(requiredEntitlementMask) || requiredEntitlementMask < 0
      || requiredEntitlementMask > U32_MAX) return SEALED_REASON_BAD_EVENT;
    if (!Number.isInteger(requiredRegionMask) || requiredRegionMask < 0
      || requiredRegionMask > U32_MAX) return SEALED_REASON_BAD_EVENT;
    if ((this.eventKeyDisclosed[eventId] ?? 0) === 1) return SEALED_REASON_DUPLICATE;
  ...
  isClientEntitledToEvent(
    eventId: number,
    clientEntitlementMask: number,
    clientRegionMask: number,
  ): boolean {
    if (!this.requireEvent(eventId)) return false;
    if ((this.eventKeyDisclosed[eventId] ?? 0) !== 1) return false;
    if (!Number.isInteger(clientEntitlementMask) || clientEntitlementMask < 0) return false;
    if (!Number.isInteger(clientRegionMask) || clientRegionMask < 0) return false;
    const entReq = this.eventEntitlementMask[eventId] ?? 0;
    const regReq = this.eventRegionMask[eventId] ?? 0;
    if ((entReq >>> 0) !== 0 && (((clientEntitlementMask >>> 0) & entReq) === 0)) return false;
    if ((regReq >>> 0) !== 0 && (((clientRegionMask >>> 0) & regReq) === 0)) return false;
    return true;
  }
  ...
  beginDecrypt(handle: number): number {
    if (!this.requireAssetSlot(handle)) return SEALED_REASON_BAD_HANDLE;
    if (this.assetState[handle] !== SEALED_STATE_KEY_DISCLOSED) return SEALED_REASON_BAD_STATE;
  ...
  completeDecrypt(handle: number, generation: number, decryptedByteLength: number, transferred: boolean): number {
    if (!this.requireAssetSlot(handle)) return SEALED_REASON_BAD_HANDLE;
    if ((this.assetGeneration[handle] ?? 0) !== (generation & 0xff)) return SEALED_REASON_STALE_GENERATION;
    if (this.assetState[handle] !== SEALED_STATE_DECRYPTING) return SEALED_REASON_BAD_STATE;
    if (!Number.isInteger(decryptedByteLength) || decryptedByteLength < 0
      || decryptedByteLength > U32_MAX) return SEALED_REASON_BAD_ENVELOPE;
  ...
  revokeAsset(handle: number): number {
    ...
    if (state === SEALED_STATE_NONE) return SEALED_REASON_BAD_STATE;
  ...
  readAsset(handle: number, out: Int32Array, outOffset: number = 0): boolean {
    if (!this.requireAssetSlot(handle)) return false;
    if (outOffset < 0 || outOffset + SEALED_ASSET_RECORD_STRIDE > out.length) return false;
  ...
  readContentHash(handle: number, out: Uint8Array): boolean {
    if (!this.requireAssetSlot(handle)) return false;
    if (out.length < AAD_HASH_BYTES) return false;
    const base = handle * AAD_HASH_BYTES;
    for (let i = 0; i < AAD_HASH_BYTES; i++) out[i] = this.assetContentHash[base + i] ?? 0;
    return true;
  }

  getAssetState(handle: number): number {
    if (!this.requireAssetSlot(handle)) return SEALED_STATE_NONE;
    return this.assetState[handle] ?? SEALED_STATE_NONE;
  }

  getAssetGeneration(handle: number): number {
    if (!this.requireAssetSlot(handle)) return 0;
    return this.assetGeneration[handle] ?? 0;
  }

  // --- helpers ---

  private requireEvent(e: number): boolean {
    return Number.isInteger(e) && e >= 0 && e < this.maxEvents;
  }

  private requireAssetSlot(s: number): boolean {
    return Number.isInteger(s) && s >= 0 && s < this.maxAssets;
  }

  private allocAssetSlot(): number {
  ...
  private findCdnSlot(cdnHash: number): number {
    let h = cdnHash >>> 0;
    ...
    for (let probe = 0; probe < this.cdnDedupKey.length; probe++) {
      ...
      if (k === 0) return -1;
      if (k === (cdnHash >>> 0)) return this.cdnDedupSlot[slot] ?? -1;
    }
    return -1;
  }

  private cdnDedupInsert(cdnHash: number, slot: number): void {
    let h = cdnHash >>> 0;
    h ^= h >>> 16; h = Math.imul(h, 0x85ebca6b);
    h ^= h >>> 13; h = Math.imul(h, 0xc2b2ae35);
    h ^= h >>> 16; h = h >>> 0;
