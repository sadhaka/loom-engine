...
export const COMPONENT_SIGNATURE_MAX_BIT: number = 31;
...
export class ComponentSignature {
  ...
  setBit(entityIdx: number, bit: number): void {
    ...
    if (entityIdx >= this.hwm) this.hwm = entityIdx + 1;
  ...
  collectMatching(mask: number): Int32Array {
    if (mask === 0) return new Int32Array(0);
    var m = mask >>> 0;
    var hwm = this.hwm;
    var i = 0;
