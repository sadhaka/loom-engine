...
export interface NeuralAnimationConfig {
  // Number of entities tracked. Each consumes (boneCount * BONE_SLOT_STRIDE)
  // Int32 of inertOffset + same of inertVelocity, plus per-entity
  // scalars. <= MAX_ENTITIES.
  maxEntities: number;
  // Number of motion clips in the feature DB.
  numClips: number;
  // Number of frames per clip (uniform across all clips).
  framesPerClip: number;
  // Feature vector stride (Int32 fp values per frame). Typical: 27.
  featureStride: number;
  // Per-pose bone count (uniform across all clips).
  boneCount: number;
  // Inertialization half-life in fp ticks. Smaller = faster decay.
  halfLifeFp: number;
}
...
export class NeuralAnimationSystem {
  ...
  // * featureStride + i].
  private readonly featureDB: Int32Array;

  // Pose DB. Indexed [(clip * framesPerClip + frame) * boneCount *
  // BONE_SLOT_STRIDE + bone * BONE_SLOT_STRIDE + slot].
  private readonly poseDB: Int32Array;

  // Per-entity state (gate 2).
  private readonly entityCurrentClip: Int32Array;
  private readonly entityCurrentFrame: Int32Array;
  private readonly entityIntentVx: Int32Array;
  private readonly entityIntentVy: Int32Array;
  private readonly entityIntentVz: Int32Array;
  private readonly entityIntentDirX: Int32Array;
  private readonly entityIntentDirY: Int32Array;
  private readonly entityIntentDirZ: Int32Array;
  private readonly entityFootFlags: Uint8Array;
  private readonly entityActive: Uint8Array;

  // Per-entity per-bone inertialization state (gate 4).
  private readonly inertOffset: Int32Array;
  private readonly inertVelocity: Int32Array;

  // Decay LUT (gate 4).
  private readonly decayLUT: Int32Array;

  private currentTick: number = 0;
  private entityCount: number = 0;
  private transitionsTotal: number = 0;
  private searchesTotal: number = 0;
  ...
  getTransitionsTotal(): number { return this.transitionsTotal; }
  getSearchesTotal(): number { return this.searchesTotal; }
  getFeatureDB(): Int32Array { return this.featureDB; }
  getPoseDB(): Int32Array { return this.poseDB; }
  getInertOffsetArray(): Int32Array { return this.inertOffset; }
  ...
  loadFeatureFrame(clipId: number, frameId: number, source: Int32Array, sourceOffset: number = 0): boolean {
    if (!this.requireClip(clipId)) return false;
    if (!this.requireFrame(frameId)) return false;
    if (sourceOffset < 0 || sourceOffset + this.featureStride > source.length) return false;
  ...
  loadPoseFrame(clipId: number, frameId: number, source: Int32Array, sourceOffset: number = 0): boolean {
    if (!this.requireClip(clipId)) return false;
    if (!this.requireFrame(frameId)) return false;
    const need = this.boneCount * BONE_SLOT_STRIDE;
    if (sourceOffset < 0 || sourceOffset + need > source.length) return false;
  ...
  addEntity(initialClip: number, initialFrame: number): number {
    if (this.entityCount >= this.maxEntities) return ANIM_ENTITY_INVALID;
    if (!this.requireClip(initialClip) || !this.requireFrame(initialFrame)) return ANIM_ENTITY_INVALID;
  ...
  updateEntityIntent(
    ...
    if (!Number.isInteger(vx) || !Number.isInteger(vy) || !Number.isInteger(vz)) return false;
    if (!Number.isInteger(dirX) || !Number.isInteger(dirY) || !Number.isInteger(dirZ)) return false;
  ...
  updateEntityFootFlags(entityId: number, footFlags: number): boolean {
    if (!this.requireEntity(entityId)) return false;
    if (!Number.isInteger(footFlags) || footFlags < 0 || footFlags > 0xff) return false;
  ...
  setEntityClip(entityId: number, newClip: number, newFrame: number): boolean {
    if (!this.requireEntity(entityId)) return false;
    if (!this.requireClip(newClip) || !this.requireFrame(newFrame)) return false;
  ...
  searchBestMatch(entityId: number): number {
    if (!this.requireEntity(entityId)) return -1;
    this.searchesTotal++;
  ...
  step(dtFp: number): void {
    if (!Number.isInteger(dtFp) || dtFp < 0) return;
    ...
    for (let e = 0; e < this.entityCount; e++) {
      if ((this.entityActive[e] ?? 0) === 0) continue;
      const base = e * total;
  ...
  readVisiblePose(entityId: number, bone: number, out: Int32Array, outOffset: number = 0): boolean {
    if (!this.requireEntity(entityId)) return false;
    if (!Number.isInteger(bone) || bone < 0 || bone >= this.boneCount) return false;
    if (outOffset < 0 || outOffset + BONE_SLOT_STRIDE > out.length) return false;
    const clip = this.entityCurrentClip[entityId] ?? -1;
    const frame = this.entityCurrentFrame[entityId] ?? -1;
    if (clip < 0 || frame < 0) {
      for (let s = 0; s < BONE_SLOT_STRIDE; s++) out[outOffset + s] = 0;
      return true;
    }
  ...
  readInertOffset(entityId: number, bone: number, out: Int32Array, outOffset: number = 0): boolean {
    if (!this.requireEntity(entityId)) return false;
    if (!Number.isInteger(bone) || bone < 0 || bone >= this.boneCount) return false;
    if (outOffset < 0 || outOffset + BONE_SLOT_STRIDE > out.length) return false;
    const inertBase = entityId * this.boneCount * BONE_SLOT_STRIDE + bone * BONE_SLOT_STRIDE;
  ...
  private computeDecayIndex(dtFp: number): number {
    ...
    if (idx < 0) idx = 0;
    if (idx >= DECAY_LUT_SIZE) idx = DECAY_LUT_SIZE - 1;
    return idx;
  }

  private requireClip(c: number): boolean {
    return Number.isInteger(c) && c >= 0 && c < this.numClips;
  }

  private requireFrame(f: number): boolean {
  ...
  private _layout = { BONE_TX, BONE_TY, BONE_TZ };
}
