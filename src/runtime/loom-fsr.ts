...
function haltonAt(index: number, base: number): number {
  let result = 0;
  let f = 1;
  let i = index;
  while (i > 0) {
...
export interface LoomFSRConfig {
  // Low-resolution input (post-jitter).
  lowResWidth: number;
  lowResHeight: number;
  // High-resolution output (the upscaled frame).
  highResWidth: number;
  highResHeight: number;
  // Jitter sample count - typically 16 (Halton 2,3). <= MAX_JITTER_SAMPLES.
  jitterSamples: number;
  // Disocclusion thresholds in fp.
  historyDepthThresholdFp: number;
  // dot(currentNormal, historyNormal) below this rejects history.
  // fp in [-FP_ONE, FP_ONE].
  historyNormalThresholdFp: number;
  // Sharpening strength in fp [0, FP_ONE]. 0 = pure TAA; >0 = FSR-class.
...
export class LoomFSR {
  ...
  private readonly jitterX: Int32Array;
  private readonly jitterY: Int32Array;
  private jitterIndex: number = 0;

  // Per-channel ping-pong texture handles (gate 3).
  private readonly frontTextureHandle: Uint32Array;
  private readonly backTextureHandle: Uint32Array;
  private readonly textureFormat: Uint32Array;
  private readonly textureUsage: Uint32Array;
  private readonly textureRegistered: Uint8Array;

  // Per-pixel reactive mask (gate 6). Sized lowResWidth * lowResHeight.
  private readonly reactiveMask: Uint8Array;

  private currentTick: number = 0;

  constructor(config: LoomFSRConfig) {
    const {
      lowResWidth, lowResHeight, highResWidth, highResHeight,
      jitterSamples, historyDepthThresholdFp, historyNormalThresholdFp,
      sharpenStrengthFp,
    } = config;
  ...
  getJitterIndex(): number { return this.jitterIndex; }
  getReactiveMask(): Uint8Array { return this.reactiveMask; }
  ...
  getCurrentJitter(out: Int32Array, outOffset: number = 0): boolean {
    if (outOffset < 0 || outOffset + 2 > out.length) return false;
  ...
  // out-of-range inputs.
  computeHistoryCoord(
    currentCoordXFp: number, currentCoordYFp: number,
    motionXFp: number, motionYFp: number,
    out: Int32Array, outOffset: number = 0,
  ): boolean {
    if (!Number.isInteger(currentCoordXFp) || !Number.isInteger(currentCoordYFp)) return false;
    if (!Number.isInteger(motionXFp) || !Number.isInteger(motionYFp)) return false;
    if (outOffset < 0 || outOffset + 2 > out.length) return false;
  ...
  // (fp in [-FP_ONE, +FP_ONE]).
  shouldRejectHistory(
    currentDepthFp: number, historyDepthFp: number,
    normalDotFp: number,
  ): boolean {
    if (!Number.isInteger(currentDepthFp) || !Number.isInteger(historyDepthFp)) return true;
    if (!Number.isInteger(normalDotFp)) return true;
    const dDelta = Math.abs(currentDepthFp - historyDepthFp);
    if (dDelta > this.historyDepthThresholdFp) return true;
    if (normalDotFp < this.historyNormalThresholdFp) return true;
    return false;
  }
  ...
  registerColorTexture(
    frontHandle: number, backHandle: number,
    format: number, usage: number, uniformAlignment: number,
  ): number {
  ...
  private registerChannel(
    channel: number, frontHandle: number, backHandle: number,
    format: number, usage: number, uniformAlignment: number,
  ): number {
    if (!Number.isInteger(channel) || channel < 0 || channel >= CHANNEL_COUNT) return FSR_REASON_BAD_CHANNEL;
    if (!Number.isInteger(frontHandle) || frontHandle === TEX_HANDLE_INVALID
      || frontHandle < 0 || frontHandle > U32_MAX) return FSR_REASON_BAD_CHANNEL;
    if (!Number.isInteger(backHandle) || backHandle === TEX_HANDLE_INVALID
      || backHandle < 0 || backHandle > U32_MAX) return FSR_REASON_BAD_CHANNEL;
    if (frontHandle === backHandle) return FSR_REASON_BAD_CHANNEL;
    ...
    if (channel === FSR_CHANNEL_DEPTH && format !== TEX_FORMAT_R32_FLOAT) return FSR_REASON_BAD_FORMAT;
    if (channel === FSR_CHANNEL_NORMAL
      && format !== TEX_FORMAT_RG16_SNORM && format !== TEX_FORMAT_RGBA16_FLOAT) {
      return FSR_REASON_BAD_FORMAT;
    }
    // Usage validation (gate 7).
    if (!Number.isInteger(usage) || usage < 0 || usage > 0xff) return FSR_REASON_BAD_USAGE;
    if ((usage & TEX_USAGE_TEXTURE_BINDING) === 0) return FSR_REASON_BAD_USAGE;
    if ((usage & TEX_USAGE_STORAGE_BINDING) === 0) return FSR_REASON_BAD_USAGE;
    // Alignment (gate 7).
  ...
  swapHistory(channel: number): boolean {
    if (!Number.isInteger(channel) || channel < 0 || channel >= CHANNEL_COUNT) return false;
    if (!this.textureRegistered[channel]) return false;
  ...
  swapAllHistories(): void {
    for (let c = 0; c < CHANNEL_COUNT; c++) {
      if (this.textureRegistered[c]) this.swapHistory(c);
    }
  }

  getFrontTexture(channel: number): number {
    if (!Number.isInteger(channel) || channel < 0 || channel >= CHANNEL_COUNT) return TEX_HANDLE_INVALID;
  ...
  setReactiveMask(x: number, y: number, mask: number): boolean {
    if (!this.requireLowResCoord(x, y)) return false;
    if (!Number.isInteger(mask) || mask < 0 || mask > 0xff) return false;
    this.reactiveMask[y * this.lowResWidth + x] = mask & 0xff;
    return true;
  }

  getReactiveMaskByte(x: number, y: number): number {
    if (!this.requireLowResCoord(x, y)) return 0;
  ...
  private requireLowResCoord(x: number, y: number): boolean {
    return Number.isInteger(x) && Number.isInteger(y)
      && x >= 0 && y >= 0
