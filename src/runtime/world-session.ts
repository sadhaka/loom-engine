...
import type { EpochResolvedEvent, Ruleset, ProposalMap, SerializedMutation } from './world-epoch.js';

// ---- the bundle (the persistable, verifiable world) ------------------------

export interface WorldBundle {
  worldId: string;
  snapshot: {
    // The chain seq this snapshot reflects (0 = genesis / before any event).
    eventIndex: number;
    // HMAC-SHA-256 of the canonical snapshot state.
    stateHash: string;
    state: WorldState;
  };
  // The HMAC-chained events AFTER the snapshot (empty when the snapshot is current).
...
function ensureEnt(state: WorldState, id: string): WorldEntity {
...
function applySerializedMutation(state: WorldState, m: SerializedMutation): void {
  var ent = ensureEnt(state, m.target);
  if (m.op === 'add_tag') {
    if (typeof m.tag === 'string') ent.tags = normalizeTags(ent.tags.concat([m.tag]));
...
export function replayEpochEvent(state: WorldState, event: EpochResolvedEvent): WorldState {
  var work = cloneState(state);
  var entries = (event && event.actions_processed) ? event.actions_processed : [];
  for (var i = 0; i < entries.length; i++) {
    var muts = (entries[i] as { mutations_applied?: SerializedMutation[] }).mutations_applied;
    if (Array.isArray(muts)) {
      for (var j = 0; j < muts.length; j++) applySerializedMutation(work, muts[j] as SerializedMutation);
    }
  }
...
export interface SuspendInput {
  ...
  worldId: string;
  // The state AS OF snapshotEventIndex (the caller takes this snapshot).
  snapshotState: WorldState;
  // The chain seq the snapshot reflects (0 = before any event).
  snapshotEventIndex: number;
  // The full event chain; records after snapshotEventIndex become the tail.
...
export function suspend(input: SuspendInput): WorldBundle {
  var records = input.chain.list();
  var tail: ChainedRecord<EpochResolvedEvent>[] = [];
  for (var i = 0; i < records.length; i++) {
    var rec = records[i];
    if (rec && rec.seq > input.snapshotEventIndex) tail.push(rec);
  }
...
export interface ResumeResult {
  worldId: string;
  state: WorldState;
  // EpochResolved events generated during catch-up - ready for the caller to
  // append to the chain + persist.
...
export function resume(input: ResumeInput): ResumeResult {
  ...
  if (tail.length > 0) {
    var res = EventChain.verifyRecords<EpochResolvedEvent>(input.key, tail, b.tailGenesis);
    if (!res.ok) {
  ...
  for (var i = 0; i < tail.length; i++) {
    var rec = tail[i];
    if (rec) work = replayEpochEvent(work, rec.payload);
  }
