...
export interface RewindResult<TState = unknown, TInput = unknown> {
  // Snapshot AT the requested tick (oldest snapshot at or before).
  snapshot: SnapshotEntry<TState>;
  // Inputs recorded AFTER snapshot.tick, in tick order.
  inputs: InputEntry<TInput>[];
}

export interface LagCompensationOptions<TState = unknown> {
  // Number of past snapshots + inputs retained. Default 60 (1 sec at
  // 60 fps).
  historySize?: number;
  // Optional serializer/cloner for state. If provided, called on
  // every recordState() so the buffer holds a deep copy independent
  // of consumer mutation. Recommended: structuredClone or a custom
  // immutable.
  stateSerialize?: (s: TState) => TState;
}

export class LagCompensation<TState = unknown, TInput = unknown> {
  private snapshots: SnapshotEntry<TState>[] = [];
  private inputs:    InputEntry<TInput>[]    = [];
  private historySize: number;
  private serialize: ((s: TState) => TState) | null;

  private constructor(opts: LagCompensationOptions<TState>) {
  ...
  static create<TState = unknown, TInput = unknown>(
    opts: LagCompensationOptions<TState> = {}): LagCompensation<TState, TInput> {
    return new LagCompensation<TState, TInput>(opts);
  }
  ...
  recordState(tick: number, state: TState): void {
    ...
    var stored: TState = this.serialize ? this.serialize(state) : state;
  ...
  rewind(tick: number): RewindResult<TState, TInput> | null {
    if (typeof tick !== 'number' || !isFinite(tick)) return null;
    var i = this.findSnapshotIndex(tick);
    if (i < 0) return null;  // no snapshot at-or-before
    var snap = this.snapshots[i]!;
    var inputsAfter: InputEntry<TInput>[] = [];
    for (var j = 0; j < this.inputs.length; j++) {
      var inp = this.inputs[j]!;
      if (inp.tick > snap.tick) inputsAfter.push(inp);
    }
  ...
  resync(tick: number, authoritativeState: TState): InputEntry<TInput>[] {
    ...
    var stored: TState = this.serialize ? this.serialize(authoritativeState) : authoritativeState;
  ...
  snapshotCount(): number { return this.snapshots.length; }
  inputCount(): number    { return this.inputs.length; }
  oldestSnapshotTick(): number | null {
    return this.snapshots.length > 0 ? (this.snapshots[0] as SnapshotEntry<TState>).tick : null;
  }
  newestSnapshotTick(): number | null {
    return this.snapshots.length > 0
      ? (this.snapshots[this.snapshots.length - 1] as SnapshotEntry<TState>).tick : null;
  }
  newestInputTick(): number | null {
    return this.inputs.length > 0
      ? (this.inputs[this.inputs.length - 1] as InputEntry<TInput>).tick : null;
  }
  ...
  setHistorySize(n: number): void {
    if (typeof n === 'number' && n > 0) {
      ...
      if (newest !== null) this.evictOldSnapshots(newest);
      var newestI = this.newestInputTick();
      if (newestI !== null) this.evictOldInputs(newestI);
    }
  }
  ...
  private findSnapshotIndex(tick: number): number {
    var lo = 0, hi = this.snapshots.length - 1, ans = -1;
    while (lo <= hi) {
      var mid = (lo + hi) >>> 1;
      if ((this.snapshots[mid] as SnapshotEntry<TState>).tick <= tick) {
        ans = mid;
        lo = mid + 1;
      } else {
        hi = mid - 1;
  ...
  private findInputInsertIndex(tick: number): number {
    var lo = 0, hi = this.inputs.length;
    while (lo < hi) {
      var mid = (lo + hi) >>> 1;
      if ((this.inputs[mid] as InputEntry<TInput>).tick <= tick) lo = mid + 1;
      else hi = mid;
    }
    return lo;
  }

  private evictOldSnapshots(currentTick: number): void {
    var minTick = currentTick - this.historySize;
    while (this.snapshots.length > 0
        && (this.snapshots[0] as SnapshotEntry<TState>).tick < minTick) {
      this.snapshots.shift();
    }
  }

  private evictOldInputs(currentTick: number): void {
    var minTick = currentTick - this.historySize;
    while (this.inputs.length > 0
        && (this.inputs[0] as InputEntry<TInput>).tick < minTick) {
      this.inputs.shift();
    }
  }
}
