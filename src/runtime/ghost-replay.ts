...
export interface Recording {
  frames: GhostFrame[];
  totalMs: number;
  // Optional human-readable label (e.g. "best_lap", "last_run").
  label?: string;
}
...
export interface GhostSnapshot {
  ...
  // Optional payload from the active frame (consumer-defined).
  data?: Record<string, unknown>;
  // 0..1 over the recording.
  progress: number;
  elapsedMs: number;
  isPlaying: boolean;
  isPaused: boolean;
  speed: number;
  // Render hint: alpha for fade-in / fade-out at boundaries; 1.0
  // unless the consumer configured fadeIn/fadeOut.
  alpha: number;
}

export interface StartRecordingOptions {
  // Minimum ms between recorded snapshots. Default 50 (~20Hz).
  // Snapshots arriving faster are dropped.
  sampleRateMs?: number;
  // Maximum frames to keep. Default 1200 (1 minute at 50ms cadence).
  // When exceeded, oldest frames drop.
  maxFrames?: number;
  // Optional label baked into the resulting Recording.
  label?: string;
}

export interface PlayGhostOptions {
  // Stable id for this playback. Default = label of the recording
  // or 'ghost'. Replaces any existing playback with same id.
  id?: string;
  // Speed multiplier (1 = normal). Default 1.
  speed?: number;
  // Loop the playback? Default false.
  loop?: boolean;
  // ms fade-in at the start of each play. Default 0 (instant).
  fadeInMs?: number;
  // ms fade-out at the end of each play. Default 0.
  fadeOutMs?: number;
  // Fired when a non-looping play reaches the end.
  onFinish?: () => void;
}

export interface GhostReplayOptions {
  // Reserved for future global hooks.
}
...
export class GhostReplay {
  private recording: InternalRecording | null = null;
  private ghosts: Map<string, ActiveGhost> = new Map();
  private disposed: boolean = false;

  private constructor(_opts: GhostReplayOptions) { /* reserved */ }
  ...
  recordSnapshot(s: { x: number; y: number; rotation?: number;
    animationId?: string; data?: Record<string, unknown> }): boolean {
    if (this.disposed || !this.recording) return false;
    ...
    if (s.rotation !== undefined && isFinite(s.rotation)) frame.rotation = s.rotation;
    if (typeof s.animationId === 'string') frame.animationId = s.animationId;
    if (s.data !== undefined) frame.data = s.data;
    rec.frames.push(frame);
    rec.lastSampleAtMs = nextAt;
    if (rec.frames.length > rec.maxFrames) {
      rec.frames.shift();
      // Re-anchor times so atMs=0 stays the first frame.
      var offset = (rec.frames[0] as GhostFrame).atMs;
      if (offset !== 0) {
        for (var i = 0; i < rec.frames.length; i++) {
          (rec.frames[i] as GhostFrame).atMs -= offset;
        }
  ...
  stopRecording(): Recording | null {
    ...
    var totalMs = rec.frames.length > 0
      ? (rec.frames[rec.frames.length - 1] as GhostFrame).atMs : 0;
    var out: Recording = {
  ...
  setSpeed(id: string, multiplier: number): boolean {
    ...
    var g = this.ghosts.get(id);
    if (!g) return false;
    if (!isFinite(multiplier) || multiplier <= 0) return false;
    g.speed = multiplier;
    return true;
  }
  ...
  list(): GhostSnapshot[] {
    var out: GhostSnapshot[] = [];
    var keys = this.ghosts.keys();
    var k = keys.next();
    while (!k.done) {
      var g = this.ghosts.get(k.value);
      if (g) out.push(this.snapshotForGhost(g));
      k = keys.next();
    }
    return out;
  }
  ...
  forEach(cb: (s: GhostSnapshot) => void): void {
    ...
    for (var i = 0; i < ghosts.length; i++) {
      try { cb(ghosts[i] as GhostSnapshot); } catch { /* ignore */ }
    }
  }
  ...
  tick(dtMs: number): void {
    ...
    for (var i = 0; i < ids.length; i++) {
      var g = this.ghosts.get(ids[i] as string);
      if (!g || g.paused) continue;
    ...
    for (var j = 0; j < finished.length; j++) {
      var fg = finished[j] as ActiveGhost;
      var cb = fg.onFinish;
      fg.onFinish = null;
      if (cb) {
        try { cb(); } catch { /* ignore */ }
  ...
  private snapshotForGhost(g: ActiveGhost): GhostSnapshot {
    var frames = g.recording.frames;
    var snapshot: GhostSnapshot = {
      id: g.id,
      x: 0,
      y: 0,
      rotation: 0,
    ...
    if (frames.length === 0) return snapshot;
    if (g.recording.totalMs > 0) {
      snapshot.progress = Math.max(0, Math.min(1, g.elapsed / g.recording.totalMs));
    }
    // Interpolate position between surrounding frames.
    var first = frames[0] as GhostFrame;
    var last = frames[frames.length - 1] as GhostFrame;
    if (g.elapsed <= first.atMs) {
      snapshot.x = first.x;
      snapshot.y = first.y;
      snapshot.rotation = first.rotation ?? 0;
      if (first.animationId !== undefined) snapshot.animationId = first.animationId;
      if (first.data !== undefined) snapshot.data = first.data;
    } else if (g.elapsed >= last.atMs) {
      snapshot.x = last.x;
      snapshot.y = last.y;
      snapshot.rotation = last.rotation ?? 0;
      if (last.animationId !== undefined) snapshot.animationId = last.animationId;
      if (last.data !== undefined) snapshot.data = last.data;
    } else {
      var fromIdx = 0;
      for (var i = 0; i < frames.length - 1; i++) {
        var fi = frames[i] as GhostFrame;
        if (fi.atMs <= g.elapsed) fromIdx = i;
        else break;
      }
      var from = frames[fromIdx] as GhostFrame;
      var to = frames[Math.min(frames.length - 1, fromIdx + 1)] as GhostFrame;
      ...
      if (from.animationId !== undefined) snapshot.animationId = from.animationId;
      if (from.data !== undefined) snapshot.data = from.data;
    }
