...
function requirePositiveFinite(value: number, name: string): void {
  if (!Number.isFinite(value) || value <= 0) {
    throw new RangeError(
      'SpatialGrid: ' + name + ' must be a positive finite number, got ' + value,
    );
  }
}
...
function requirePositiveInt(value: number, name: string): void {
  if (!Number.isInteger(value) || value <= 0) {
    throw new RangeError(
      'SpatialGrid: ' + name + ' must be a positive integer, got ' + value,
    );
  }
}
...
export class SpatialGrid {
  ...
  constructor(
    ...
    if (numCells > MAX_CELL_COUNT) {
      throw new RangeError(
        'SpatialGrid: numCells ' + numCells + ' exceeds the cap ' + MAX_CELL_COUNT,
      );
    }
    if (maxEntities > MAX_ENTITY_COUNT) {
      throw new RangeError(
        'SpatialGrid: maxEntities ' + maxEntities
  ...
  cellIndexOf(x: number, y: number): number {
    ...
    if (col < 0) col = 0;
    else if (col >= this.gridWidth) col = this.gridWidth - 1;
    if (row < 0) row = 0;
    else if (row >= this.gridHeight) row = this.gridHeight - 1;
  ...
  query(cellIdx: number, out: Int32Array): number {
    if (!Number.isInteger(cellIdx) || cellIdx < 0 || cellIdx >= this.numCells) {
      throw new RangeError(
        'SpatialGrid.query: cellIdx ' + cellIdx
        + ' out of range [0, ' + this.numCells + ')',
      );
    }
    ...
    while (curr !== EMPTY && count < cap) {
      ...
      if (++guard > this.maxEntities) {
        throw new Error(
          'SpatialGrid.query: traversal of cell ' + cellIdx
  ...
  firstInCell(cellIdx: number): number {
    if (!Number.isInteger(cellIdx) || cellIdx < 0 || cellIdx >= this.numCells) {
      throw new RangeError(
        'SpatialGrid.firstInCell: cellIdx ' + cellIdx
        + ' out of range [0, ' + this.numCells + ')',
      );
    }
  ...
  nextOf(entityId: number): number {
    if (!Number.isInteger(entityId) || entityId < 0 || entityId >= this.maxEntities) {
      throw new RangeError(
        'SpatialGrid.nextOf: entityId ' + entityId
        + ' out of range [0, ' + this.maxEntities + ')',
      );
    }
