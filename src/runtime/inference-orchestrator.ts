...
export function makeRequestHandle(slot: number, lane: number, generation: number): RequestHandle {
...
export function requestSlot(handle: RequestHandle): number {
  return handle & HANDLE_SLOT_MASK;
}

export function requestLane(handle: RequestHandle): number {
  return (handle >>> HANDLE_LANE_SHIFT) & HANDLE_LANE_MASK;
}

export function requestGeneration(handle: RequestHandle): number {
  return (handle >>> HANDLE_GENERATION_SHIFT) & HANDLE_GENERATION_MASK;
}

export interface InferenceOrchestratorConfig {
  // Upper bound on npcId space. Submit rejects npcId >= this.
  maxNpc: number;
  // Upper bound on actionType - per-actionType allowed-result-mask
  // table is sized to this.
  maxActionTypes: number;
  // Per-lane request slot count. Each lane independently caps its
  // pending requests at this; submit past this drops with
  // REASON_RATE_LIMITED.
  perLaneCapacity: number;
  // Max NPCs per drainBatch call. Caps drainBatch's out array
  // requirement; the dispatcher chooses a batch up to this.
  maxBatchSize: number;
  // Drop-event ring capacity. Past this, drop events are dropped (a
  // dropOverflowCount counter tracks).
  dropEventCapacity: number;
  // Per-lane budget config (LOCAL_SLM, CLOUD).
  localSlmMaxBudget: number;
  localSlmRefillPerTick: number;
  cloudMaxBudget: number;
  cloudRefillPerTick: number;
  // Per-lane request rate cap (per tick).
  localSlmMaxRequestsPerTick: number;
  cloudMaxRequestsPerTick: number;
  // Critical-priority ceiling per lane per tick (gate 6).
  localSlmCriticalCeiling: number;
  cloudCriticalCeiling: number;
  // Default TTL for new requests (caller may override per-request).
...
export class InferenceOrchestrator {
  ...
  private readonly laneMaxBudget: Uint32Array;             // [LANE_COUNT]
  private readonly laneRefillPerTick: Uint32Array;
  private readonly laneMaxRequestsPerTick: Uint32Array;
  private readonly laneCriticalCeiling: Uint32Array;

  // Per-lane runtime state.
  private readonly laneBudget: Uint32Array;                // [LANE_COUNT]
  private readonly laneRequestsThisTick: Uint32Array;
  private readonly laneCriticalSpentThisTick: Uint32Array;

  // Per-lane request slot tables. Indexed [lane * perLaneCapacity + slot].
  private readonly slotState: Uint8Array;
  private readonly slotNpcId: Int32Array;
  private readonly slotPriority: Uint8Array;
  private readonly slotEstimatedTokens: Uint32Array;
  private readonly slotSubmittedAtTick: Uint32Array;
  private readonly slotExpiresAtTick: Uint32Array;
  private readonly slotGeneration: Uint8Array;

  // Per-lane pending count.
  private readonly lanePendingCount: Uint32Array;
  // Per-lane next-free probe hint - O(1) in the common path.
  private readonly laneFreeHint: Uint32Array;
  ...
  private readonly actionAllowedMask: Uint32Array;
  // Whether the actionType has been registered - distinguishes "no
  // bits allowed" (allowed mask 0) from "type unknown" (rejected).
  private readonly actionRegistered: Uint8Array;

  // Drop-event ring (gate 1, 2 - the consumer drains drops to track
  // metrics; the orchestrator never retries on its own).
  private readonly dropEventRing: Int32Array;
  private dropEventHead: number = 0;
  private dropEventTail: number = 0;
  private dropOverflowCount: number = 0;

  private localSlmEnabled: boolean = true;
  private currentTick: number = 0;
  private completedTotal: number = 0;
  private cancelledTotal: number = 0;
  private expiredTotal: number = 0;
  private rejectedResultsTotal: number = 0;

  constructor(config: InferenceOrchestratorConfig) {
    const {
      maxNpc, maxActionTypes, perLaneCapacity, maxBatchSize, dropEventCapacity,
      localSlmMaxBudget, localSlmRefillPerTick, cloudMaxBudget, cloudRefillPerTick,
      localSlmMaxRequestsPerTick, cloudMaxRequestsPerTick,
      localSlmCriticalCeiling, cloudCriticalCeiling, defaultTtlTicks,
    } = config;
  ...
  getCompletedTotal(): number { return this.completedTotal; }
  getCancelledTotal(): number { return this.cancelledTotal; }
  getExpiredTotal(): number { return this.expiredTotal; }
  getRejectedResultsTotal(): number { return this.rejectedResultsTotal; }
  getDropEventCount(): number { return this.dropEventTail - this.dropEventHead; }
  getDropOverflowCount(): number { return this.dropOverflowCount; }
  isLocalSlmEnabled(): boolean { return this.localSlmEnabled; }
  ...
  registerActionType(actionType: number, allowedResultMask: number): boolean {
    ...
    if (!Number.isInteger(allowedResultMask) || allowedResultMask < 0 || allowedResultMask > U32_MAX) return false;
  ...
  private submitToLane(
    ...
    if (priority === PRIORITY_CRITICAL) {
      this.laneCriticalSpentThisTick[lane] = (
        (this.laneCriticalSpentThisTick[lane] ?? 0) + estimatedTokens
      ) >>> 0;
    }
  ...
  // the lane is full.
  private allocSlot(lane: number): number {
  ...
  drainBatch(lane: number, count: number, out: Int32Array): number {
    if (!this.requireLane(lane)) return 0;
    if (!Number.isInteger(count) || count < 1 || count > this.maxBatchSize) return 0;
    if (out.length < count) return 0;
    const cap = this.perLaneCapacity;
    let drained = 0;
    ...
    for (let pri = PRIORITY_CRITICAL; pri >= PRIORITY_LOW; pri--) {
      for (let slot = 0; slot < cap && drained < count; slot++) {
        const idx = lane * cap + slot;
        if (this.slotState[idx] !== REQUEST_STATE_PENDING) continue;
        if (this.slotPriority[idx] !== pri) continue;
  ...
  drainBatchWithHandles(lane: number, count: number, outNpcs: Int32Array, outHandles: Int32Array): number {
    if (!this.requireLane(lane)) return 0;
    if (!Number.isInteger(count) || count < 1 || count > this.maxBatchSize) return 0;
    if (outNpcs.length < count || outHandles.length < count) return 0;
    const cap = this.perLaneCapacity;
    let drained = 0;
    for (let pri = PRIORITY_CRITICAL; pri >= PRIORITY_LOW; pri--) {
      for (let slot = 0; slot < cap && drained < count; slot++) {
  ...
  completeRequest(handle: RequestHandle, actionType: number, resultActionMask: number): number {
    ...
    if (!this.requireLane(lane)) return REASON_BAD_LANE;
    if (!Number.isInteger(slot) || slot < 0 || slot >= this.perLaneCapacity) return REASON_STALE_HANDLE;
    const idx = lane * this.perLaneCapacity + slot;
  ...
  private failSlot(idx: number, lane: number): void {
  ...
  cancelRequest(handle: RequestHandle): boolean {
    ...
    if (!Number.isInteger(slot) || slot < 0 || slot >= this.perLaneCapacity) return false;
    const idx = lane * this.perLaneCapacity + slot;
    if ((this.slotGeneration[idx] ?? 0) !== gen) return false;
    const state = this.slotState[idx] ?? 0;
    if (state !== REQUEST_STATE_PENDING && state !== REQUEST_STATE_INFLIGHT) return false;
    this.slotState[idx] = REQUEST_STATE_NONE;
    this.slotNpcId[idx] = -1;
    this.lanePendingCount[lane] = ((this.lanePendingCount[lane] ?? 0) - 1) >>> 0;
    this.cancelledTotal++;
  ...
  tick(t: number): void {
    ...
    for (let lane = 0; lane < LANE_COUNT; lane++) {
      for (let slot = 0; slot < cap; slot++) {
        const idx = lane * cap + slot;
        const state = this.slotState[idx] ?? 0;
        if (state !== REQUEST_STATE_PENDING && state !== REQUEST_STATE_INFLIGHT) continue;
  ...
  private pushDrop(npcId: number, lane: number, priority: number, reason: number): void {
  ...
  consumeDropEvent(out: Int32Array, outOffset: number = 0): boolean {
    if (this.dropEventHead >= this.dropEventTail) return false;
    if (outOffset < 0 || outOffset + DROP_EVENT_STRIDE > out.length) return false;
  ...
  private requirePriority(p: number): boolean {
    return Number.isInteger(p) && p >= 0 && p < PRIORITY_COUNT;
  }

  private requireNpcId(id: number): boolean {
  ...
  getSlotState(handle: RequestHandle): number {
    ...
    const lane = requestLane(handle);
    const gen = requestGeneration(handle);
    if (!this.requireLane(lane)) return REQUEST_STATE_NONE;
    if (!Number.isInteger(slot) || slot < 0 || slot >= this.perLaneCapacity) return REQUEST_STATE_NONE;
    const idx = lane * this.perLaneCapacity + slot;
    if ((this.slotGeneration[idx] ?? 0) !== gen) return REQUEST_STATE_NONE;
