...
interface BusEntry {
  node: GainNode;
  baseGain: number;       // user-set gain BEFORE budget gating
  muted: boolean;
  priority: BusPriority;
}

const DEFAULT_BUSES: Array<{ name: string; opts: BusOptions }> = [
...
export class AudioBus {
  ...
  private buses: Map<string, BusEntry> = new Map();
  private masterGain: number = 1;
  private currentBudget: number = 1.0;
  ...
  async unlock(): Promise<void> {
    ...
    try {
      ...
      this.suspended = (this.ctx.state as string) !== 'running';
    } catch {
  ...
  isUnlocked(): boolean {
    return !this.suspended && (this.ctx.state as string) === 'running';
  }
  ...
  addBus(name: string, opts: BusOptions = {}): void {
    if (this.buses.has(name)) return;   // idempotent
  ...
  private applyBudgetToBus(name: string): void {
    const entry = this.buses.get(name);
    if (!entry) return;
    let effective = entry.baseGain;
    if (entry.muted) effective = 0;
    if (this.currentBudget < AUDIO_BUDGET_ESSENTIAL_FLOOR) {
      effective = 0;
    } else if (
      entry.priority === 'ambient' &&
  ...
  playOneShot(busName: string, buffer: AudioBuffer, options: { rate?: number; gain?: number } = {}): AudioBufferSourceNode | null {
  ...
  playTone(busName: string, freq: number, durationMs: number, options: { gain?: number; type?: OscillatorType } = {}): void {
    const entry = this.buses.get(busName);
    if (!entry) return;
