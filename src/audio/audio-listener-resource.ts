...
// Shape of the resource registered under RESOURCE_AUDIO_LISTENER.
// Mutating .pose directly is fine; SpatialAudioSystem owns the writer
// path. Consumers who need to snapshot the pose should clone .pose
// because the resource is a singleton.
export interface AudioListenerResource {
  pose: AudioListenerPose;
  // Frame counter (TimeResource.frame) of the most recent
  // SpatialAudioSystem update. -1 means "never updated".
...
export const DEFAULT_LISTENER_FORWARD: { x: number; y: number; z: number } = {
  x: 0, y: 0, z: -1,
};

export const DEFAULT_LISTENER_UP: { x: number; y: number; z: number } = {
  x: 0, y: 1, z: 0,
};
