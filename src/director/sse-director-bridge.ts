...
export interface SSEDirectorBridgeOptions {
  ...
  // Optional backoff overrides (0.20.0). Defaults: BASE=500, MAX=30000.
  baseBackoffMs?: number;
  maxBackoffMs?: number;
  // Optional setTimeout / clearTimeout shims for deterministic tests.
  // Production code uses globalThis.setTimeout.
  setTimeoutFn?: (fn: () => void, ms: number) => unknown;
  clearTimeoutFn?: (handle: unknown) => void;
  // Optional jitter source (0..1). Defaults to Math.random. Tests
  // pin to a fixed value to assert exact delays.
  randomFn?: () => number;
  // Optional wall clock for stats timestamps. Defaults to Date.now.
  nowFn?: () => number;
...
export class SSEDirectorBridge implements IDirectorBridge {
  ...
  private reorderTimeoutHandle: unknown = null;
  ...
  private reconnectTimeoutHandle: unknown = null;
  ...
  constructor(opts: SSEDirectorBridgeOptions) {
    ...
    this.clearTimeoutFn = opts.clearTimeoutFn || function (h) { clearTimeout(h as ReturnType<typeof setTimeout>); };
    ...
    if (opts.statusEventTarget === null) {
      ...
      try {
        defaultTarget = (typeof globalThis !== 'undefined' && (globalThis as { window?: Window }).window)
    ...
    if (opts.eventSourceFactory) {
      ...
      this.eventSourceFactory = function (u: string) { return new ESCtor(u, { withCredentials: true }); };
    }
  }
  ...
  private openConnection(): void {
    ...
    for (var i = 0; i < knownTypes.length; i++) {
      var t = knownTypes[i];
      if (!t) continue;
      es.addEventListener(t, function (e: Event) {
        self.handleRaw(e as MessageEvent);
      });
    }
  }
  ...
  private cancelReconnect(): void {
    if (this.reconnectTimeoutHandle !== null) {
      try { this.clearTimeoutFn(this.reconnectTimeoutHandle); } catch { /* ignore */ }
  ...
  private drainReorderBuffer(): void {
    ...
    if (this.reorderBuffer.size === 0 && this.reorderTimeoutHandle !== null) {
      try { this.clearTimeoutFn(this.reorderTimeoutHandle); } catch { /* ignore */ }
  ...
  private transitionTo(next: DirectorBridgeStatus): void {
