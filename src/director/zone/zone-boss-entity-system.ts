...
import {
  ...
  applyTick,
} from './zone-boss-entity.js';

export class ZoneBossEntitySystem implements System {
  readonly name: string = 'zone-boss-entity';
  ...
  update(world: World, _dt: number): void {
    ...
    const entities = world.resources.get<ZoneBossEntityResource>(
      RESOURCE_ZONE_BOSS_ENTITY,
    );
    if (!log || !entities) return;
...
function applyEventToEntity(
  ...
  switch (ev.type) {
    ...
    case 'zone.boss.tick': {
      const e = ev as ZoneEventEnvelope<'zone.boss.tick'>;
      const current = entities.byZone.get(zoneId);
      if (!current) return;
      if (current.boss_id !== e.data.boss_id) return;
