...
// Frame layout, all little-endian via SnapshotWriter:
//   magic        u32   DELTA_WIRE_MAGIC
//   version      u16   DELTA_WIRE_VERSION
//   columnCount  u16   1..32 - the record width
//   tick         u32   the tick `curr` represents
//   baselineTick u32   the tick `prev` represents (desync guard)
//   mask         u32   bit i set <=> column i changed
//   payloadLen   u32   popcount(mask) * 4 - cross-checks the mask
//   values       payloadLen bytes - the changed columns, in order
// A 24-byte header, then 4 bytes per changed column.

// What decode() recovers from a frame's header beyond the rebuilt
// record - the two ticks the caller needs to confirm it decoded
// against the right baseline.
export interface DeltaFrameInfo {
  // The tick the decoded record represents.
  tick: number;
  // The tick the frame was encoded against. A caller holding a
  // different baseline tick must NOT apply this frame - that is the
  // protocol-desync guard the Codex audit calls for.
  baselineTick: number;
}
...
function requireU32(value: number, name: string): void {
  if (!Number.isInteger(value) || value < 0 || value > 0xffffffff) {
    throw new RangeError(
      'DeltaCompressor: ' + name + ' must be a u32 integer in [0, 4294967295], got ' + value,
    );
  }
}
...
export class DeltaCompressor {
  ...
  static encode(
    ...
    if (columnCount < 1 || columnCount > DELTA_MAX_COLUMNS) {
      throw new RangeError(
        'DeltaCompressor.encode: record width ' + columnCount
        + ' out of [1, ' + DELTA_MAX_COLUMNS + ']',
      );
    }
    if (curr.length !== columnCount) {
      throw new RangeError(
        'DeltaCompressor.encode: curr width ' + curr.length
  ...
  static decode(
    ...
    if (magic !== DELTA_WIRE_MAGIC) {
      throw new Error(
        'DeltaCompressor.decode: bad magic 0x' + (magic >>> 0).toString(16),
      );
    }
    const version = reader.readU16();
    if (version !== DELTA_WIRE_VERSION) {
      throw new Error(
        'DeltaCompressor.decode: version ' + version
    ...
    if (columnCount < 1 || columnCount > DELTA_MAX_COLUMNS) {
      throw new RangeError(
        'DeltaCompressor.decode: record width ' + columnCount
        + ' out of [1, ' + DELTA_MAX_COLUMNS + ']',
      );
    }
    if (prev.length !== columnCount) {
      throw new RangeError(
        'DeltaCompressor.decode: prev width ' + prev.length
        + ' != frame width ' + columnCount,
      );
    }
    if (out.length !== columnCount) {
      throw new RangeError(
        'DeltaCompressor.decode: out width ' + out.length
        + ' != frame width ' + columnCount,
      );
    }
    ...
    if (columnCount < 32) {
      ...
      if ((mask & ~validMask) !== 0) {
        throw new Error(
          'DeltaCompressor.decode: mask 0x' + (mask >>> 0).toString(16)
    ...
    let changed = 0;
    for (let i = 0; i < columnCount; i++) {
      if ((mask & (1 << i)) !== 0) changed++;
    }
    const payloadLength = reader.readU32();
    if (payloadLength !== changed * 4) {
      throw new Error(
        'DeltaCompressor.decode: payloadLength ' + payloadLength
...
export function deltaFrameToBase64(bytes: Uint8Array): string {
  let binary = '';
...
export function deltaFrameFromBase64(text: string): Uint8Array {
