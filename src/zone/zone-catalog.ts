...
export interface ZoneTilePalette {
  // Tile fill color (the body of the diamond).
  fill: Readonly<ColorRGBA>;
  // Tile outline color.
  stroke: Readonly<ColorRGBA>;
  // Highlight color on the upper edge for fake 3D feel.
  highlight: Readonly<ColorRGBA>;
}

export interface ZoneCatalogEntry {
  id: ZoneId;
  // Display name for HUD / dialog headers.
  name: string;
  // The dominant knot whose palette tints this zone (per
  // LOOM-CLASS-SYSTEM-SPEC Section 4). Plaza = neutral; pure knot
  // zones use their own knot color; hybrid hamlets blend.
  knot: 'str' | 'dex' | 'int' | 'mixed' | 'center';
  // Tile palette for the floor. Engine renders tiles from this
  // (the demo bakes a procedural canvas per zone).
  palette: ZoneTilePalette;
  // Ambient music bus level multiplier. Plaza calm, knot zones
  // tense. Director-bridge can override per-encounter.
  musicLevel: number;
  // List of zone ids that have portals from here. Demo wires the
  // first one as the active portal NPC's payload.
  exits: ReadonlyArray<ZoneId>;
}

export const ZONE_CATALOG: Record<ZoneId, ZoneCatalogEntry> = {
  lastlight_plaza: {
    id: 'lastlight_plaza',
