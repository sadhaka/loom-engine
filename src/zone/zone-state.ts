...
export type ZoneId =
  ...
  | 'saltsprig'            // Dexknot
  | 'the_archive'          // Intknot
  | 'hammerwash'           // Str/Dex hybrid
  | 'crystwell'            // Dex/Int hybrid
  | 'forge_archive'        // Str/Int hybrid
...
export interface ZoneStateResource {
  ...
  activeZoneId: ZoneId;
  // Pending transition - if non-null, ZoneSystem is in the middle
  // of swapping zones. Renderer can fade the screen during this.
  transition: {
    fromZoneId: ZoneId;
    toZoneId: ZoneId;
    kind: TransitionKind;
...
export function createZoneState(initial: ZoneId = 'lastlight_plaza'): ZoneStateResource {
  return {
...
export function beginTransition(
  state: ZoneStateResource,
  toZoneId: ZoneId,
  kind: TransitionKind,
  durationMs: number,
  nowMs: number,
): boolean {
  if (state.activeZoneId === toZoneId) return false;
  state.transition = {
...
export function tickTransition(state: ZoneStateResource, nowMs: number): number {
...
export function isTransitioning(state: ZoneStateResource): boolean {
