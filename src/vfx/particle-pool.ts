...
// Spawn parameters. Caller fills in what they need; defaults cover
// the rest. life is in seconds.
export interface ParticleSpawn {
  ...
  life: number;
  size?: number;          // pixels
  endSize?: number;       // pixels at end of life; defaults to size
  color: Readonly<ColorRGBA>;
  endColor?: Readonly<ColorRGBA>;  // defaults to color with a=0
  additive?: boolean;
}
...
export class ParticlePool implements ISnapshotable {
  ...
  maxLife: Float32Array;     // total seconds at spawn (for fade interpolation)
  size: Float32Array;
  ...
  constructor(initialCapacity: number = 256, maxParticles: number = 4096) {
