...
import { LOOM_ENGINE_VERSION, Engine, POOL_TRANSFORM, POOL_SPRITE, POOL_ANIMATION, POOL_HEALTH, POOL_PURSUE, POOL_PROJECTILE, ISO_TILE_WIDTH, ISO_TILE_HEIGHT, ProjectileSystem, ProjectileRenderSystem, RangedAttackSystem, spawnMob, AnimationSystem, SpriteRenderSystem, ParticleSimulationSystem, ParticleEmitterSystem, ParticleRenderSystem, InputSystem, PursueSystem, AttackSystem, DamageSystem, RESOURCE_DEVICE, RESOURCE_CAMERA, RESOURCE_TIME, RESOURCE_DEATH_LOG, POOL_EMITTER, SYSTEM_PHASE_INPUT, SYSTEM_PHASE_LOGIC, SYSTEM_PHASE_PHYSICS, SYSTEM_PHASE_ANIMATION, SYSTEM_PHASE_RENDER, loadSpriteSheet, hexToRgba, } from '../dist/index.js';
...
class WaveSpawnerSystem {
    ...
    name = 'wave-spawner';
    waveCount = 0;
    constructor(player, enemyAtlas, targetEnemyCount, radius) {
        this.player = player;
        this.enemyAtlas = enemyAtlas;
        this.targetEnemyCount = targetEnemyCount;
        this.radius = radius;
    }
    getWaveCount() {
    ...
    update(world, _dt) {
        ...
        for (let i = 1; i < hwm; i++) {
            if (i === playerIdx)
                continue;
            if ((pursuit.flags[i] ?? 0) & 1)
                live++;
        }
        if (live >= this.targetEnemyCount)
            return;
        // Wave composition: 2 warriors + 1 archer + 1 caster cycling.
        // Position around the player at edge-of-floor.
        const types = ['skel_warrior', 'skel_archer', 'skel_warrior', 'skel_caster'];
...
class DeathFxSystem {
    ...
    lastSeenKills = 0;
    update(world, _dt) {
        const log = world.resources.get(RESOURCE_DEATH_LOG);
        if (!log)
            return;
        const kills = log.totalKills;
        if (kills === this.lastSeenKills)
            return;
...
(async function boot() {
    ...
    function tick(now) {
        ...
        const deathLog = engine.world.resources.require(RESOURCE_DEATH_LOG);
        ...
        // Count alive enemies.
        const pursuit = engine.world.requirePool(POOL_PURSUE);
        const playerIdx = knight & 0x00ffffff;
        let alive = 0;
        const hwm = pursuit.getHighWaterMark();
        for (let i = 1; i < hwm; i++) {
            if (i === playerIdx)
        ...
        const projectiles = engine.world.requirePool(POOL_PROJECTILE);
