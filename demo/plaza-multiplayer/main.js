...
import { Engine, POOL_TRANSFORM, POOL_SPRITE, ISO_TILE_WIDTH, ISO_TILE_HEIGHT, InputSystem, SpriteRenderSystem, RESOURCE_DEVICE, RESOURCE_CAMERA, RESOURCE_INPUT, SYSTEM_PHASE_INPUT, SYSTEM_PHASE_LOGIC, SYSTEM_PHASE_RENDER, 
// Multiplayer
MockMultiplayerBridge, PeerPool, PeerPresenceSystem, PeerRenderSystem, PeerSpritePool, POOL_PEER_SPRITE, RESOURCE_MULTIPLAYER_BRIDGE, RESOURCE_PEER_POOL, } from '@sadhaka/loom-engine';
...
class WalkBroadcastSystem {
    ...
    update(world, dt) {
        ...
        if (bridge) {
            bridge.broadcastPosition(transforms.x[i] ?? 0, transforms.y[i] ?? 0, ZONE, Date.now());
        }
    }
}
