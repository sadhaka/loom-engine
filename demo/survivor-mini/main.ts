...
function paintSprite(w: number, h: number, body: string, eye: string): HTMLCanvasElement {
  ...
  ctx.fillStyle = eye; ctx.fillRect(w * 0.25, h * 0.25, 2, 2); ctx.fillRect(w * 0.65, h * 0.25, 2, 2);
  return c;
}
...
class AutoTargetSystem implements System {
  ...
  update(world: World, _dt: number): void {
    ...
    for (let i = 1; i < hwm; i++) {
      ...
      if (((pursuit.flags[i] ?? 0) & 1) === 0) continue;
...
class EdgeSpawnerSystem implements System {
  readonly name: string = 'edge-spawner';
  private nextSpawnAt: number = 0;
  constructor(private player: EntityId, private atlas: number, private cooldownMs: number, private radius: number) {}
...
(async function boot(): Promise<void> {
  ...
  const ranged = engine.world.requirePool<RangedAttackPool>(POOL_RANGED);
  ...
  function tick(now: number): void {
    ...
    const log = engine.world.resources.require<DeathLog>(RESOURCE_DEATH_LOG);
    stats.textContent =
      'fps   ' + fps + '\n' +
