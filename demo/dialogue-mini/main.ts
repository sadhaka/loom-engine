...
const speakerEl = document.getElementById('speaker') as HTMLDivElement;
...
interface Node { speaker: string; speakerColor: string; line: string; choices: Choice[]; }
...
interface DialogueState { nodeId: string; }
...
class DialogueSystem implements System {
  readonly name: string = 'dialogue';
  private renderedNode: string = '';
  pendingNext: string | null = null;
  update(world: World, _dt: number): void {
    const state = world.resources.require<DialogueState>(RESOURCE_DIALOGUE);
    ...
    if (state.nodeId === this.renderedNode) return;
    this.renderedNode = state.nodeId;
    this.render(state.nodeId);
  }
  private render(nodeId: string): void {
...
function paintAvatar(hex: string): void {
  ...
  ctx.fill();
  ctx.strokeStyle = '#14110d'; ctx.lineWidth = 4; ctx.stroke();
}

(function boot(): void {
  const engine = Engine.create({ canvas });
  const resources = engine.world.resources as ResourceRegistry;
  resources.set<DialogueState>(RESOURCE_DIALOGUE, { nodeId: 'start' });
  engine.world.addSystem(new InputSystem(), SYSTEM_PHASE_INPUT);
  engine.world.addSystem(new DialogueSystem(), SYSTEM_PHASE_LOGIC);
  function tick(now: number): void { engine.tick(now); requestAnimationFrame(tick); }
