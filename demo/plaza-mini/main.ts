...
const narratorEl = document.getElementById('narrator') as HTMLDivElement;
...
class NarratorOverlaySystem implements System {
  readonly name: string = 'narrator-overlay';
  private lastSeen: string | null = null;
  private hideAt: number = 0;
  update(world: World, _dt: number): void {
    const log = world.resources.require<DirectorEventLog>(RESOURCE_DIRECTOR_LOG);
...
(async function boot(): Promise<void> {
  ...
  let lineIdx = 0;
  setInterval(() => {
    (bridge as MockDirectorBridge).enqueue({
      id: nextEventId++,
      ts: Date.now() / 1000,
      type: 'narrator.line',
      character_id: 'demo',
