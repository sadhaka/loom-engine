...
import { EventChain, canonicalJson, tickEpoch, suspend, resume, replayEpochEvent, worldStateHash, regionLeaves, globalRegionHash, partitionRegions, diffRegionLeaves, applyPartialSync, } from '@sadhaka/loom-engine';
...
function runScenario(vec) {
    const i = vec.inputs;
    // (1) BUILD - work on a private clone so two runs share nothing.
    const s0 = JSON.parse(JSON.stringify(i.s0));
    ...
    const chain = EventChain.create({ key: i.key, genesis: i.genesis });
    const rec1 = chain.append('EpochResolved', t1.event);
    const rec2 = chain.append('EpochResolved', t2.event);
    if (!rec1 || !rec2)
        throw new Error('chain rejected a live event');
    ...
    const tailVerify = EventChain.verifyRecords(i.key, bundle.chainTail, bundle.tailGenesis, seal);
    const truncated = bundle.chainTail.slice(0, 1);
    const truncatedBare = EventChain.verifyRecords(i.key, truncated, bundle.tailGenesis);
    const truncatedSealed = EventChain.verifyRecords(i.key, truncated, bundle.tailGenesis, seal);
    ...
    for (const id of diff.changed)
        pulledRegions[id] = serverRegions[id];
...
function buildChecks(vec, run, identical) {
    const e = vec.expect;
    return [
...
function paintSprite(w, h, body, eye) {
    ...
    sctx.fillStyle = eye;
    sctx.fillRect(w * 0.25, h * 0.25, 2, 2);
    sctx.fillRect(w * 0.65, h * 0.25, 2, 2);
    return c;
}
const spriteCache = {};
function spriteFor(region, actor) {
...
function groupByRegion(state, prefix) {
    const out = {};
...
function drawWorld(state, prefix, opts) {
    ...
    for (const region of Object.keys(REGION_QUADS)) {
        ...
        if (status === 'pulled') {
            ctx.strokeStyle = '#9fce8f';
            ctx.lineWidth = 2;
            ctx.strokeRect(q.x + 6, q.y + 6, 308, 188);
        }
        if (opts.flashRegion === region) {
            ctx.strokeStyle = '#d97b6c';
            ctx.lineWidth = 3;
            ctx.strokeRect(q.x + 6, q.y + 6, 308, 188);
        }
        ...
        for (let v = 0; v < ids.length; v++) {
            ...
            if (actor) { // offline-actor badge: a coin above the head
                ...
                ctx.beginPath();
                ctx.arc(vx + 16, vy - 8, 4, 0, Math.PI * 2);
                ctx.fill();
            }
...
function renderStats(stage, state, run, extra) {
...
function renderChecks(checks, visible) {
    checksEl.innerHTML = '';
...
(async function boot() {
    ...
    if (!res.ok)
        throw new Error('vector.json fetch failed: ' + res.status);
    ...
    const regionStatus = {};
    for (const id of run.syncedPulled)
        regionStatus[id] = 'pulled';
    for (const id of run.syncedKept)
        regionStatus[id] = 'kept';
    const steps = [];
    ...
    steps.push({
        at: 2800, fn: () => {
            ...
            renderStats('SUSPEND - bundle packed + chain sealed', run.liveStates[1], run, 'seal   count ' + run.seal.count + ', head ' + run.seal.head.slice(0, 16) + '...');
    ...
    tamperBtn.addEventListener('click', () => {
        const tampered = JSON.parse(JSON.stringify(run.pulledRegions));
