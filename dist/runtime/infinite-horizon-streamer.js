...
export function makeChunkHandle(slot, generation) {
    return ((generation & CHUNK_GENERATION_MASK) << CHUNK_GENERATION_SHIFT)
        | (slot & CHUNK_INDEX_MASK);
}
export function chunkSlot(handle) {
    return handle & CHUNK_INDEX_MASK;
}
export function chunkGeneration(handle) {
...
export class InfiniteHorizonStreamer {
    ...
    chunkY;
    chunkState;
    chunkGeneration; // bumped on free - the epoch (gate 5)
    ...
    constructor(config) {
        ...
        if (!Number.isInteger(worldBitsPerAxis) || worldBitsPerAxis < 1 || worldBitsPerAxis > MAX_WORLD_BITS) {
            throw new RangeError('InfiniteHorizonStreamer: worldBitsPerAxis must be an integer in [1, ' + MAX_WORLD_BITS + '], got '
                + worldBitsPerAxis);
        }
        if (!Number.isInteger(horizonRadius) || horizonRadius < 0 || horizonRadius > MAX_HORIZON_RADIUS) {
            throw new RangeError('InfiniteHorizonStreamer: horizonRadius must be an integer in [0, ' + MAX_HORIZON_RADIUS + '], got '
                + horizonRadius);
        }
        if (!Number.isInteger(maxChunks) || maxChunks < 1 || maxChunks > MAX_CHUNKS
            || (maxChunks & (maxChunks - 1)) !== 0) {
            throw new RangeError('InfiniteHorizonStreamer: maxChunks must be a power of two in [1, ' + MAX_CHUNKS + '], got ' + maxChunks);
        }
        if (!Number.isInteger(payloadStride) || payloadStride < 1 || payloadStride > MAX_PAYLOAD_STRIDE) {
            throw new RangeError('InfiniteHorizonStreamer: payloadStride must be an integer in [1, ' + MAX_PAYLOAD_STRIDE + '], got '
                + payloadStride);
        }
        const horizonSpan = 2 * horizonRadius + 1;
        const horizonArea = horizonSpan * horizonSpan;
        if (maxChunks < horizonArea) {
            throw new RangeError('InfiniteHorizonStreamer: maxChunks ' + maxChunks + ' < horizon area (2r+1)^2 = ' + horizonArea
                + ' - the registry could not hold a full horizon');
        }
        if (!Number.isInteger(evictionQueueSize) || evictionQueueSize < horizonArea) {
            throw new RangeError('InfiniteHorizonStreamer: evictionQueueSize must be an integer >= horizon area ' + horizonArea
                + ', got ' + evictionQueueSize);
        }
        if (maxChunks * payloadStride > MAX_TOTAL_PAYLOAD) {
            throw new RangeError('InfiniteHorizonStreamer: maxChunks * payloadStride = ' + (maxChunks * payloadStride)
    ...
    // overflowing.
    updateHorizon(viewChunkX, viewChunkY) {
        ...
        for (let slot = 0; slot < this.maxChunks; slot++) {
            ...
            if (state === CHUNK_STATE_NONE || state === CHUNK_STATE_TOMBSTONE)
                continue;
    ...
    publishChunk(handle, values, count) {
        const slot = this.resolveSlot(handle);
        if (slot < 0 || (this.chunkState[slot] ?? CHUNK_STATE_NONE) !== CHUNK_STATE_LOADING)
            return false;
        ...
        if (n > this.payloadStride) {
            throw new RangeError('InfiniteHorizonStreamer.publishChunk: count ' + n + ' exceeds payloadStride ' + this.payloadStride);
        }
        if (n > values.length) {
            throw new RangeError('InfiniteHorizonStreamer.publishChunk: count ' + n + ' exceeds values.length ' + values.length);
        }
    ...
    cancelChunk(handle) {
        ...
        if (slot < 0)
            return false;
        const state = this.chunkState[slot] ?? CHUNK_STATE_NONE;
        if (state !== CHUNK_STATE_QUEUED && state !== CHUNK_STATE_LOADING)
            return false;
    ...
    dequeueEviction(out) {
        if (out.length < EVICTION_RECORD_STRIDE) {
            throw new RangeError('InfiniteHorizonStreamer.dequeueEviction: out must hold at least ' + EVICTION_RECORD_STRIDE
    ...
    findSlot(morton) {
        ...
        for (let probe = 0; probe < this.maxChunks; probe++) {
            ...
            if (state === CHUNK_STATE_NONE)
                return -1;
            if (state === CHUNK_STATE_TOMBSTONE)
                continue;
            if ((this.chunkMorton[slot] ?? 0) === morton)
                return slot;
        }
        return -1;
    }
    ...
    pushEviction(morton, cx, cy, priorState) {
        if (this.evictionCount >= this.evictionQueueSize) {
            throw new Error('InfiniteHorizonStreamer: eviction queue full (evictionQueueSize=' + this.evictionQueueSize
    ...
    resolveSlot(handle) {
        ...
        if (slot >= this.maxChunks)
            return -1;
        const state = this.chunkState[slot] ?? CHUNK_STATE_NONE;
        if (state === CHUNK_STATE_NONE || state === CHUNK_STATE_TOMBSTONE)
            return -1;
        if ((this.chunkGeneration[slot] ?? 0) !== chunkGeneration(handle))
            return -1;
        return slot;
    }
    requireChunkCoord(value, op, name) {
        if (!Number.isInteger(value) || value < -this.coordBias || value >= this.coordBias) {
            throw new RangeError('InfiniteHorizonStreamer.' + op + ': ' + name + ' ' + value + ' out of ['
                + (-this.coordBias) + ', ' + this.coordBias + ')');
        }
    }
}
//# sourceMappingURL=infinite-horizon-streamer.js.map