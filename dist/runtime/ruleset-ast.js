...
// ---- Degree matching -------------------------------------------------------
function matchDegree(cond, delta, natural, depth = 0) {
    if (depth > MAX_EXPR_DEPTH)
    ...
    switch (cond.type) {
        ...
        case 'or': {
            ...
            for (var i = 0; i < cond.conditions.length; i++) {
                if (matchDegree(cond.conditions[i], delta, natural, depth + 1))
                    return true;
            }
            return false;
        }
...
function applyMutation(state, node, ctx) {
    if (!node || typeof node.type !== 'string')
...
function validateExpr(node, b, depth) {
    bumpNode(b);
    if (depth > MAX_EXPR_DEPTH)
        throw new Error('AST: expression exceeds max depth ' + MAX_EXPR_DEPTH);
    if (!node || typeof node.type !== 'string')
...
function validateDegreeCond(cond, b, depth) {
    bumpNode(b);
    if (depth > MAX_EXPR_DEPTH)
        throw new Error('AST: degree condition exceeds max depth ' + MAX_EXPR_DEPTH);
    if (!cond || typeof cond.type !== 'string')
    ...
    switch (cond.type) {
        ...
        case 'or':
            if (!Array.isArray(cond.conditions))
                throw new Error('AST: or condition requires a conditions array');
            for (var i = 0; i < cond.conditions.length; i++) {
                validateDegreeCond(cond.conditions[i], b, depth + 1);
            }
            return;
        default:
...
function validateMutation(node, b) {
    bumpNode(b);
    if (!node || typeof node.type !== 'string')
        throw new Error('AST: malformed mutation node');
    validateTargetRef(node.target);
...
function validateMutationList(mutations, b) {
    if (!Array.isArray(mutations))
        throw new Error('AST: mutations must be an array');
    for (var i = 0; i < mutations.length; i++)
        validateMutation(mutations[i], b);
}
...
export function applyTriggeredMutations(state, mutations, ctx) {
    ...
    var ctx2 = { state: work, actorId: ctx.actorId, targetId: ctx.targetId, rng: ctx.rng, naturalRoll: null };
    var applied = [];
    for (var i = 0; i < mutations.length; i++) {
        applied.push(applyMutation(work, mutations[i], ctx2));
    }
...
export function evaluateAction(state, check, ctx) {
    ...
    for (var d = 0; d < DEGREE_ORDER.length; d++) {
        ...
        if (!branch)
            continue;
        if (matchDegree(branch.condition, delta, natural)) {
            chosen = name;
    ...
    for (var i = 0; i < muts.length; i++) {
        applied.push(applyMutation(work, muts[i], ctx2));
    }
