...
function parseUintField(input, start, end) {
    if (end <= start)
        return -1;
    let value = 0;
...
export class AIActionInterpreter {
    ...
    head = 0;
    tail = 0;
    constructor(maxQueueSize, maxEntityId) {
        if (!Number.isInteger(maxQueueSize) || maxQueueSize < 2 || maxQueueSize > MAX_QUEUE_SIZE
            || (maxQueueSize & (maxQueueSize - 1)) !== 0) {
            throw new RangeError('AIActionInterpreter: maxQueueSize must be a power of two in [2, '
                + MAX_QUEUE_SIZE + '], got ' + maxQueueSize);
        }
        if (!Number.isInteger(maxEntityId) || maxEntityId < 0 || maxEntityId > U32_MAX) {
            throw new RangeError('AIActionInterpreter: maxEntityId must be an integer in [0, ' + U32_MAX + '], got ' + maxEntityId);
        }
    ...
    pop(out) {
        if (out.length < RECORD_STRIDE) {
            throw new RangeError('AIActionInterpreter.pop: out must hold at least ' + RECORD_STRIDE + ' u32, got ' + out.length);
        }
    ...
    pushRecord(npcId, actionId, targetId) {
    ...
    parse(input) {
        ...
        for (let i = 0; i < len; i++) {
            ...
            let end = i;
            if (end > start && input.charCodeAt(end - 1) === CHAR_CR)
                end--;
            lineStart = i + 1;
            if (end <= start)
                continue; // blank line - not a row, skip silently
            ...
            if (npcId < 0 || actionId < 0 || targetId < 0) {
                rejected++;
                continue;
            }
            if (npcId > this.maxEntityId || targetId > this.maxEntityId) {
                rejected++;
                continue;
            }
            if (this.pushRecord(npcId, actionId, targetId))
                accepted++;
            else
                dropped++;
        }
        ...
        let tailEnd = len;
        if (tailEnd > lineStart && input.charCodeAt(tailEnd - 1) === CHAR_CR)
            tailEnd--;
...
//# sourceMappingURL=ai-action-interpreter.js.map