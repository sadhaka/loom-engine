...
import { LANE_CLOUD, LANE_LOCAL_SLM, PRIORITY_HIGH, REQUEST_HANDLE_INVALID, REQUEST_STATE_NONE, } from './inference-orchestrator.js';
import { PERCEPTION_EVENT_STRIDE, } from './sonic-sync.js';
...
export const DEFAULT_LISTENER_SEMANTIC_MASK = (1 << SONIC_SEMANTIC_PLAYER_FOOTSTEP) |
...
function resolveMoodChannel(channel) {
    if (channel === 'AGITATION')
        return MOOD_AGITATION;
    if (channel === 'FEAR')
        return MOOD_FEAR;
    if (channel === 'CAUTION')
        return MOOD_CAUTION;
    if (channel === 'BLOODLUST')
        return MOOD_BLOODLUST;
    if (channel === 'SORROW')
        return MOOD_SORROW;
    if (channel === 'DOMINANCE')
        return MOOD_DOMINANCE;
    return MOOD_INVALID;
}
// Resolve a death-fx string from the spec to a numeric code.
function resolveDeathFx(id) {
    if (id === 'BONE_SHATTER')
        return DEATH_FX_BONE_SHATTER;
    if (id === 'SOUL_WISP_RISE')
        return DEATH_FX_SOUL_WISP_RISE;
    if (id === 'SIGIL_BURST')
        return DEATH_FX_SIGIL_BURST;
    if (id === 'CYAN_DIVIDE_SPLIT_2')
        return DEATH_FX_CYAN_DIVIDE_SPLIT_2;
    if (id === 'CHAMPION_COLLAPSE')
        return DEATH_FX_CHAMPION_COLLAPSE;
    return DEATH_FX_NONE;
}
// Resolve an inference-lane string from the spec to a numeric code.
function resolveInferenceLane(lane) {
    if (lane === 'local_slm_consented')
        return INFERENCE_LANE_LOCAL_SLM;
    if (lane === 'cloud')
        return INFERENCE_LANE_CLOUD;
...
export const CREATURE_CATALOG = resolveCatalog();
...
export function getVariantIndex(id) {
...
export function getSpec(variantIdx) {
    if (!Number.isInteger(variantIdx))
        return null;
    if (variantIdx < 0 || variantIdx >= CREATURE_CATALOG.length)
        return null;
...
export function isCatalogValid() {
    for (let i = 0; i < CREATURE_CATALOG.length; i++) {
        ...
        if (!spec)
            return false;
        if (spec.moodChannelId === MOOD_INVALID)
            return false;
        if (spec.deathFxCode === DEATH_FX_NONE)
            return false;
    }
    return true;
}
...
export function makeCreatureHandle(slot, generation) {
    return (((generation & 0xffff) << 16) | (slot & 0xffff)) | 0;
}
export function creatureSlot(handle) {
    return handle & 0xffff;
}
export function creatureGeneration(handle) {
...
export class BestiaryKernel {
    ...
    deathFxEventRing1;
    frontRingIsRing0;
    frontDeathFxEventCount;
    backDeathFxEventCount;
    deathFxEventsDroppedTotal;
    ...
    constructor(config, integrations) {
        ...
        if (!Number.isInteger(maxCreatures) || maxCreatures < 1 || maxCreatures > MAX_CREATURES) {
            throw new RangeError('BestiaryKernel: maxCreatures must be in [1, ' + MAX_CREATURES + '], got ' + maxCreatures);
        }
        const deathFxCap = config?.deathFxEventCapacity;
        if (!Number.isInteger(deathFxCap) || deathFxCap < 1 || deathFxCap > MAX_DEATH_FX_CAPACITY) {
            throw new RangeError('BestiaryKernel: deathFxEventCapacity must be in [1, ' + MAX_DEATH_FX_CAPACITY + '], got ' + deathFxCap);
        }
        const maxSimInf = config.maxSimultaneousInference ?? 1;
        if (!Number.isInteger(maxSimInf) || maxSimInf < 0 || maxSimInf > 64) {
            throw new RangeError('BestiaryKernel: maxSimultaneousInference must be in [0, 64], got ' + maxSimInf);
        }
        ...
        if (this.inferenceOrchestrator) {
            this.inferenceOrchestrator.registerActionType(this.inferenceActionType, this.inferenceAllowedResultMask);
        }
    ...
    setSonicSync(ss) { this.sonicSync = ss; }
    setLoomPulse(lp) { this.loomPulse = lp; }
    setInferenceOrchestrator(io) {
        this.inferenceOrchestrator = io;
        if (io) {
            io.registerActionType(this.inferenceActionType, this.inferenceAllowedResultMask);
        }
    }
    setNarrativeMemory(nm) { this.narrativeMemory = nm; }
    setBehaviorTreeFactory(f) {
    ...
    getFreeCount() { return this.freeSlotTop; }
    getCurrentTick() { return this.currentTick; }
    getInflightInferenceCount() { return this.inflightInferenceCount; }
    getSpawnedTotal() { return this.spawnedTotal; }
    getDespawnedTotal() { return this.despawnedTotal; }
    getDeathFxEventsDroppedTotal() { return this.deathFxEventsDroppedTotal; }
    getFrontDeathFxEventCount() { return this.frontDeathFxEventCount; }
    // --- catalog facade (instance-mirror of module exports) ---
    static getVariantIndex(id) { return getVariantIndex(id); }
    static getSpec(variantIdx) { return getSpec(variantIdx); }
    static getCatalog() { return CREATURE_CATALOG; }
    ...
    spawnCreature(variantIdOrIdx, posX, posY) {
        // Resolve variant.
        let vIdx;
        ...
        if (vIdx < 0 || vIdx >= CREATURE_CATALOG.length)
            return CREATURE_HANDLE_INVALID;
        ...
        let listenerSlot = -1;
        if (this.sonicSync) {
            listenerSlot = this.sonicSync.addListener(posX | 0, posY | 0, 0, spec.perceptionRadius | 0, DEFAULT_LISTENER_SEMANTIC_MASK);
        }
        ...
        if (this.narrativeMemory) {
            try {
                ...
                if (recalled && recalled.length > 0) {
                    ...
                    if (tree)
                        tree.setBlackboardEntry(BB_KEY_BIAS_FROM_MEMORY, biased);
                }
                else if (tree) {
        ...
        if (spec.inferenceLaneId === INFERENCE_LANE_CLOUD &&
            this.inferenceOrchestrator &&
            this.inflightInferenceCount < this.maxSimultaneousInference) {
            const reqHandle = this.inferenceOrchestrator.submitRequest(slot, LANE_CLOUD, PRIORITY_HIGH, this.inferenceTokensPerRequest, this.inferenceTtlTicks);
            if (reqHandle !== REQUEST_HANDLE_INVALID) {
                this.inferenceRequestHandle[slot] = reqHandle;
                this.inflightInferenceCount++;
            }
        }
        else if (spec.inferenceLaneId === INFERENCE_LANE_LOCAL_SLM &&
            this.inferenceOrchestrator &&
            this.inflightInferenceCount < this.maxSimultaneousInference) {
            const reqHandle = this.inferenceOrchestrator.submitRequest(slot, LANE_LOCAL_SLM, PRIORITY_HIGH, this.inferenceTokensPerRequest, this.inferenceTtlTicks);
            if (reqHandle !== REQUEST_HANDLE_INVALID) {
                this.inferenceRequestHandle[slot] = reqHandle;
                this.inflightInferenceCount++;
            }
    ...
    despawnCreature(handle) {
        const slot = creatureSlot(handle);
        if (!this.requireLiveHandle(handle))
            return false;
        ...
        if (lSlot >= 0 && this.sonicSync) {
            try {
                this.sonicSync.deactivateListener(lSlot);
            }
            catch (_e) { /* defensive */ }
        }
        ...
        if (reqHandle !== REQUEST_HANDLE_INVALID && this.inferenceOrchestrator) {
            try {
                if (this.inferenceOrchestrator.cancelRequest(reqHandle)) {
                    if (this.inflightInferenceCount > 0)
                        this.inflightInferenceCount--;
                }
            }
            catch (_e) { /* defensive */ }
    ...
    tickCreatures(dtMsFp) {
        if (!Number.isInteger(dtMsFp) || dtMsFp < 0 || dtMsFp > MAX_TICK_DT_FP)
            return;
        ...
        if (this.sonicSync) {
            ...
            for (let i = 0; i < eventCount; i++) {
                if (!this.sonicSync.readEvent(i, scratch, 0))
                    continue;
                ...
                for (let cSlot = 0; cSlot < this.maxCreatures; cSlot++) {
                    if (!this.active[cSlot])
                        continue;
                    if (this.sonicListenerSlot[cSlot] !== listenerSlot)
                        continue;
        ...
        for (let slot = 0; slot < this.maxCreatures; slot++) {
            ...
            tree.setBlackboardEntry(BB_KEY_TICK_COUNT, (tree.getBlackboardEntry(BB_KEY_TICK_COUNT) | 0) + 1);
            // Step the tree.
            try {
                tree.tick(dtMs);
            }
            ...
            const intentAction = typeof intentActionAny === 'number' ? intentActionAny | 0 : CREATURE_ACTION_IDLE;
            ...
            if (lSlot >= 0 && this.sonicSync) {
                try {
                    this.sonicSync.updateListener(lSlot, this.posX[slot] ?? 0, this.posY[slot] ?? 0, 0, spec.perceptionRadius, DEFAULT_LISTENER_SEMANTIC_MASK);
                }
                catch (_e) { /* defensive */ }
            }
    ...
    readDeathFxEvent(i, out, outOffset = 0) {
        if (!Number.isInteger(i) || i < 0 || i >= this.frontDeathFxEventCount)
            return false;
        if (outOffset < 0 || outOffset + DEATH_FX_EVENT_STRIDE > out.length)
            return false;
    ...
    injectPerceptionPing(handle, sourceSlot, distanceFp) {
        const slot = creatureSlot(handle);
        if (!this.requireLiveHandle(handle))
            return false;
        const tree = this.behaviorTrees[slot];
    ...
    applyInferenceDecision(handle, decisionAction) {
        const slot = creatureSlot(handle);
        if (!this.requireLiveHandle(handle))
            return false;
        if (!Number.isInteger(decisionAction))
            return false;
        const tree = this.behaviorTrees[slot];
        if (!tree)
            return false;
    ...
    getCreaturePos(handle, out, outOffset = 0) {
        const slot = creatureSlot(handle);
        if (!this.requireLiveHandle(handle))
            return false;
        if (outOffset < 0 || outOffset + 3 > out.length)
            return false;
    ...
    listActiveHandles(out, outOffset = 0) {
        let written = 0;
...
function readBlackboardNumber(ctx, key, fallback) {
    const v = ctx.blackboard[key];
    return typeof v === 'number' ? v : fallback;
}
function setIntentIdle(ctx) {
...
function setIntentPursue(ctx, speedFp) {
...
function setIntentRetreat(ctx, speedFp) {
...
function distSqFp(px, py, tx, ty) {
    const dx = tx - px;
    const dy = ty - py;
    return dx * dx + dy * dy;
}
function hasLiveTarget(ctx) {
    const th = readBlackboardNumber(ctx, BB_KEY_TARGET_HANDLE, TARGET_HANDLE_NONE);
    return th !== TARGET_HANDLE_NONE;
}
function inMeleeRange(ctx, meleeRangeFp) {
    if (!hasLiveTarget(ctx))
        return false;
    const px = readBlackboardNumber(ctx, BB_KEY_POS_X, 0);
    const py = readBlackboardNumber(ctx, BB_KEY_POS_Y, 0);
    const tx = readBlackboardNumber(ctx, BB_KEY_TARGET_X, px);
...
function inKiteRange(ctx, minFp, maxFp) {
    if (!hasLiveTarget(ctx))
        return false;
    const px = readBlackboardNumber(ctx, BB_KEY_POS_X, 0);
    const py = readBlackboardNumber(ctx, BB_KEY_POS_Y, 0);
...
export function defaultBehaviorTreeFactory(variantIdx) {
    ...
    if (spec.id === 'skel_first_standing_t3') {
        ...
        function isInferenceDecided(ctx) {
            const d = ctx.blackboard[BB_KEY_INFERENCE_DECISION];
            return typeof d === 'number' && d >= 0;
        }
        function applyInferenceAction(ctx) {
