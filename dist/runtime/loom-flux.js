...
export class LoomFlux {
    ...
    pendingList;
    pendingCount = 0;
    constructor(tierStrides, maxEntities) {
        if (!Array.isArray(tierStrides) || tierStrides.length < 1 || tierStrides.length > MAX_TIER_COUNT) {
            throw new RangeError('LoomFlux: tierStrides must be an array of 1 to ' + MAX_TIER_COUNT + ' strides, got '
        ...
        if (!Number.isInteger(maxEntities) || maxEntities < 1 || maxEntities > MAX_ENTITIES_CAP) {
            throw new RangeError('LoomFlux: maxEntities must be an integer in [1, ' + MAX_ENTITIES_CAP + '], got ' + maxEntities);
        }
        ...
        for (let t = 0; t < this.tierCount; t++) {
            ...
            if (typeof s !== 'number' || !Number.isInteger(s) || s < 1 || s > U32_MAX) {
                throw new RangeError('LoomFlux: tierStrides[' + t + '] must be a positive integer, got ' + s);
            }
            this.strides[t] = s;
        }
    ...
    // last target wins.
    requestMigration(entityId, toTier) {
    ...
    tick(globalTick) {
        if (!Number.isInteger(globalTick) || globalTick < 0 || globalTick > U32_MAX) {
            throw new RangeError('LoomFlux.tick: globalTick must be an integer in [0, ' + U32_MAX + '], got ' + globalTick);
        }
    ...
    requireTier(tier, op) {
        if (!Number.isInteger(tier) || tier < 0 || tier >= this.tierCount) {
            throw new RangeError('LoomFlux.' + op + ': tier ' + tier + ' out of [0, ' + this.tierCount + ')');
        }
    }
    requireEntity(entityId, op) {
        if (!Number.isInteger(entityId) || entityId < 0 || entityId >= this.maxEntities) {
            throw new RangeError('LoomFlux.' + op + ': entityId ' + entityId + ' out of [0, ' + this.maxEntities + ')');
        }
    }
    // Append an entity to the end of a tier's dense bucket.
    appendToTier(entityId, tier) {
    ...
    // buckets dense.
    applyMigration(entityId, toTier) {
...
//# sourceMappingURL=loom-flux.js.map