...
export class LoomChrono {
    ...
    snapshot(tick, stateBytes) {
        ...
        if (!stateBytes || stateBytes.byteLength !== this.keyframeBytes) {
            throw new RangeError('LoomChrono.snapshot: stateBytes.byteLength must equal keyframeBytes ('
    ...
    // bulk.
    getKeyframe(handle, destBytes) {
        const slot = this.resolveKeyframeSlot(handle);
        if (slot < 0)
            return false;
        if (!destBytes || destBytes.byteLength < this.keyframeBytes)
            return false;
        const offset = slot * this.keyframeBytes;
    ...
    latestKeyframeHandle() {
        if (this.lastSnapshotSlot < 0)
            return -1;
        if ((this.keyframeValids[this.lastSnapshotSlot] ?? 0) !== SLOT_VALID)
            return -1;
    ...
    logInput(tick, words) {
        ...
        if (!words || words.length < this.inputWords) {
            throw new RangeError('LoomChrono.logInput: words must be ArrayLike<number> of length >= '
        ...
        if ((this.inputValids[slot] ?? 0) === SLOT_VALID) {
            ...
            if (oldTick > this.inputsEvictedMaxTick)
                this.inputsEvictedMaxTick = oldTick;
    ...
    findReplayPlan(targetTick, outInputIndices) {
        if (!Number.isFinite(targetTick)) {
            throw new RangeError('LoomChrono.findReplayPlan: targetTick must be a finite number, got ' + targetTick);
        }
        if (!outInputIndices) {
    ...
    invalidateAfter(tick) {
        if (!Number.isFinite(tick)) {
            throw new RangeError('LoomChrono.invalidateAfter: tick must be a finite number, got ' + tick);
        }
        let count = 0;
        for (let s = 0; s < this.maxKeyframes; s++) {
            if ((this.keyframeValids[s] ?? 0) === SLOT_VALID
    ...
    resolveKeyframeSlot(handle) {
        if (!Number.isInteger(handle) || handle < 0 || handle > 0xffffffff)
            return -1;
        const slot = handle & SLOT_MASK;
        if (slot >= this.maxKeyframes)
            return -1;
        if ((this.keyframeValids[slot] ?? 0) !== SLOT_VALID)
            return -1;
        const gen = (handle >>> GEN_SHIFT) & GEN_MASK;
        if (gen !== (this.keyframeGens[slot] ?? 0))
            return -1;
        return slot;
    }
    requireValidInput(logicalIdx, op) {
        if (!Number.isInteger(logicalIdx)
            || logicalIdx < 0
            || logicalIdx >= this.inputWriteCountInternal) {
            throw new RangeError('LoomChrono.' + op + ': logicalIdx ' + logicalIdx
        ...
        const slot = logicalIdx % this.maxInputs;
        if ((this.inputValids[slot] ?? 0) !== SLOT_VALID) {
            throw new RangeError('LoomChrono.' + op + ': logicalIdx ' + logicalIdx + ' has been invalidated');
        }
    }
}
// ---------- module helpers ----------
...
function requireCap(name, value, lo, hi) {
    if (!Number.isInteger(value) || value < lo || value > hi) {
        throw new RangeError('LoomChrono: ' + name + ' must be an integer in [' + lo + ', ' + hi + '], got ' + value);
    }
}
// Decode helpers for a chrono handle - exported so a consumer can
// inspect a handle without round-tripping through the chrono.
export function chronoSlot(handle) {
    return (handle >>> 0) & SLOT_MASK;
}
export function chronoGeneration(handle) {
    return ((handle >>> 0) >>> GEN_SHIFT) & GEN_MASK;
}
//# sourceMappingURL=loom-chrono.js.map