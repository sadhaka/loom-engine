...
export function makeNeuralMaterialHandle(slot, generation) {
    return ((generation & SLOT_GENERATION_MASK) << SLOT_GENERATION_SHIFT)
        | (slot & SLOT_INDEX_MASK);
}
export function neuralMaterialSlot(handle) {
    return handle & SLOT_INDEX_MASK;
}
export function neuralMaterialGeneration(handle) {
...
export function pickPath(capabilities) {
...
export class NeuralMaterial {
    ...
    slotGeneration;
    slotLastUsedTick;
    slotMipmapReady; // bit i = mipmap level i ready
    slotAtlasIndex; // dense atlas slot in [0, atlasSlotCount)
    // Per-atlas-slot reverse lookup; -1 if unassigned.
    atlasSlotToTableSlot;
    ...
    jobHead = 0;
    jobTail = 0;
    nextJobId = 1; // jobId 0 reserved as INVALID-but-not-quite
    ...
    selectedPath;
    constructor(config) {
        const { atlasCols, atlasRows, sliceCapacity, maxMaterialId, latentDim, dispatchTilePixels, mipmapLevels, jobQueueCapacity, destroyQueueCapacity, destroyDelay, benchmarkWindow, capabilities, } = config;
    ...
    // --- counts ---
    getCachedCount() { return this.cachedCount; }
    getJobQueueCount() { return this.jobTail - this.jobHead; }
    getJobsDroppedTotal() { return this.jobsDroppedTotal; }
    getCompletedJobsTotal() { return this.completedJobsTotal; }
    getDestroyQueueCount() { return this.destroyCount; }
    getLatencySampleCount() { return this.latencyCount; }
    ...
    requestMaterial(materialId, currentTick) {
        if (!this.requireMaterialId(materialId))
            return MATERIAL_HANDLE_INVALID;
        if (!this.requireTick(currentTick))
            return MATERIAL_HANDLE_INVALID;
    ...
    dequeueJob(out, outOffset = 0) {
        if (this.jobHead >= this.jobTail)
            return false;
        if (outOffset < 0 || outOffset + JOB_RECORD_STRIDE > out.length)
            return false;
    ...
    completeJob(handle, jobId, gpuDurationUs) {
        const slot = neuralMaterialSlot(handle);
        const gen = neuralMaterialGeneration(handle);
        if (!Number.isInteger(slot) || slot < 0 || slot >= this.hashTableSize)
            return false;
        if ((this.slotGeneration[slot] ?? 0) !== gen)
            return false;
        if (this.slotState[slot] !== NEURAL_SLOT_STATE_SYNTHESIZING)
            return false;
        if (!Number.isInteger(jobId) || jobId < 1)
            return false;
        if (!Number.isInteger(gpuDurationUs) || gpuDurationUs < 0 || gpuDurationUs > U32_MAX)
            return false;
        ...
        if (this.latencyCount < this.benchmarkWindow)
            this.latencyCount++;
        return true;
    }
    // --- mipmaps (gate 6) ---
    ...
    markMipmapReady(handle, level) {
        ...
        if ((this.slotGeneration[slot] ?? 0) !== gen)
            return false;
        if (this.slotState[slot] !== NEURAL_SLOT_STATE_RESIDENT)
            return false;
        if (!Number.isInteger(level) || level < 0 || level >= this.mipmapLevels)
            return false;
    ...
    getAtlasCoords(handle, out, outOffset = 0) {
        const slot = neuralMaterialSlot(handle);
        const gen = neuralMaterialGeneration(handle);
        if (!Number.isInteger(slot) || slot < 0 || slot >= this.hashTableSize)
            return false;
    ...
    drainDestroyed(currentTick) {
        ...
        if (((currentTick - readyAt) >>> 0) >= 0x80000000)
            return NEURAL_DESTROY_NONE;
    ...
    findSlot(materialId) {
        ...
        for (let probe = 0; probe < this.hashTableSize; probe++) {
            ...
            if (id < 0 && state === NEURAL_SLOT_STATE_FREE)
                return -1; // EMPTY
    ...
    allocAtlasIndex() {
        for (let i = 0; i < this.atlasSlotCount; i++) {
            if ((this.atlasSlotToTableSlot[i] ?? -1) < 0)
                return i;
        }
        return -1;
    }
    ...
    evictLRU(currentTick) {
        ...
        for (let i = 0; i < this.hashTableSize; i++) {
            ...
            if (state === NEURAL_SLOT_STATE_FREE || state === NEURAL_SLOT_STATE_TOMBSTONE)
                continue;
    ...
    isValidPixelCoord(u, v) {
        if (!Number.isInteger(u) || !Number.isInteger(v))
            return false;
        if (u < 0 || v < 0)
            return false;
        if (u >= this.dispatchTilePixels || v >= this.dispatchTilePixels)
            return false;
        return true;
    }
...
//# sourceMappingURL=neural-material.js.map