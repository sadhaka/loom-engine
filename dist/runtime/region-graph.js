...
export class RegionGraph {
    ...
    removeZone(id) {
        ...
        for (var i = 0; i < inFroms.length; i++) {
            var fromNode = this.nodes.get(inFroms[i]);
            if (fromNode && fromNode.outEdges.delete(id))
                this.edgeCount_--;
        }
    ...
    zoneCount() { return this.nodes.size; }
    getZone(id) {
    ...
    addConnection(edge) {
        ...
        if (!edge || typeof edge.fromZone !== 'string'
            || typeof edge.toZone !== 'string')
            return false;
        if (edge.fromZone === edge.toZone)
            return false;
    ...
    addBidirectional(fromZone, toZone, opts = {}) {
        if (this.disposed)
            return false;
        var a = { fromZone: fromZone, toZone: toZone };
        if (opts.weight !== undefined)
            a.weight = opts.weight;
        ...
        if (opts.data !== undefined)
            a.data = opts.data;
        var b = { fromZone: toZone, toZone: fromZone };
        if (opts.weight !== undefined)
            b.weight = opts.weight;
        if (opts.kind !== undefined)
            b.kind = opts.kind;
        if (opts.gate !== undefined)
            b.gate = opts.gate;
    ...
    removeConnection(fromZone, toZone) {
        if (this.disposed)
            return false;
    ...
    hasConnection(fromZone, toZone) {
        var from = this.nodes.get(fromZone);
        return !!from && from.outEdges.has(toZone);
    }
    getConnection(fromZone, toZone) {
        var from = this.nodes.get(fromZone);
        if (!from)
            return null;
    ...
    edgeCount() { return this.edgeCount_; }
    // ---------- traversal ----------
    ...
    // are skipped.
    shortestPath(fromZone, toZone, ctx = {}) {
        if (!this.nodes.has(fromZone) || !this.nodes.has(toZone))
            return null;
        if (fromZone === toZone)
            return [fromZone];
        var dist = new Map();
        ...
        while (open.length > 0) {
            ...
            var bestIdx = 0;
            var bestDist = dist.get(open[0]);
            for (var i = 1; i < open.length; i++) {
                var d = dist.get(open[i]);
    ...
    reachable(fromZone, ctx = {}) {
        if (!this.nodes.has(fromZone))
            return [];
        var visited = new Set();
    ...
    isReachable(fromZone, toZone, ctx = {}) {
        if (fromZone === toZone)
            return this.nodes.has(fromZone);
