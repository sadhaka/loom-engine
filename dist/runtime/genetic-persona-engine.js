...
function requireEntropy(entropy, op) {
...
export class GeneticPersonaEngine {
    ...
    constructor(capacity) {
        if (!Number.isInteger(capacity) || capacity < 1 || capacity > MAX_CAPACITY) {
            throw new RangeError('GeneticPersonaEngine: capacity must be an integer in [1, ' + MAX_CAPACITY + '], got ' + capacity);
        }
    ...
    setGenome(entityId, words) {
        this.requireEntity(entityId, 'setGenome');
        if (!words || words.length < GENOME_WORDS) {
            throw new RangeError('GeneticPersonaEngine.setGenome: words must have at least ' + GENOME_WORDS + ' elements');
        }
        const base = entityId * GENOME_WORDS;
        for (let w = 0; w < GENOME_WORDS; w++) {
    ...
    mutate(entityId, entropy, mutationCount) {
        ...
        if (!Number.isInteger(mutationCount) || mutationCount < 0 || mutationCount > GENOME_BITS) {
            throw new RangeError('GeneticPersonaEngine.mutate: mutationCount ' + mutationCount
    ...
    crossover(parentA, parentB, childId, entropy) {
    ...
    setTrait(entityId, traitBit, value) {
    ...
    getGenomeWord(entityId, wordIndex) {
    ...
    hammingDistance(entityA, entityB) {
    ...
    // ---------- private ----------
    requireEntity(entityId, op) {
        if (!Number.isInteger(entityId) || entityId < 0 || entityId >= this.capacity) {
            throw new RangeError('GeneticPersonaEngine.' + op + ': entityId ' + entityId + ' out of [0, ' + this.capacity + ')');
        }
    }
    requireTraitBit(traitBit, op) {
        if (!Number.isInteger(traitBit) || traitBit < 0 || traitBit >= GENOME_BITS) {
            throw new RangeError('GeneticPersonaEngine.' + op + ': traitBit ' + traitBit + ' out of [0, ' + GENOME_BITS + ')');
        }
    }
    requireWordIndex(wordIndex, op) {
        if (!Number.isInteger(wordIndex) || wordIndex < 0 || wordIndex >= GENOME_WORDS) {
            throw new RangeError('GeneticPersonaEngine.' + op + ': wordIndex ' + wordIndex + ' out of [0, ' + GENOME_WORDS + ')');
        }
    }
}
//# sourceMappingURL=genetic-persona-engine.js.map