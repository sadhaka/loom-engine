...
export class LoomStudioOrchestrator {
    ...
    querySlotState;
    querySlotType;
    querySlotTelemetryEpoch;
    querySlotPayloadOffset;
    querySlotPayloadLength;
    querySlotSubmittedAt;
    querySlotGeneration;
    nextQuerySlot = 0;
    ...
    factGeneration;
    currentTick = 0;
    queriesEnqueuedTotal = 0;
    queriesCompletedTotal = 0;
    queriesRejectedTotal = 0;
    factsProposedTotal = 0;
    factsRejectedTotal = 0;
    factsExpiredTotal = 0;
    constructor(config) {
    ...
    getTelemetryEpoch() { return this.telemetryEpoch; }
    getQueriesEnqueuedTotal() { return this.queriesEnqueuedTotal; }
    getQueriesCompletedTotal() { return this.queriesCompletedTotal; }
    getQueriesRejectedTotal() { return this.queriesRejectedTotal; }
    getFactsProposedTotal() { return this.factsProposedTotal; }
    getFactsRejectedTotal() { return this.factsRejectedTotal; }
    getFactsExpiredTotal() { return this.factsExpiredTotal; }
    // --- telemetry (gate 2) ---
    recordTelemetrySignal(signalId, value) {
        if (!this.requireSignal(signalId))
            return false;
    ...
    registerQueryAllowedMask(queryType, allowedActionMask) {
        if (!this.requireQueryType(queryType))
            return STUDIO_REASON_BAD_QUERY_TYPE;
        if (!Number.isInteger(allowedActionMask) || allowedActionMask < 0
            || allowedActionMask > U32_MAX)
    ...
    isQueryTypeRegistered(queryType) {
        if (!this.requireQueryType(queryType))
            return false;
    ...
    // on rejection.
    enqueueQuery(queryType, telemetryEpoch, payloadOffset, payloadLength) {
        if (!this.requireQueryType(queryType))
            return QUERY_HANDLE_INVALID;
        if (!this.queryRegistered[queryType])
            return QUERY_HANDLE_INVALID;
        if (!Number.isInteger(telemetryEpoch) || telemetryEpoch < 0 || telemetryEpoch > U32_MAX) {
            return QUERY_HANDLE_INVALID;
        }
        if (!Number.isInteger(payloadOffset) || payloadOffset < 0)
            return QUERY_HANDLE_INVALID;
    ...
    drainQueryBatch(count, out) {
        if (!Number.isInteger(count) || count < 1)
            return 0;
        if (out.length < count * QUERY_RECORD_STRIDE)
            return 0;
        let drained = 0;
        for (let s = 0; s < this.maxQueries && drained < count; s++) {
            if (this.querySlotState[s] !== QUERY_STATE_PENDING)
                continue;
    ...
    completeQuery(handle, telemetryEpoch, proposedActionMask) {
        const slot = handleSlot(handle);
        const gen = handleGen(handle);
        if (!this.requireQuerySlot(slot))
            return STUDIO_REASON_BAD_HANDLE;
        if ((this.querySlotGeneration[slot] ?? 0) !== gen)
            return STUDIO_REASON_BAD_HANDLE;
        if (this.querySlotState[slot] !== QUERY_STATE_INFLIGHT)
            return STUDIO_REASON_BAD_STATE;
    ...
    proposeFact(factIndex, factTier, sourceId, telemetryEpoch, expiresAtTick, payloadHash) {
        ...
        if (factTier !== FACT_TIER_LOW && factTier !== FACT_TIER_MEDIUM && factTier !== FACT_TIER_HIGH) {
            ...
            this.factsRejectedTotal++;
            if (factTier === FACT_TIER_VERIFIED)
                return STUDIO_REASON_TIER_FORBIDDEN;
    ...
    adminProposeFact(factIndex, factTier, sourceId, telemetryEpoch, expiresAtTick, payloadHash) {
        if (!Number.isInteger(factIndex) || factIndex < 0 || factIndex >= this.maxFacts) {
            return STUDIO_REASON_BAD_FACT_INDEX;
        }
        if (factTier < FACT_TIER_LOW || factTier > FACT_TIER_VERIFIED)
            return STUDIO_REASON_BAD_TIER;
        return this.writeFact(factIndex, factTier, sourceId, telemetryEpoch, expiresAtTick, payloadHash);
    }
    writeFact(factIndex, factTier, sourceId, telemetryEpoch, expiresAtTick, payloadHash) {
    ...
    // provenance + expiry).
    readFact(factIndex, out, outOffset = 0) {
        if (!Number.isInteger(factIndex) || factIndex < 0 || factIndex >= this.maxFacts)
            return false;
        if (outOffset < 0 || outOffset + FACT_RECORD_STRIDE > out.length)
            return false;
    ...
    tick(t) {
        ...
        for (let i = 0; i < this.maxFacts; i++) {
            ...
            if (state === FACT_STATE_NONE || state === FACT_STATE_EXPIRED)
                continue;
...
//# sourceMappingURL=studio-orchestrator.js.map