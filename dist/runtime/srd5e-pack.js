...
import { MAX_SLOT_LEVEL, casterKind, spendLowestAvailable, spellBaseLevel, spellRequiresConcentration, totalDiceForCast, upcastEffect, } from './srd5e-spell-slots.js';
export var CANTRIPS = {
...
export var CLASS_LEVELED_SPELLS = {
...
export function classCanCast(classId, spellId) {
...
export function eldritchBlastBeams(level) {
...
export function scaledCantripDice(dice, level) {
    var m = DICE_RE.exec(typeof dice === 'string' ? dice : '');
    if (!m)
        return dice;
    var count = parseInt(m[1], 10) * cantripDiceCount(level);
...
function landingCondition(saveAbility, targetRef) {
    var base = { type: 'delta_lte', value: -1 };
...
function attackCheck(atkProp, hitMuts, critMuts) {
    return {
        type: 'check',
...
export function buildWeaponAttackCheck(opts) {
...
export function buildAttackCantripCheck(cantripId, casterLevel, opts) {
    ...
    var hitVal = agonizing ? exAdd(exDice(hitDice), exProp('actor', 'cha_mod')) : exDice(hitDice);
    var critVal = agonizing ? exAdd(exDice(critDice), exProp('actor', 'cha_mod')) : exDice(critDice);
    var hitMuts = [muSubHp('target', hitVal)];
    var critMuts = [muSubHp('target', critVal)];
    if (def.rider_tag) {
...
export function buildSaveCantripCheck(cantripId, casterLevel) {
    var id = normId(cantripId);
    var def = CANTRIPS[id];
    ...
    if (def.damaged_dice) {
        ...
        if (def.rider_tag)
            landMuts.push(muAddTag('target', def.rider_tag));
    }
    return {
        type: 'check',
...
export function buildAttackSpellCheck(spellId, castSlotLevel) {
    var id = normId(spellId);
    ...
    var critDice = doubleDice(hitDice);
    var hitVal = def.add_ability_to_damage
        ? exAdd(exDice(hitDice), exProp('actor', 'spell_mod'))
        : exDice(hitDice);
    var critVal = def.add_ability_to_damage
...
export function buildSaveSpellCheck(spellId, castSlotLevel) {
    var id = normId(spellId);
    var def = LEVELED_SPELLS[id];
    ...
    var savedMuts = def.half_on_save ? [muSubHp('target', exHalf(exDice(full)))] : [];
    return {
        type: 'check',
        roll: exAdd(exDice('1d20'), exProp('target', saveProp(def.save_ability))),
        dc: exProp('actor', 'spell_dc'),
...
export function buildMultiTargetSaveTrigger(spellId, castSlotLevel, opts) {
    var id = normId(spellId);
    var def = LEVELED_SPELLS[id];
...
export function buildMagicMissileTrigger(castSlotLevel) {
    var def = LEVELED_SPELLS['magic_missile'];
    var info = upcastEffect('magic_missile', castSlotLevel);
    var darts = def.darts + (info ? info.extra_instances : 0);
    var dartEq = def.base_dice + '+' + String(def.dart_bonus);
    return [{
...
export function buildHealTrigger(spellId, castSlotLevel) {
    var id = normId(spellId);
    var def = LEVELED_SPELLS[id];
...
function clonePoolShallow(slots) {
...
// never mutated.
export function planLeveledCast(slots, spellId, classId, requestedSlotLevel) {
    var id = normId(spellId);
    var def = LEVELED_SPELLS[id];
    if (!def) {
...
//# sourceMappingURL=srd5e-pack.js.map