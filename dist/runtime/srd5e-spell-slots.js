...
export var MAX_SLOT_LEVEL = 9;
export var PACT_KEY = 'pact';
// ---- Caster taxonomy (SRD 5.1) ---------------------------------------------
var FULL_CASTERS = {
    bard: true, cleric: true, druid: true, sorcerer: true, wizard: true,
};
var HALF_CASTERS = { paladin: true, ranger: true };
var PACT_CASTERS = { warlock: true };
var SPELL_ABILITY = {
...
export function spellAbilityForClass(classId) {
...
function normEntry(maxRaw, usedRaw) {
    var max = isInt(maxRaw) && maxRaw > 0 ? maxRaw : 0;
    var used = isInt(usedRaw) ? usedRaw : 0;
    if (used < 0)
        used = 0;
    if (used > max)
        used = max;
...
export function sanitizeSlotPool(slots) {
    return clonePool(slots);
}
function setPact(pool, p) {
    pool.pact = p;
}
function clonePool(slots) {
    var out = {};
    for (var k in slots) {
        ...
        if (k === PACT_KEY) {
            ...
            if (p) {
                ...
                setPact(out, { slot_level: p.slot_level, max: np.max, used: np.used });
            }
        }
        else {
            var e = slots[k];
            if (e) {
                var ne = normEntry(e.max, e.used);
                out[k] = { max: ne.max, used: ne.used };
            }
        }
    }
    return out;
...
export function spellSlotsFor(classId, level) {
    var out = {};
    ...
    if (kind === 'pact') {
        ...
        if (row)
            setPact(out, { slot_level: row[0], max: row[1], used: 0 });
        return out;
    }
    ...
    for (var i = 0; i < slots.length; i++) {
        var max = slots[i];
        if (max > 0)
            out[String(i + 1)] = { max: max, used: 0 };
    }
    return out;
}
// Highest slot level with max > 0 anywhere in the pool (pact included). 0 if none.
export function highestSlotLevel(slots) {
    var best = 0;
    for (var L = 1; L <= MAX_SLOT_LEVEL; L++) {
        var e = slots[String(L)];
        if (e && normEntry(e.max, e.used).max > 0)
            best = L; // P2: clamp untrusted max
    }
    var p = slots.pact;
    if (p && normEntry(p.max, p.used).max > 0 && p.slot_level > best)
        best = p.slot_level;
    return best;
}
// Unused slots at exactly slotLevel (numeric tier + a matching pact tier summed).
export function slotAvailable(slots, slotLevel) {
    var n = 0;
...
function spendReject(slots, reason) {
...
export function spendSlot(slots, slotLevel) {
    if (slotLevel === 0)
        return spendReject(slots, 'not_a_slot'); // a cantrip is not a slot
...
export function spendLowestAvailable(slots, minLevel) {
    if (minLevel === 0)
        return spendReject(slots, 'not_a_slot');
    if (!isInt(minLevel) || minLevel < 1 || minLevel > MAX_SLOT_LEVEL) {
        return spendReject(slots, 'bad_slot_level');
    }
    for (var L = minLevel; L <= MAX_SLOT_LEVEL; L++) {
        if (slotAvailable(slots, L) > 0)
            return spendSlot(slots, L);
    }
...
export function restoreSlot(slots, slotLevel, count) {
    var out = clonePool(slots);
    var n = isInt(count) && count > 0 ? count : 1;
    var e = out[String(slotLevel)];
    if (e) {
...
export function slotsRemaining(slots) {
    ...
    if (p) {
        ...
        if (np.max > 0) {
            ...
            out[p.slot_level] = (out[p.slot_level] === undefined ? 0 : out[p.slot_level]) + rem;
        }
    }
    return out;
}
// Long rest: every slot refreshes - a fresh pool for the class + level.
export function longRest(classId, level) {
...
export function shortRest(classId, level, slots) {
    var out = clonePool(slots);
    if (casterKind(classId) !== 'pact')
        return out;
    var fresh = spellSlotsFor(classId, level);
    var fp = fresh.pact;
    if (fp)
        setPact(out, { slot_level: fp.slot_level, max: fp.max, used: 0 });
    return out;
}
...
export function widenSlots(stored, classId, level) {
    var kind = casterKind(classId);
    if (kind === null)
        return stored ? clonePool(stored) : {};
    var fresh = spellSlotsFor(classId, level);
...
export function spellRequiresConcentration(spellId) {
    var def = SPELL_UPCAST[normId(spellId)];
    return def ? def.concentration : false;
}
export function spellBaseLevel(spellId) {
    var def = SPELL_UPCAST[normId(spellId)];
...
export function upcastEffect(spellId, castSlotLevel) {
    ...
    if (def.added_dice !== '' && steps > 0) {
        ...
        if (m)
            addedDice = String(parseInt(m[1], 10) * steps) + 'd' + m[2];
    }
...
export function totalDiceForCast(baseDice, spellId, castSlotLevel) {
    ...
    if (am[2] !== bm[2])
        return baseDice;
    var total = parseInt(bm[1], 10) + parseInt(am[1], 10);
    return String(total) + 'd' + bm[2] + (bm[3] ? bm[3] : '');
}
//# sourceMappingURL=srd5e-spell-slots.js.map