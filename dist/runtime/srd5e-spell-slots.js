...
function clonePool(slots) {
    ...
    for (var k in slots) {
        ...
        if (k === PACT_KEY) {
            var p = slots.pact;
            if (p)
                setPact(out, { slot_level: p.slot_level, max: p.max, used: p.used });
        }
        else {
            var e = slots[k];
            if (e)
...
export function highestSlotLevel(slots) {
    ...
    var p = slots.pact;
    if (p && p.max > 0 && p.slot_level > best)
        best = p.slot_level;
    return best;
}
...
export function slotAvailable(slots, slotLevel) {
    ...
    if (e && e.max > e.used)
        n += e.max - e.used;
    var p = slots.pact;
    if (p && p.slot_level === slotLevel && p.max > p.used)
        n += p.max - p.used;
    return n;
}
...
export function slotsRemaining(slots) {
    ...
    for (var L = 1; L <= MAX_SLOT_LEVEL; L++) {
        var e = slots[String(L)];
        if (e && e.max > 0)
            out[L] = e.max - e.used > 0 ? e.max - e.used : 0;
    }
    var p = slots.pact;
    if (p && p.max > 0) {
