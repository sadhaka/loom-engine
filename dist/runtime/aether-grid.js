...
export class AetherGrid {
    ...
    transferFromNode;
    transferToNode;
    transferProposedEpoch;
    transferIdempotencyKey;
    transferDeadlineTick;
    transferLastUpdated;
    transferGeneration;
    nextTransferSlot = 0;
    // Per-chunk replication state (gate 4).
    chunkSeqCounter;
    chunkLastDeliveredSeq; // -1 = none yet
    // Replication ring (gate 4).
    replRingChunkId;
    replRingSeq;
    replRingEpoch;
    replRingIdempotency;
    replRingPayloadOffset;
    replRingPayloadLength;
    replRingEnqueuedAt;
    replHead = 0;
    replTail = 0;
    // Payload arena.
    ...
    expiredTotal = 0;
    splitBrainsTotal = 0;
    replicationDroppedTotal = 0;
    replicationDeliveredTotal = 0;
    constructor(config) {
        const { maxEntities, maxNodes, maxTransfers, maxChunks, replicationRingCapacity, payloadArenaBytes, localNodeId, defaultDeadlineTicks, idempotencyWindow, } = config;
    ...
    getCommitsTotal() { return this.commitsTotal; }
    getAbortsTotal() { return this.abortsTotal; }
    getExpiredTotal() { return this.expiredTotal; }
    getSplitBrainsTotal() { return this.splitBrainsTotal; }
    getReplicationDroppedTotal() { return this.replicationDroppedTotal; }
    getReplicationDeliveredTotal() { return this.replicationDeliveredTotal; }
    getReplicationPending() { return this.replTail - this.replHead; }
    ...
    setOwner(entityId, ownerNode, epoch) {
        if (!this.requireEntity(entityId))
            return false;
        if (!this.requireNode(ownerNode))
            return false;
        if (!Number.isInteger(epoch) || epoch < 0 || epoch > U32_MAX)
            return false;
    ...
    proposeTransfer(entityId, fromNode, toNode, idempotencyKey, deadlineTicks) {
        if (!this.requireEntity(entityId))
            return TRANSFER_HANDLE_INVALID;
        if (!this.requireNode(fromNode) || !this.requireNode(toNode))
            return TRANSFER_HANDLE_INVALID;
        ...
        if (!Number.isInteger(ddl) || ddl < 1 || ddl > MAX_TTL)
            return TRANSFER_HANDLE_INVALID;
    ...
    commitTransfer(handle) {
        const slot = handleSlot(handle);
        const gen = handleGen(handle);
        if (!this.requireTransferSlot(slot))
            return AETHER_REASON_BAD_HANDLE;
        if ((this.transferGeneration[slot] ?? 0) !== gen)
            return AETHER_REASON_BAD_HANDLE;
        const state = this.transferState[slot] ?? 0;
        // Idempotent re-commit.
        if (state === TRANSFER_STATE_COMMITTED)
            return AETHER_REASON_NONE;
        if (state !== TRANSFER_STATE_PROPOSED)
            return AETHER_REASON_BAD_STATE;
        const entityId = this.transferEntity[slot] ?? -1;
        if (!this.requireEntity(entityId))
            return AETHER_REASON_BAD_ENTITY;
    ...
    abortTransfer(handle) {
        ...
        if ((this.transferGeneration[slot] ?? 0) !== gen)
            return AETHER_REASON_BAD_HANDLE;
        const state = this.transferState[slot] ?? 0;
        if (state === TRANSFER_STATE_ABORTED)
            return AETHER_REASON_NONE;
        if (state !== TRANSFER_STATE_PROPOSED)
            return AETHER_REASON_BAD_STATE;
    ...
    readTransfer(handle, out, outOffset = 0) {
        const slot = handleSlot(handle);
        const gen = handleGen(handle);
        if (!this.requireTransferSlot(slot))
            return false;
        if ((this.transferGeneration[slot] ?? 0) !== gen)
            return false;
        if (outOffset < 0 || outOffset + TRANSFER_RECORD_STRIDE > out.length)
            return false;
    ...
    observeRemoteCommit(entityId, observedEpoch, observedOwner) {
        if (!this.requireEntity(entityId))
            return AETHER_REASON_BAD_ENTITY;
        if (!this.requireNode(observedOwner))
            return AETHER_REASON_BAD_NODE;
    ...
    // or -1 on rejection.
    enqueueChunkPayload(chunkId, epoch, idempotencyKey, payload) {
    ...
    drainChunkReplication(out, outOffset = 0) {
        if (this.replHead >= this.replTail)
            return false;
        if (outOffset < 0 || outOffset + REPLICATION_RECORD_STRIDE > out.length)
            return false;
    ...
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    _reserved_p2(_n) { return nextPow2(_n); }
    // --- lifecycle ---
    clear() {
