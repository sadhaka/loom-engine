...
export class OmniveilSKB {
    ...
    activeCount = 0;
    constructor(maxClaims, maxSources, maxClaimsPerSource) {
        if (!Number.isInteger(maxClaims) || maxClaims < 2 || maxClaims > MAX_CLAIMS
            || (maxClaims & (maxClaims - 1)) !== 0) {
            throw new RangeError('OmniveilSKB: maxClaims must be a power of two in [2, ' + MAX_CLAIMS + '], got ' + maxClaims);
        }
        if (!Number.isInteger(maxSources) || maxSources < 1 || maxSources > MAX_SOURCES) {
            throw new RangeError('OmniveilSKB: maxSources must be an integer in [1, ' + MAX_SOURCES + '], got ' + maxSources);
        }
        if (!Number.isInteger(maxClaimsPerSource) || maxClaimsPerSource < 1 || maxClaimsPerSource > maxClaims) {
            throw new RangeError('OmniveilSKB: maxClaimsPerSource must be an integer in [1, maxClaims=' + maxClaims + '], got '
                + maxClaimsPerSource);
        }
    ...
    assertClaim(source, subject, predicate, object) {
        ...
        for (let w = 0; w < this.sourceWords; w++)
            this.sourceBits[rowStart + w] = 0;
    ...
    retractClaim(source, subject, predicate, object) {
        ...
        if ((word & bit) === 0)
            return false; // this source was not asserting it
    ...
    consensusOf(subject, predicate, object) {
    ...
    isContested(subject, predicate) {
        ...
        for (let slot = 0; slot < this.maxClaims; slot++) {
            if (((this.flags[slot] ?? 0) & CLAIM_FLAG_ACTIVE) === 0)
                continue;
            if ((this.subject[slot] ?? 0) !== subject || (this.predicate[slot] ?? 0) !== predicate)
                continue;
    ...
    hashTriple(subject, predicate, object) {
        let h = FNV_OFFSET;
        h ^= subject;
    ...
    findSlot(subject, predicate, object) {
        const mask = this.mask;
        const start = this.hashTriple(subject, predicate, object) & mask;
        for (let probe = 0; probe < this.maxClaims; probe++) {
            const slot = (start + probe) & mask;
    ...
    requireSource(source, op) {
        if (!Number.isInteger(source) || source < 0 || source >= this.maxSources) {
            throw new RangeError('OmniveilSKB.' + op + ': source ' + source + ' out of [0, ' + this.maxSources + ')');
        }
    }
    requireU32(value, op, name) {
        if (!Number.isInteger(value) || value < 0 || value > U32_MAX) {
            throw new RangeError('OmniveilSKB.' + op + ': ' + name + ' ' + value + ' must be a u32 integer in [0, ' + U32_MAX + ']');
        }
    }
}
//# sourceMappingURL=omniveil-skb.js.map