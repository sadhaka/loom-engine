...
function deriveSeed(masterSeed, salt) {
    return masterSeed + ':' + salt;
}
function masterToString(seed) {
    if (typeof seed === 'number')
        return 'n:' + (seed >>> 0).toString(16);
...
export class WorldSeed {
    ...
    generateWorld(opts) {
        if (!(opts.width > 0))
            throw new Error('WorldSeed: width must be > 0');
        if (!(opts.height > 0))
            throw new Error('WorldSeed: height must be > 0');
        ...
        for (var i = 0; i < biomeSpecs.length; i++) {
            ...
            var spec = { id: b.id };
            if (b.minElev !== undefined)
                spec.minElev = b.minElev;
            if (b.maxElev !== undefined)
                spec.maxElev = b.maxElev;
            if (b.minMoist !== undefined)
                spec.minMoist = b.minMoist;
            if (b.maxMoist !== undefined)
                spec.maxMoist = b.maxMoist;
            bm.defineBiome(spec);
            biomeNames.push(b.id);
        }
        var fallback = biomeNames.length > 0 ? biomeNames[0] : null;
        if (fallback)
            bm.setFallback(fallback);
    ...
    getMasterSeed() { return this.masterSeed; }
    getCorpusSize() { return this.corpus.length; }
}
function mulberry32FromString(seed) {
    var t = fnv1a(seed) >>> 0;
    return function () {
        t = (t + 0x6D2B79F5) >>> 0;
        var x = t;
...
//# sourceMappingURL=world-seed.js.map