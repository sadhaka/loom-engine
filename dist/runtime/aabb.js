...
export function aabbFromRect(x, y, width, height) {
...
export function aabbContainsPoint(box, px, py) {
...
export function aabbWidth(box) {
    return box.maxX - box.minX;
}
export function aabbHeight(box) {
    return box.maxY - box.minY;
}
export function aabbArea(box) {
    return aabbWidth(box) * aabbHeight(box);
}
// Center of the box.
export function aabbCenter(box, out) {
...
export function aabbExpand(box, margin) {
    box.minX -= margin;
    box.minY -= margin;
    box.maxX += margin;
    box.maxY += margin;
...
export function aabbTranslate(box, dx, dy) {
    box.minX += dx;
    box.minY += dy;
    box.maxX += dx;
    box.maxY += dy;
...
export function aabbRangeQuery(boxes, query, out) {
    var result = out ? out : [];
    if (out)
        result.length = 0;
    for (var i = 0; i < boxes.length; i++) {
        var b = boxes[i];
        if (b && aabbOverlaps(b, query))
            result.push(i);
    }
    return result;
}
...
export function aabbRaycastSegment(box, p0x, p0y, p1x, p1y) {
    var dx = p1x - p0x;
    var dy = p1y - p0y;
    var tMin = 0;
    var tMax = 1;
