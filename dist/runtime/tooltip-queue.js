...
export class TooltipQueue {
    ...
    show(anchorId, content, opts = {}) {
        ...
        var id = this.nextId++;
        var initialState = this.fadeInDefault > 0 ? 'fadeIn' : 'visible';
    ...
    tick(dtMs) {
        ...
        if (this.onRemoved) {
            ...
            for (var j = 0; j < removed.length; j++) {
                try {
                    cb(this.publicView(removed[j]), 'expired');
                }
                catch { /* ignore */ }
            }
        }
    ...
    forEach(cb) {
        ...
        for (var i = 0; i < this.tips.length; i++) {
            try {
                cb(this.publicView(this.tips[i]));
            }
            catch { /* ignore */ }
        }
    }
    list() {
        var out = [];
        for (var i = 0; i < this.tips.length; i++) {
            out.push(this.publicView(this.tips[i]));
        }
        return out;
    }
    ...
    clear() {
        ...
        if (this.onRemoved) {
            ...
            for (var i = 0; i < toRemove.length; i++) {
                try {
                    cb(this.publicView(toRemove[i]), 'hidden');
                }
                catch { /* ignore */ }
            }
        }
    ...
    evictOne() {
        if (this.tips.length === 0)
            return;
        var bestIdx = 0;
        var best = this.tips[0];
        for (var i = 1; i < this.tips.length; i++) {
            var cand = this.tips[i];
        ...
        if (this.onRemoved) {
            try {
                this.onRemoved(this.publicView(best), 'evicted');
            }
            catch { /* ignore */ }
        }
    }
...
//# sourceMappingURL=tooltip-queue.js.map