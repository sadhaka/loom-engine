...
export class LoomVerify {
    ...
    resyncViolationWeight;
    rejectViolationWeight;
    passDecayWeight;
    acceptedTickSkew;
    ...
    verdictRing;
    verdictHead = 0; // consumer position
    verdictTail = 0; // producer position
    verdictsDroppedTotal = 0;
    ...
    currentTick = 0;
    constructor(config) {
        const { maxEntities, maxActionTypes, maxRegions, nonceTableCapacity, verdictRingCapacity, maxKeyEpochs, payloadStride, nonceTtlTicks, gracePeriodTicks, violationDecayPerTick, resyncViolationWeight, rejectViolationWeight, passDecayWeight, acceptedTickSkew, } = config;
        ...
        if (!Number.isInteger(violationDecayPerTick) || violationDecayPerTick < 0
            ...
            throw new RangeError('LoomVerify: violationDecayPerTick must be in [0, ' + MAX_VIOLATION_DECAY_PER_TICK + '], got ' + violationDecayPerTick);
        }
    ...
    // --- counts ---
    getNonceEntryCount() { return this.nonceEntryCount; }
    getVerdictsPending() { return this.verdictTail - this.verdictHead; }
    getVerdictsDroppedTotal() { return this.verdictsDroppedTotal; }
    getCurrentTick() { return this.currentTick; }
    getActiveKeyEpoch() { return this.activeKeyEpoch; }
    isActiveKeyEpochSet() { return this.activeKeyEpochSet; }
    ...
    setActionValueClass(actionType, valueClass) {
    ...
    getActionValueClass(actionType) {
        if (!Number.isInteger(actionType) || actionType < 0 || actionType >= this.maxActionTypes)
            return VALUE_CLASS_LOW;
    ...
    setRegionRoot(regionId, root) {
        if (!Number.isInteger(regionId) || regionId < 0 || regionId >= this.maxRegions)
            return false;
        if (!Number.isInteger(root) || root < 0 || root > 0xffffffff)
            return false;
        this.regionRoot[regionId] = root >>> 0;
        this.regionRootSet[regionId] = 1;
        return true;
    }
    getRegionRoot(regionId) {
        if (!Number.isInteger(regionId) || regionId < 0 || regionId >= this.maxRegions)
            return 0;
    ...
    rotateKey(newEpoch) {
        if (!Number.isInteger(newEpoch) || newEpoch < 0 || newEpoch >= this.maxKeyEpochs)
            return false;
        if (this.keyEpochState[newEpoch] !== KEY_EPOCH_INACTIVE)
            return false; // can't reactivate
    ...
    retireKeyEpoch(epoch) {
        if (!Number.isInteger(epoch) || epoch < 0 || epoch >= this.maxKeyEpochs)
            return false;
        this.keyEpochState[epoch] = KEY_EPOCH_INACTIVE;
        return true;
    }
    getKeyEpochState(epoch) {
        if (!Number.isInteger(epoch) || epoch < 0 || epoch >= this.maxKeyEpochs)
            return KEY_EPOCH_INACTIVE;
    ...
    submitClaim(claim) {
        ...
        if (!claim || !claim.payloadFp || claim.payloadFp.length < this.payloadStride) {
            return this.emitVerdict(VERDICT_REJECT, REASON_BAD_ACTION, claim?.entityId ?? 0, claim?.actionType ?? 0, claim?.nonce ?? 0, claim?.regionId ?? 0);
        }
        if (!Number.isInteger(claim.entityId) || claim.entityId < 0 || claim.entityId >= this.maxEntities) {
            return this.emitVerdict(VERDICT_REJECT, REASON_BAD_ENTITY, 0, claim.actionType, claim.nonce, claim.regionId);
        }
        if (!Number.isInteger(claim.actionType) || claim.actionType < 0 || claim.actionType >= this.maxActionTypes) {
            return this.emitVerdict(VERDICT_REJECT, REASON_BAD_ACTION, claim.entityId, 0, claim.nonce, claim.regionId);
        }
        if (!Number.isInteger(claim.regionId) || claim.regionId < 0 || claim.regionId >= this.maxRegions) {
            return this.emitVerdict(VERDICT_REJECT, REASON_BAD_REGION_ROOT, claim.entityId, claim.actionType, claim.nonce, 0);
        }
        if (!Number.isInteger(claim.tick) || claim.tick < 0 || claim.tick > 0xffffffff) {
            return this.emitVerdict(VERDICT_REJECT, REASON_BAD_TICK, claim.entityId, claim.actionType, claim.nonce, claim.regionId);
        }
        if (!Number.isInteger(claim.nonce) || claim.nonce < 0 || claim.nonce > 0xffffffff) {
            return this.emitVerdict(VERDICT_REJECT, REASON_BAD_NONCE, claim.entityId, claim.actionType, 0, claim.regionId);
        }
        if (!Number.isInteger(claim.regionRoot) || claim.regionRoot < 0 || claim.regionRoot > 0xffffffff) {
            return this.emitVerdict(VERDICT_REJECT, REASON_BAD_REGION_ROOT, claim.entityId, claim.actionType, claim.nonce, claim.regionId);
        }
        ...
        if (tickDelta > this.acceptedTickSkew) {
            ...
            return this.emitVerdict(VERDICT_RESYNC, REASON_BAD_TICK, claim.entityId, claim.actionType, claim.nonce, claim.regionId);
        }
        ...
        if (!this.regionRootSet[claim.regionId]) {
            ...
            return this.emitVerdict(VERDICT_RESYNC, REASON_BAD_REGION_ROOT, claim.entityId, claim.actionType, claim.nonce, claim.regionId);
        }
        ...
        if (claim.entityId === 0 && claim.nonce === 0) {
            ...
            return this.emitVerdict(VERDICT_REJECT, REASON_BAD_NONCE, claim.entityId, claim.actionType, claim.nonce, claim.regionId);
        }
        ...
        if (nonceCheck === REASON_NONCE_EXPIRED) {
            return this.emitVerdict(VERDICT_RESYNC, REASON_NONCE_EXPIRED, claim.entityId, claim.actionType, claim.nonce, claim.regionId);
        }
        ...
        if (valueClass === VALUE_CLASS_HIGH) {
            ...
            if (!proof || proof.length === 0) {
                ...
                return this.emitVerdict(VERDICT_REJECT, REASON_NEEDS_PROOF, claim.entityId, claim.actionType, claim.nonce, claim.regionId);
            }
            ...
            if (epochState === KEY_EPOCH_INACTIVE) {
                return this.emitVerdict(VERDICT_REJECT, REASON_BAD_KEY_EPOCH, claim.entityId, claim.actionType, claim.nonce, claim.regionId);
            }
        ...
        if (!heuristicOk) {
            return this.emitVerdict(VERDICT_REJECT, REASON_PHYSICS, claim.entityId, claim.actionType, claim.nonce, claim.regionId);
        }
        return this.emitVerdict(VERDICT_PASS, REASON_NONE, claim.entityId, claim.actionType, claim.nonce, claim.regionId);
    }
    ...
    consumeVerdict(out, outOffset = 0) {
        if (this.verdictHead >= this.verdictTail)
            return false;
        if (outOffset < 0 || outOffset + VERDICT_RECORD_STRIDE > out.length)
            return false;
    ...
    tickWithDecay(t) {
        this.setTick(t);
        if (this.violationDecayPerTick === 0)
            return;
    ...
    checkAndStampNonce(entityId, nonce, claimTick) {
        const tickDelta = ((this.currentTick - claimTick) >>> 0);
        if (tickDelta > this.nonceTtlTicks)
            return REASON_NONCE_EXPIRED;
    ...
    gcExpiredNonces() {
        for (let i = 0; i < this.nonceTableSize; i++) {
            ...
            if (eHash === NONCE_KEY_EMPTY || eHash === NONCE_KEY_TOMBSTONE)
                continue;
    ...
    emitVerdict(kind, reason, entityId, actionType, nonce, regionId) {
...
//# sourceMappingURL=loom-verify.js.map