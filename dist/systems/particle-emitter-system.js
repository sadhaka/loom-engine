    ...
    update(world, dt) {
        ...
        for (let i = 1; i < hwm; i++) { // index 0 is NULL_ENTITY
            ...
            for (let k = 0; k < total; k++) {
                ...
                const slot = particles.spawnRaw(x, y, z, SCRATCH_DIR.x * speed, SCRATCH_DIR.y * speed, SCRATCH_DIR.z * speed, ax, ay, az, life, startSize, endSize, sr, sg, sb, sa, er, eg, eb, ea, additive);
                if (slot < 0)
                    break; // budget hit
                if (k < burstCount)
                    spawnedFromBurst++;
