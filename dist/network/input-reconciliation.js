...
function requireTick(value, name) {
    if (!Number.isInteger(value) || value < 0 || value > INT32_MAX) {
        throw new RangeError('InputReconciliation: ' + name + ' must be a non-negative Int32 integer, got ' + value);
    }
}
function requireFixed(value, name) {
    if (!Number.isInteger(value) || value < INT32_MIN || value > INT32_MAX) {
        throw new RangeError('InputReconciliation: ' + name + ' must be an Int32 integer, got ' + value);
    }
}
...
export function floatToFixed(value) {
    ...
    if (fixed < INT32_MIN || fixed > INT32_MAX) {
        throw new RangeError('InputReconciliation.floatToFixed: ' + value
...
export function fixedToFloat(fixed) {
...
export class InputReconciliation {
    ...
    lastRecordedTick = EMPTY_TICK;
    constructor(capacity) {
        if (!Number.isInteger(capacity) || capacity < 1 || capacity > MAX_HISTORY_CAPACITY) {
            throw new RangeError('InputReconciliation: capacity must be an integer in [1, '
    ...
    // newer tick, throws.
    record(tick, xFixed, yFixed, inputMask) {
        ...
        if (this.lastRecordedTick !== EMPTY_TICK) {
            ...
            if (tick < oldest || tick > newest) {
                throw new RangeError('InputReconciliation.record: tick ' + tick + ' out of the ordered window ['
    ...
    reconcile(serverTick, serverXFixed, serverYFixed) {
    ...
    readSlot(tick, out) {
        ...
        const slot = (tick % this.capacity) * SLOT_STRIDE;
        if ((this.history[slot] ?? EMPTY_TICK) !== tick)
            return false;
    ...
    static smoothVisual(clientXFixed, clientYFixed, serverXFixed, serverYFixed, lerpFixed, out) {
        ...
        if (lerp < 0)
            lerp = 0;
        else if (lerp > FIXED_POINT_ONE)
            lerp = FIXED_POINT_ONE;
...
//# sourceMappingURL=input-reconciliation.js.map