...
var out = {
  meta: {
    ...
    key: KEY,
    note: 'Generated by the real TS modules. One seeded run: build S0 (12 villagers, 4 regions) -> tickEpoch live epochs 1..2 appended to an HMAC EventChain -> suspend (snapshot @ index 0, tail = 2 events; the bundle CARRIES its ChainSeal + signed identity binding structurally, bundle format v3) -> resume at currentEpoch 14 (snapshot verified, structural seal + tail HMAC verified fail-closed, tail replayed by the recorded-mutation reducer, then 12 offline epochs resolved / 0 voided) -> partitionRegions on the resumed (server) and pre-suspend (client) states -> diffRegionLeaves reports exactly the 2 regions the offline proposals touched (east + south) -> applyPartialSync pulls only those 2 partitions, verifies each leaf, recombines with the kept cached regions, and proves the recombined root equals the server root. Consumers (tests/plaza-persistent.test.ts headless via src; demo/plaza-persistent/main.ts in-browser via dist) re-drive the same inputs and must reproduce every pinned hash. The seal expectations pin the truncation hole a bare hash chain cannot see: a truncated tail verifies WITHOUT the seal and is rejected WITH it - which is why suspend() now embeds the seal in the bundle and resume() rejects an end-truncated tail outright.',
  },
  inputs: {
    key: KEY,
    worldId: WORLD,
