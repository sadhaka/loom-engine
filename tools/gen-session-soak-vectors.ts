...
var out = {
  meta: {
    ...
    key: KEY,
    note: 'Generated by the real TS suspend/resume + catchUpEpochs. BUNDLE FORMAT v2: every embedded bundle carries a structural ChainSeal (seal field) which TS resume() verifies fail-closed; ports that have not grown the check yet ignore the extra field. Five cases: S1 pins 120-epoch catch-up composability (four 30-epoch chunks == single-shot, with 30/60/90 checkpoints); S2 pins the zero-catch-up resume boundary and mid-chain suspend (snapshotEventIndex 2) snapshot-position independence; S3 pins three suspend->resume->append->re-suspend cycles on ONE chain == one 21-epoch resume; S4 pins the sealed 21-record chain verifying across the whole gap (tests add the truncation/tamper negative space - bare verifyRecords without a seal cannot see a cut tail, which is why the bundle now CARRIES the seal structurally); S5 pins void-at-scale (resolve 100 of 500, void 400) and a deterministic SECOND resume across the void boundary. Ports (Rust/Python) gain v3_5 consumers in a follow-up; the format matches what golden_epoch.rs / golden_session.rs parse.',
  },
  cases: cases,
};
